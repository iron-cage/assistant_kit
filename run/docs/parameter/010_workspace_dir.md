...
- **Where It Flows:** `runbox.yml workspace_dir:` → `--build-arg WORKSPACE_DIR` → `WORKDIR`, `ENV HOME`, `COPY`, and `RUN` paths in planner/cook/test stages; `$WORKSPACE_DIR` in all `runbox-run` volume mounts and the test entrypoint path
