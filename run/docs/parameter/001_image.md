...
```
`_build()` invokes `docker build -f run/runbox.dockerfile -t workspace_test .`. `cmd_test()` runs `docker run ... workspace_test /workspace/verb/test`. Build-cache volumes are also derived from the tag: `workspace_test_target`, `workspace_test_plugin_targets` — so both images coexist on the same host with fully isolated caches.
