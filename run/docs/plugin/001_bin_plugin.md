...
Configured via `runbox.yml` key `bin_plugin: name:/container/path`. Current use: `w3:/usr/local/bin/w3`. Plugin logic lives entirely in `run/plugins.sh` — core `docker-run` has no plugin knowledge. A second binary plugin slot requires additions to `plugins.sh` only; `docker-run` is unchanged.
