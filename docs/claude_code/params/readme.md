...
- **In Scope**: All 65 parameters — positional args, long/short flags, `CLAUDE_CODE_*` env vars, `ANTHROPIC_*` env vars, `~/.claude/settings.json` config keys.
...
| model.md | `--model` — model alias or full ID |
...
| 4 | [model](model.md) | `--model <model>` | — | — | string | `claude-sonnet-4-6` | Model alias or full ID |
...
| 11 | [permission_mode](permission_mode.md) | `--permission-mode <mode>` | — | — | enum | `default` | `default` `acceptEdits` `bypassPermissions` `dontAsk` `plan` `auto` |
...
| 20 | [allowed_tools](allowed_tools.md) | `--allowed-tools <tools...>` | — | — | string[] | all | Allowlist of permitted tools |
| 21 | [disallowed_tools](disallowed_tools.md) | `--disallowed-tools <tools...>` | — | — | string[] | none | Denylist of forbidden tools |
...
| 31 | [effort](effort.md) | `--effort <level>` | — | — | enum | `medium` | `low` `medium` `high` `max` |
