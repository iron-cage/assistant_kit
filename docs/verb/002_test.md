...
`verb/test` doubles as the runbox `test_script`: `cmd_test()` in `docker-run` mounts and executes `/workspace/module/<name>/verb/test` inside the container. The `_ensure_image()` probe checks for this file inside the image before running; a stale image (missing the script) triggers an automatic rebuild. See `docs/runbox/parameter/005_test_script.md`.
