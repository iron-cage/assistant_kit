...
| `CLAUDE_CODE_FORCE_SESSION_PERSISTENCE` | 📖 Documented | Set to `1` to override the exclusion above when it's a false positive (e.g. inherited via `screen` or a background launcher) | v2.1.178+ auto-detects the tmux case, so tmux no longer needs this. |
