...
- `SessionStats`: per-session totals (entries, input/output tokens, cache tokens, first/last timestamps, agent-session flag, `max_context_tokens`, `last_context_tokens`). The two context figures are both `input_tokens + cache_read + cache_creation` for one deduplicated call, but answer different questions: `max_context_tokens` is a high-water mark that never falls, while `last_context_tokens` is the newest call's — what the conversation occupies *now*, which follows a compaction back down. Neither is derivable from the `total_*` sums, since every turn re-sends the whole conversation and those sums exceed the window many times over in a long session. Also aggregates `cwd` (from the first parsed line carrying one) and `model` (from the first assistant message carrying one) — both first-entry-wins, `None` when no line/message carries the field. Used by the `claude_storage` crate's `.usage` command as its Dir column (see [`docs/cli/command/13_usage.md`](../../../claude_storage/docs/cli/command/13_usage.md)).
