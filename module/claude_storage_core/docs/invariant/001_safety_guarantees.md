...
**Format validation:** All JSON parsing validates structure and syntax. A line with invalid JSON syntax is silently skipped line-by-line, not surfaced with a warning (BUG-489). A line that fails to decode as UTF-8 (e.g. a crash-truncated write) is silently skipped the same way by every JSONL reader in this crate: `Session::search()` and `export::export_json()` (BUG-503), plus `Session::load_entries()`, `Session::count_entries()`, and `Session::stats()` (BUG-504) — all five now stream per-line via `BufReader::lines()` rather than validating UTF-8 across the whole file at once with `fs::read_to_string()`. The library never panics on corrupted input.
