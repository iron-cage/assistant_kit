...
fn bug_reproducer_hyphen_prefixed_with_internal_hyphens()
{
  ...
  let encoded = "-home-alice-projects-consumer-app-module-claude-storage--default-topic";

  let decoded = decode_path( encoded ).unwrap();

  // Should decode to actual filesystem path with underscores
  let expected = "/home/alice/projects/consumer-app/module/claude_storage/-default_topic";
