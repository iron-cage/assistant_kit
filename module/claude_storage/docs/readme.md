...
- **In Scope:** CLI command and parameter reference (`cli/`), feature requirements (`feature/`), operational procedures (`operation/`), algorithms (`algorithm/`), system invariants (`invariant/`), and doc cross-reference graph.
...
| `algorithm/` | Algorithm design and correctness documentation |
...
**Contents**:
- [command/readme.md](cli/command/readme.md) — All 16 commands (15 active, 1 deprecated) with syntax, parameters, examples
...
#### [Agent Session Tracking](algorithm/001_agent_session_tracking.md)

**Discover and enumerate agent sessions across both flat and hierarchical storage formats.**

**Contents**:
- Sub-agent conversations (`agent-*.jsonl` format)
...
2. **`algorithm/001_agent_session_tracking.md`** - Understand agent session discovery and tracking
