...
| 9 | [`path::`](09_path.md) | varies | varies | varies | Path argument (semantics vary by command) | 16 |
...
| 13 | [`session::`](13_session.md) | [`SessionFilter`](../type/08_session_filter.md) / [`SessionId`](../type/09_session_id.md) | — | ID substring or exact | Session filter or scope pin | 5 |
...
| 17 | [`topic::`](17_topic.md) | [`TopicName`](../type/12_topic_name.md) | — | identifier string | Session topic suffix | 6 |
...
| 41 | [`latest::`](41_latest.md) | Boolean | `0` (effective default) | `0`, `1` | Most-recent session selector for `.session.path` | 1 |
| 42 | [`full::`](42_full.md) | Boolean | unset (folding active) | `0`, `1` | Unfold long turns for `.tail` | 1 |
| 43 | [`compact::`](43_compact.md) | Boolean | unset (full bodies) | `0`, `1` | One line per turn for `.tail` | 1 |
| 44 | [`live::`](44_live.md) | Boolean | unset (no filter) | `0`, `1` | Attached-process filter for `.projects` | 1 |

**Total:** 44 parameters, all implemented across their specified commands — including `live::` (row 44), introduced for `.projects`' attachment-state view (see `../command/07_projects.md` and `../../algorithm/002_session_liveness.md`); `full::`/`compact::` (rows 42-43), introduced for `.tail`'s turn-grouped rendering (see `../command/12_tail.md` — mutually interacting, `compact::` wins when both are set); `latest::` (row 41), introduced for `.session.path` (see `../command/16_session_path.md`); `depth::` (row 26) for `.usage` and `.rollup` (see `../command/13_usage.md`, `../command/14_rollup.md`); `filter::`, `detail::`, `ids::` (rows 29-31) for `.projects`' absorption of `.list` (see `../command/07_projects.md` and `../command_group/readme.md § Command Removal: .list -> .projects`); `last::`/`detail::` (rows 25, 30) for `.show`'s project-overview branch (see `../command/03_show.md`); `fields::`/`index::` (rows 32-33) for `.show`'s attribute-projection and single-message selection (see `../command/03_show.md`); `group::`/`sort::`/`order::`/`model::`/`columns::` (rows 34-38), introduced entirely for `.rollup` (see `../command/14_rollup.md`); and `session_ids::`/`agents::` (rows 39-40), introduced entirely for `.cost` (see `../command/15_cost.md` — `agents::` is deliberately distinct from row 1's `agent::` session-type filter, see `40_agents.md`).
