...
| `34_group.md` | group:: — aggregation dimension for .table |
| `35_sort.md` | sort:: — sort column for .table's grouped rows |
| `36_order.md` | order:: — sort direction for .table |
| `37_model.md` | model:: — model-name substring filter for .table |
| `38_columns.md` | columns:: — column projection for .table |
...
| 9 | [`path::`](09_path.md) | varies | varies | varies | Path argument (semantics vary by command) | 14 |
...
| 34 | [`group::`](34_group.md) | String enum | `session` | `session`, `project`, `model`, `day` | Aggregation dimension for `.table` | 1 |
| 35 | [`sort::`](35_sort.md) | String enum | `total` | `total`, `input`, `output`, `cache`, `max_context`, `calls`, `sessions`, `group` | Sort column for `.table`'s grouped rows | 1 |
| 36 | [`order::`](36_order.md) | String enum | `desc` | `asc`, `desc` | Sort direction for `.table` | 1 |
| 37 | [`model::`](37_model.md) | String | — | any string | Model-name substring filter for `.table` | 1 |
| 38 | [`columns::`](38_columns.md) | String (comma list) | 9-column default | 11 valid keys | Column projection for `.table` | 1 |

**Total:** 38 parameters, all implemented across their specified commands — including `depth::` (row 26) for `.usage` and `.table` (see `../command/13_usage.md`, `../command/14_table.md`); `filter::`, `detail::`, `ids::` (rows 29-31) for `.projects`' absorption of `.list` (see `../command/07_projects.md` and `../command_group/readme.md § Command Removal: .list -> .projects`); `tail::`/`detail::` (rows 25, 30) for `.show`'s project-overview branch (see `../command/03_show.md`); `fields::`/`index::` (rows 32-33) for `.show`'s attribute-projection and single-message selection (see `../command/03_show.md`); and `group::`/`sort::`/`order::`/`model::`/`columns::` (rows 34-38), introduced entirely for `.table` (see `../command/14_table.md`).
