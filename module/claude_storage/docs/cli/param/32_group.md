...
Aggregation dimension for [`.rollup`](../command/14_rollup.md)'s grouped result set — which sessions collapse into the same row.
...
# Cost per project
.rollup group::project

# Which model was used most?
.rollup group::model sort::calls

# Busiest days
.rollup group::day sort::sessions

# Invalid value
.rollup group::user        # "group must be session|project|model|day, got user"
```
...
|---|---------|---------|-------|
| 14 | [`.rollup`](../command/14_rollup.md) | `session` | Aggregation dimension for the whole result set |
