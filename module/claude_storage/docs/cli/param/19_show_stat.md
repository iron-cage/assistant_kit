...
**Purpose:** Accepted for backward compatibility but has no effect in any `.show` branch — session-detail content mode's key:val attribute block, `show_metadata::1` mode's structured fields, and the project-overview summary block (see [`../command/03_show.md`](../command/03_show.md)) all already show total entry/session counts and timestamp range unconditionally. Independent of `show_tokens::`.
