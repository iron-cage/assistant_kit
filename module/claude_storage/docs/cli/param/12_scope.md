...
**Commands:** [`.projects`](../command/07_projects.md), [`.list`](../command/02_list.md), [`.count`](../command/04_count.md), [`.search`](../command/05_search.md), [`.show`](../command/03_show.md), [`.export`](../command/06_export.md), [`.usage`](../command/13_usage.md) — all seven implemented, each genuinely narrowing its discovery/search/count/lookup behavior per the semantics below. See the Status column below and [Scope Configuration](../param_group/05_scope_configuration.md) for the full per-command breakdown.
...
|---------|---------|-----------------|--------|
| `.list` | `global` | Discovery boundary for project listing when `type::` is `all` (default); `type::uuid`/`type::path` ignore it | Implemented |
...
|---|---------|---------|-------|
| 2 | [`.list`](../command/02_list.md) | `global` | Discovery boundary for project listing when `type::` is `all` — implemented |
