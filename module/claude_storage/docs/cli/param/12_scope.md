...
| `.rollup` | `local` | Which sessions' stats are aggregated into the grouped table — identical boundary to `.usage`, different rendering afterward | Implemented |
...
| 14 | [`.rollup`](../command/14_rollup.md) | `local` | Grouped table aggregation boundary, same as `.usage` — implemented |
