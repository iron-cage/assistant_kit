...
- The `model` column is deliberately excluded from this table: `SessionStats` does carry an aggregate `model` field (first assistant entry's model wins, same idiom as `cwd`), but `.usage`'s own `UsageRow`/output table does not surface it — adding a Model column here is a straightforward future addition, not part of this command's initial scope.
