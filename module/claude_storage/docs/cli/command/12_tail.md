...
- **Rule line** — the turn boundary. A rule, not a blank line: message bodies contain blank lines of their own, so whitespace alone can never be an unambiguous separator. It carries the speaker (`You` / `Claude`), relative age, and wall clock.
...
- **Tool calls** — `⚙ Name · summary`, with the result folded onto the same line as a right-aligned `↳ 3 lines` / `↳ 1 line` / `↳ empty` / `↳ error`. The summary is the tool input's most telling string value (`command` for `Bash`, `file_path` for `Read`/`Edit`/`Write`, `pattern` for `Grep`/`Glob`, …); paths elide from the front (`…/src/cli/tail.rs`) since the filename identifies them, everything else from the back.
