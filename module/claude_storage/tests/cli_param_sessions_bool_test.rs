//! Edge case tests for the `show_sessions::` (bool) parameter.
//!
//! ## Source
//!
...
///
/// ## Purpose
/// Validates that `show_sessions::1` forces session display even without any
...
///
/// ## Validation Strategy
/// Create project with sessions. Run `.list ``show_sessions::``1` with no other filters.
...
///
/// ## Purpose
/// Validates that `show_sessions::0` suppresses session rows even when `session::` filter present.
///
/// ## Coverage
/// Exit 0; no sessions displayed despite `session::` filter.
///
/// ## Validation Strategy
/// Create sessions. Run `.list ``show_sessions::0`` ``session::defaul``t`.
...
///
/// ## Purpose
/// Validates that `show_sessions::0` suppresses explicit session expansion
...
///
/// ## Validation Strategy
/// Create agent session. Run `.list ``show_sessions::0`` ``agent::``1`.
...
///
/// ## Purpose
/// Validates that `show_sessions::0` overrides `min_entries::` auto-enable.
///
/// ## Coverage
/// Exit 0; no sessions displayed despite `min_entries::` filter.
///
/// ## Validation Strategy
/// Create sessions with varying entry counts. Run `.list ``show_sessions::0`` ``min_entries::``2`.
...
///
/// ## Purpose
/// Validates that without `show_sessions::` and no session filters, only project
...
///
/// ## Purpose
/// Validates that `session::` auto-enables session display when `show_sessions::` omitted.
///
/// ## Coverage
/// Exit 0; sessions displayed automatically (auto-enable triggered by `session::` filter).
///
/// ## Validation Strategy
/// Create session. Run `.list ``session::defaul``t` with no `show_sessions::`.
...
///
/// ## Purpose
/// Validates that `show_sessions::yes` is accepted by the implementation
/// (the parser treats "yes" as a truthy value equivalent to `show_sessions::1`).
///
/// ## Coverage
/// Exit 0; `show_sessions::yes` accepted without error.
///
/// ## Validation Strategy
