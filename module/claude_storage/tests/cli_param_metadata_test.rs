//! Edge case tests for the `show_metadata::` parameter.
//!
//! ## Source
//!
...
//! - EC-5: `show_metadata::1` output includes entry count
//! - EC-6: `show_metadata::1` output includes session timestamps
...
///
/// ## Purpose
/// Validates that `show_metadata::0` shows conversation message content.
///
/// ## Coverage
/// Exit 0; conversation content present in output.
///
/// ## Validation Strategy
/// Create session with known message text. Run `.show ```show_metadata::```0`.
...
///
/// ## Purpose
/// Validates that `show_metadata::1` suppresses message text and shows technical metadata.
///
/// ## Coverage
/// Exit 0; metadata fields present; conversation content absent.
///
/// ## Validation Strategy
/// Create session with unique message text. Run `.show ```show_metadata::```1`.
...
///
/// ## Purpose
/// Validates that `show_metadata::true` is accepted by the unilang boolean parser
/// as a truthy value (equivalent to `show_metadata::1`).
///
/// ## Coverage
/// Exit 0 or non-type-error exit; no type validation error emitted.
///
/// ## Validation Strategy
/// Run `.show ```show_metadata::tru```e`. Assert no type validation error (may exit 1
...
///
/// ## Purpose
/// Validates that omitting `show_metadata::` defaults to showing conversation content.
///
/// ## Coverage
/// Exit 0; conversation content shown identical to `show_metadata::0`.
///
/// ## Validation Strategy
...
///
/// ## Purpose
/// Validates that `show_metadata::1` output shows a count of entries in the session.
///
/// ## Coverage
/// Exit 0; entry count field present in metadata output.
///
/// ## Validation Strategy
/// Create 4-entry session. Run `.show ```show_metadata::```1`. Assert output contains
...
///
/// ## Purpose
/// Validates that `show_metadata::1` output shows first/last entry timestamp fields.
///
/// ## Coverage
/// Exit 0; timestamp fields present in output (ISO 8601 or similar).
///
/// ## Validation Strategy
/// Create session with synthetic timestamps. Run `.show ```show_metadata::```1`.
...
fn ec_6_metadata_1_includes_timestamps()
{
  ...
  assert!(
    ...
    "EC-6: show_metadata::1 must show timestamp fields; got: {output}"
  );
}
