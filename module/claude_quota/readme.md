...
Provides types and transport functions for six Anthropic quota and account APIs:
...
- **`fetch_oauth_account`**: GET `/api/oauth/account` — account identity, billing type, and subscription state.
- **`fetch_userinfo`**: GET `/api/oauth/userinfo` — stable user identity (tagged_id, uuid, email).
- **`fetch_subscriptions`**: GET `/api/oauth/claude_cli/subscriptions` — all org memberships with billing state.
...
| `enabled` | `fetch_rate_limits`, `fetch_oauth_usage`, `fetch_oauth_account`, `fetch_userinfo`, `fetch_subscriptions`, `fetch_claude_cli_roles` | `ureq ~3` |
...
pub const OAUTH_USERINFO_URL           : &str;  // GET /api/oauth/userinfo
pub const CLAUDE_CLI_SUBSCRIPTIONS_URL : &str;  // GET /api/oauth/claude_cli/subscriptions
...
pub struct OauthAccountData   { billing_type: String, has_max: bool, org_created_at: String }
pub struct MembershipRaw      { index, billing_type, has_max, capabilities, org_created_at }
pub struct UserinfoData       { tagged_id, uuid, email_address: String }
