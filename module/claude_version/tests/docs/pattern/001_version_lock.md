...
- **Input A:** `clv .version.install version::stable dry::1` → output contains `"autoUpdates = false"` (lock applied)
- **Input B:** `clv .version.install version::latest dry::1` → output contains `"autoUpdates = true"` (lock inverted)
