...
- [Path Discovery](user_story/008_path_discovery.md)
...
Command integration test files use `IT-` as the test case ID prefix (e.g., `IT-1`, `IT-2`) rather than the rulebook-specified `INT-`. Rationale: `IT-` was established early and is consistent across all 16 command test files and their corresponding `tests/cli/` source functions; renaming would require synchronized changes across 16 spec files and all source function tables without any functional benefit.
