...
- **Out of Scope**: Config resolution (-> `.config` command), version management (-> `002_version_upgrade.md`).
...
- **When:** `cm .settings.get key::theme` and `cm .settings.get key::nonexistent`
...
- **When:** `cm .settings.set key::theme value::light dry::1`
...
- **Given:** `~/.claude/settings.json` exists
- **When:** `cm .settings.set key::theme value::light`
...
- **Given:** `~/.claude/settings.json` exists
- **When:** `cm .settings.set key::a value::true` then `cm .settings.set key::b value::42` then `cm .settings.set key::c value::3.14`
