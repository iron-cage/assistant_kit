...
|----|-----------|----------|
| GI-1 | `key::K value::V` -> set mode, writes to user config | Valid: set |
| GI-2 | `key::K value::V scope::project` -> set mode, writes to project config | Valid: set + scope |
| GI-3 | `key::K unset::1` -> unset mode, removes key from user config | Valid: unset |
| GI-4 | `key::K unset::1 scope::project` -> unset mode, removes from project config | Valid: unset + scope |
| GI-5 | `key::K value::V unset::1` -> exit 1, mutual exclusion | Invalid: mutual exclusion |
| GI-6 | `value::V` without `key::K` -> exit 1, key required | Invalid: missing key |
| GI-7 | `unset::1` without `key::K` -> exit 1, key required | Invalid: missing key |
| GI-8 | `scope::project` without write operation -> exit 1, scope only applies to writes | Invalid: scope without write |
| GI-9 | `key::K` alone -> get mode (no write), scope:: ignored | Valid: get mode |
| GI-10 | `key::K value::V dry::1` -> preview, no file modification | Cross-group: dry run |
