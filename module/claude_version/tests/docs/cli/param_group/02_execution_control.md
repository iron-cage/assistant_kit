...
- **Input A:** `clv .version.install dry::1 force::1` → `[dry-run]` prefix; no install (dry wins over force)
- **Input B:** `clv .version.install dry::0 force::1` → no dry-run prefix; force install proceeds
...
- **When:** `clv .version.guard dry::0 force::1`
...
- **Given:** clean environment
- **When:** `clv .version.guard dry::1 force::0`
...
- **Given:** no preference stored
- **When:** `clv .version.guard dry::1 force::1 interval::0`
...
- **Given:** clean environment
- **When:** `clv .version.install force::1 dry::1` with `dry::` intentionally absent → `clv .version.install force::1`
...
- **Given:** clean environment
- **When:** `clv .version.install dry::0 force::0 dry::1` — no: `clv .version.guard dry::0 force::0`
- **Then:** behavior identical to `clv .version.guard` with no flags; explicit zeros equal absent
- **Exit:** 0
- **Source:** [004_parameter_interactions.md](../../../../docs/cli/004_parameter_interactions.md)
