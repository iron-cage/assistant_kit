...
| EC-1 | Absent `version::` → defaults to stable | Default |
| EC-2 | `version::STABLE` → exit 1 (case-sensitive) | Validation: case |
| EC-3 | `version::` (empty) → exit 1 | Validation: empty |
| EC-4 | `version::1.2` → exit 1 (two-part semver) | Validation: format |
| EC-5 | `version::1.2.3.4` → exit 1 (four-part semver) | Validation: format |
| EC-6 | `version::01.02.03` → exit 1 (leading zeros) | Validation: format |
...
- Case sensitivity: 1 test (EC-2)
- Empty value: 1 test (EC-3)
- Format violations: 3 tests (EC-4, EC-5, EC-6)
