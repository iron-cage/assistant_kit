...
- **Out of Scope**: Key parsing (→ `04_settings_key.md`), settings file I/O (→ `../command/11_settings_set.md`).
...
| TC-7 | Missing `value::` entirely → exit 1 | Validation: required |
| TC-8 | `value::` (empty) → exit 1 | Validation: empty |
...
- Required field: 1 test (TC-7)
- Empty value: 1 test (TC-8)
...
- **When:** `clv .settings.set key::flag value::true` then read `~/.claude/settings.json`
...
- **Given:** clean isolated HOME
- **When:** `clv .settings.set key::flag value::false` then read `~/.claude/settings.json`
...
- **Given:** clean isolated HOME
- **When:** `clv .settings.set key::count value::42` then read `~/.claude/settings.json`
...
- **Given:** clean isolated HOME
- **When:** `clv .settings.set key::rate value::3.14` then read `~/.claude/settings.json`
...
- **Given:** clean isolated HOME
- **When:** `clv .settings.set key::theme value::dark` then read `~/.claude/settings.json`
...
- **Given:** clean isolated HOME
- **When:** `clv .settings.set key::special value::NaN` then read `~/.claude/settings.json`
...
### TC-7: Missing `value::` entirely → exit 1

- **Given:** clean environment
- **When:** `clv .settings.set key::theme` (no `value::` parameter)
