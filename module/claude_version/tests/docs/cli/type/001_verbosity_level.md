...
| EC-1 | Absent `v::` → defaults to level 1 | Default |
| EC-2 | `v::3` → exit 1 (out of range) | Validation: range |
| EC-3 | `v::` (empty) → exit 1 | Validation: empty |
| EC-4 | `v::abc` → exit 1 (non-integer) | Validation: type |
| EC-5 | `v::-1` → exit 1 (negative integer) | Validation: range |

## Test Coverage Summary

- Level Semantics: 3 tests (TC-1, TC-2, TC-3)
- Default Behavior: 1 test (EC-1)
- Range validation: 2 tests (EC-2, EC-5)
- Type validation: 2 tests (EC-3, EC-4)

**Total:** 8 tests

**Behavioral Divergence Pair:** TC-1 (`cm .status v::0` → raw values only, no "version:" label) ↔ TC-3 (`cm .status v::2` → labeled output plus diagnostic context lines absent from v::1)
...
### EC-1: Absent `v::` → defaults to level 1

- **Given:** clean environment
- **When:** `cm .status` (no `v::` parameter)
