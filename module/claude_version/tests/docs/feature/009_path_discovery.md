...
- **Purpose**: FT- test cases for the `.version.paths` command — path enumeration, single-key lookup, verbosity-tiered unresolvable-path handling, and error paths.
...
Feature test surface for `.version.paths`. See [feature/009_path_discovery.md](../../../docs/feature/009_path_discovery.md) for specification.

## Behavioral Divergence Pair

Two valid `.version.paths` invocations that produce structurally different outcomes:

- **Input A:** `HOME=/valid/home clv .version.paths` → exit 0; all 5 labeled paths emitted
- **Input B:** `HOME= clv .version.paths` → exit 2; no path output

## Test Case Index

| FT | Scenario | Source fn |
|----|----------|-----------|
| FT-1 | `.version.paths` show-all exits 0; all 5 keys present with labels | `ft1_show_all_exits_0_with_all_keys` |
| FT-2 | `.version.paths v::0` outputs plain unlabeled paths, one per line | `ft2_v0_output_is_unlabeled` |
| FT-3 | `.version.paths key::versions_dir` returns exactly that single path | `ft3_single_key_returns_one_path` |
...
- **Given:** `HOME=/tmp/test_home`
- **When:** `clv .version.paths key::versions_dir v::0`
