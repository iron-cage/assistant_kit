...
- **Out of Scope**: Type inference algorithm (→ `../../algorithm/001_settings_type_inference.md`), resolution algorithm step-by-step (→ `../../algorithm/002_config_resolution.md`), deprecated `.settings.*` commands (→ `003_settings_management.md`).
...
- **Input A:** `cm .config` (no params) → shows all resolved keys with source annotations
- **Input B:** `cm .config key::model` → shows single key effective value with source layer
...
- **When:** `cm .config key::autoUpdates value::false`
...
- **When:** `cm .config key::model value::claude-haiku-4-5-20251001 scope::project`
...
- **When:** `cm .config key::hasCompletedOnboarding`
...
- **Given:** isolated HOME; empty `settings.json`
- **When:** `cm .config key::myCustomKey value::customValue`
