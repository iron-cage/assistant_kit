...
- **Out of Scope**: Individual command normal-mode behavior (-> `001_version_management.md`, `002_process_lifecycle.md`, `003_settings_management.md`).
...
- **Input A:** `cm .version.install dry::1` → stdout contains `"[dry-run]"` prefix; no actual install attempted
- **Input B:** `cm .version.install` (without dry::1) → stdout does NOT contain `"[dry-run]"` prefix; install attempted
...
- **When:** `cm .settings.set key::x value::1 dry::1`
