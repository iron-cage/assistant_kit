...
- **In Scope**: `.ps`, `.ps.kill`, no-process output, `dry::1`, `force::1`, `dry::1 force::1` precedence.
...
Two valid `.ps.kill` invocations produce distinct output when no processes are present:

- **Input A:** `clv .ps.kill` → stdout contains `"no active processes"` (normal mode, no dry prefix)
- **Input B:** `clv .ps.kill dry::1` → stdout contains `"[dry-run]"` prefix (dry mode preview)
