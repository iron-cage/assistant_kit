...
- **Input A:** `cm .processes.kill` → stdout contains `"no active processes"` (normal mode, no dry prefix)
- **Input B:** `cm .processes.kill dry::1` → stdout contains `"[dry-run]"` prefix (dry mode preview)
...
- **Given:** environment with no detectable claude processes
- **When:** `cm .processes.kill dry::1 force::1`
