...
- **Out of Scope**: Type inference algorithm (-> `../../algorithm/001_settings_type_inference.md`), dry-run semantics (-> `004_dry_run.md`).
...
- **Input A:** `cm .settings.set key::flag value::true` → `settings.json` contains `"flag": true` (JSON boolean)
- **Input B:** `cm .settings.set key::count value::42` → `settings.json` contains `"count": 42` (JSON integer)
...
- **Given:** isolated HOME with empty `settings.json`
- **When:** `cm .settings.set key::count value::42`
...
- **Given:** isolated HOME with empty `settings.json`
- **When:** `cm .settings.set key::color value::blue` then `cm .settings.get key::color`
...
- **When:** `cm .settings.set key::x value::1`
