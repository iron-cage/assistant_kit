...
- Duplicating Task 005's nested-`env`-key `config_resolve.rs` fix inside this task — this task reuses that fix via `config_resolve::resolve()` rather than reimplementing nested-key parsing locally. This is a hard prerequisite, narrower than "wait on Task 005 to land": only the `env.DISABLE_AUTOUPDATER`/`env.DISABLE_UPDATES` rows (T01/T03/T07) need it; `autoUpdates`/`preferredVersionSpec`/`chmod` reporting is independent of Task 005. This task's Work Procedure step 1 gates on Task 005's `config_resolve.rs` Step 3 fix already being present — this task MUST NOT start before Task 005 reaches 🎯 (Verified), since `config_resolve::resolve()` today has no nested-`env`-object parsing at all (confirmed: `resolve()`'s Step 3 does only flat exact-key matching) and any T01/T03/T07 fixture exercising `env.DISABLE_AUTOUPDATER`/`env.DISABLE_UPDATES` would silently misreport a genuinely-compliant key as absent
...
1. Confirm Task 005 (`005_adopt_unused_version_pinning_mechanisms.md`) is in state 🎯 (Verified) — if not, STOP; this task's T01/T03/T07 rows depend on `config_resolve.rs` Step 3's nested-`env`-key fix, which does not exist until Task 005 lands. Once confirmed, read `module/claude_version/src/commands/status.rs` in full (current `v::2`/JSON arms) and `claude_version_core/src/version.rs` `versions_dir_path()`/`lock_version()` (current chmod mode semantics)
...
- `module/claude_version/task/unverified/005_adopt_unused_version_pinning_mechanisms.md` — T01/T03's `env.DISABLE_AUTOUPDATER`/`env.DISABLE_UPDATES` reporting depends on this task's `config_resolve.rs` Step 3 nested-key fix (see Out of Scope)
