...
Integration test planning for the `.version.history` command. See [commands.md](../../commands.md) for specification.
...
| -1 | Negative integer | Invalid: graceful degradation to 0 |
...
| TC-446 | `count::-1` (negative) → treated as 0, empty output | N | 0 | F1=-1 | Graceful Degradation |
...
- **Total:** 25 tests (15 positive, 10 negative)
- **Negative ratio:** 40.0% (meets ≥40% requirement)
- **TC range:** TC-425 to TC-449
...
|-----------|---------|-------|
| 0 | Success (with or without output) | TC-425 through TC-439, TC-446 |
| 1 | Invalid arguments (format, type, unknown param, syntax) | TC-440 through TC-443, TC-447 through TC-449 |
...
- (-1, 0/1/2): After unsigned conversion, `-1` becomes 0; same as `count::0` pruning rationale.
...
| Unilang adapter | Type mismatch for Integer param | 1 | TC-447, TC-448 |
...
### TC-446: count::-1 (negative) treated as 0

**Goal:** Negative count degrades gracefully to 0 (empty output), not a crash.
**Setup:** Network available (may need fetch, then show 0 entries).
**Command:** `cm .version.history count::-1`
**Expected:** Exit 0; empty output (same as count::0).
**Verification:**
- exit code 0
- stdout is empty
**Isolation:** Network-conditional.
...
|---------|-------|--------|
| No isolation needed | TC-440 to TC-443, TC-447 to TC-449 | Fails at adapter/validation, before state access |
| Network-conditional | TC-425 to TC-439, TC-446 | `skip_if_no_network()` helper |
| HOME isolation | TC-445 | `run_cm_with_env(&[...], &[("HOME", "")])` |
...
| Graceful Degradation | TC-446 | 1 |
| Type Validation | TC-447, TC-448 | 2 |
