...
- **Purpose**: Document the on-disk cache storing GitHub Releases API responses for `.version.list mode::history`.
...
- **Created:** On the first successful `.version.list mode::history` invocation when no cache file exists.
- **Refreshed:** On any `.version.list mode::history` invocation when the existing cache file's mtime age exceeds `CACHE_TTL_SECS` (3600 seconds).
...
A missing or corrupt cache file causes one additional GitHub API call on the next `.version.list mode::history` invocation; the file is then re-created. No permanent data loss occurs. Deletion is always safe.

### Features

| File | Relationship |
|------|-------------|
| [feature/001_version_management.md](../feature/001_version_management.md) | `.version.list` (`mode::history`) command that reads and writes this cache |
