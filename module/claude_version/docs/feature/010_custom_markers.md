...
- **In Scope**: `.version.mark` command, `CustomMarker` struct, `version-markers.json` storage, name validation, integration with `.version.install`/`.version.guard`/`.version.list`.
