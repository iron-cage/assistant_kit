...
**Responsibilities:** Shell scripts implementing the `do` protocol verbs for `svg_chart` (cargo ecosystem).
...
| File | Responsibility |
|------|---------------|
| `build` | Compile the crate: `cargo build -p svg_chart`. |
| `clean` | Remove build artifacts: `cargo clean -p svg_chart`. |
