...
- **Purpose**: Document how claude_tools aggregates all Layer 2 Claude CLI crates into a single `clt` binary via programmatic command registration.
...
**Architecture:** claude_tools is the Layer 3 super-app. It owns no domain logic of its own — its sole responsibility is to compose commands from five Layer 2 crates (`claude_assets`, `claude_manager`, `claude_profile`, `claude_runner`, `claude_storage`) into a single `CommandRegistry` and run the shared unilang pipeline.
...
claude_manager::register_commands(&mut registry)   // .status, .version.*, .processes.*, .settings.*
...
Duplicate registrations via `command_add_runtime` are silently skipped — the first registration wins. This means `claude_manager`'s `.status` takes precedence over `claude_storage`'s `.status`.
...
**Adapter reuse:** `src/main.rs` calls `claude_manager::adapter::argv_to_unilang_tokens()` for argv preprocessing. claude_tools does not implement its own adapter — it delegates to the manager's adapter, which covers all commands in the registry.
