...
- **Purpose**: Enforce that `agent_kit` contains no own type definitions, keeping it a pure facade.
...
(`claude_common`), Layer 1 (`claude_profile_core`, `claude_runner_core`, `claude_manager_core`),
and the out-of-hierarchy primitive (`claude_storage_core`) are listed. No Layer 2 CLI crate
(`claude_profile`, `claude_runner`, `claude_manager`, `claude_storage`) or Layer 3
(`claude_tools`) may appear.

### Violation Consequences

- **INV-1 violated:** Own types in `agent_kit` create a coupling point; consumers that only
