...
//!
//! Generic over the request type: this crate does not know what a daemon built
//! on it chooses to be asked. [`Response`] is not generic — its `{ok:true,
//! result}` / `{ok:false, error}` shape is the one thing every daemon built here
//! answers with, whatever `result` itself turns out to hold.
...
///
/// Comfortably above the time a slow request (e.g. one that has to wait on a
/// child process to register itself) legitimately takes — a client timeout
/// under it would abandon requests that were about to succeed. Everything else
/// answers in milliseconds, so the margin costs nothing in practice and only
/// matters when the daemon has genuinely stopped answering.
...
#[ inline ]
pub fn request< Req : Serialize >( socket_path : &Path, request : &Req ) -> Result< Response >
{
...
/// As [`request`].
#[ inline ]
pub fn request_within< Req : Serialize >
(
  socket_path : &Path,
  request : &Req,
  timeout : Duration,
...
/// failure.
#[ inline ]
pub fn call< Req : Serialize >( socket_path : &Path, request : &Req ) -> Result< serde_json::Value >
{
