...
//
// Fix(BUG-482): `claude_journal` was hardcoded into the Layer-1 arm below, but its own
...
fn layer_of( name : &str ) -> Option< u8 >
{
  match name
  {
    ...
    // (includes claude_journal, svg_chart, json_redact — see Fix(BUG-482) doc comment above)
    _ => None,
  }
}
