...
//
// Fix(BUG-008): doc comment said "18" while this constant already listed 19 entries
// (including `json_redact`) — the array itself was correct; only this count literal,
// plus 2 sibling doc-comment counts elsewhere in this file, had drifted stale.
...
//
// Fix(BUG-008): `claude_journal` was hardcoded into the Layer-1 arm below, but its own
...
fn layer_of( name : &str ) -> Option< u8 >
{
  match name
  {
    ...
    // (includes claude_journal — see Fix(BUG-008) doc comment above)
    _ => None,
  }
}
