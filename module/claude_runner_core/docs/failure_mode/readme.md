...
| 002 | [Diagnostic on Stdout](002_diagnostic_on_stdout.md) | Error text written to stdout, not stderr | stdout scan | pattern match | `classify_error()` scans both channels ✅; stdout content NOT forwarded to caller on failure ⚠️ BUG-247 |
| 003 | [CLAUDECODE Env Leak](003_claudecode_env_leak.md) | Inherited env var silently changes child behavior | env inspection | `CLAUDECODE` present | `unset_claudecode` default-on removes var before spawn ✅; no warning when `--keep-claudecode` disables protection ⚠️ BUG-248 |
