...
|---------|-------------|--------------|
| `"You've hit your limit"` | `RateLimit` | T03 — also appears in stderr |
...
```
priority order:
  1. AuthError  — "Your organization does not have access to Claude"
  2. RateLimit  — "You've hit your limit"
  3. ApiError   — "API Error: "
  4. Signal     — exit_code >= 128
  5. RateLimit  — exit_code == 2 (no text)
...
**Gap (BUG-247):** The stdout content itself is NOT forwarded to the caller when `exit_code != 0`. In `run_print_mode()`, the stdout forward (`print!("{out}")`) only executes on the success path (`exit_code == 0`). On failure, after stderr is forwarded and the error label is emitted, `run_print_mode()` calls `std::process::exit(output.exit_code)` without forwarding `output.stdout`. Any diagnostic text on stdout — e.g., `"API Error: 529 overloaded"` — is silently discarded. The caller receives only the classified label (`"Error: API error (exit 1)"`), not the raw diagnostic.

Fix location: `module/claude_runner/src/cli/mod.rs` — `run_print_mode()` — add stdout-to-stderr forward before the exit call when `exit_code != 0`. See BUG-247.
...
| test | `../../tests/classify_error_test.rs` | T03 (RateLimit in stderr), T04 (AuthError in stdout), T05 (ApiError in stderr), T12 (AuthError in stderr), priority test |
