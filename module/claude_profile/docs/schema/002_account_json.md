...
| `model` | string | `~/.claude/settings.json` → `model` | `save()` at save time (anthropic backend only — redirect saves never write it and strip a stray pre-gate one, [071](../feature/071_redirect_backend_accounts.md) AC-18); `switch_account()` restores on switch | `list()` → `model` field; `switch_account()` restores model to settings.json (anthropic; redirect switch removes the live key instead) | [014](../feature/014_rich_account_metadata.md) |
