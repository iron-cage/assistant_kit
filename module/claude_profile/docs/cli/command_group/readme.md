...
**Total:** 0 groups. All 15 live `clp` commands registered in `src/registry.rs` (`.credentials.status`, `.accounts`, `.account.limits`, `.account.save`, `.account.use`, `.account.delete`, `.account.relogin`, `.account.renewal`, `.account.inspect`, `.model`, `.models`, `.model.select`, `.token.status`, `.paths`, `.usage`) — plus the binary-specific `.` (`dot_routine`, registered inline in `src/cli.rs`) — were evaluated pairwise under the Representation Absorption Test. **Zero pairs qualify.**
