...
**Note:** `host::` is a display label only — it does not control ownership or access enforcement. The `owner` field in `{name}.json` (set explicitly via `write_owner()`, never written by `.account.save` or user-specified via CLI) is what governs which identity may operate on the account's credentials. See [feature/036_account_ownership.md](../../feature/036_account_ownership.md).
