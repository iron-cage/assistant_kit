...
- Each diagnostic line is prefixed with a UTC timestamp in `YYYY-MM-DD · HH:MM:SS UTC · ` format — the explicit `UTC` marker disambiguates it from any other (e.g. local-clocked) timestamp source appearing in the same combined log stream (BUG-338) — enabling correlation with watchdog and other time-stamped output.
...
- Full trace output for an expired account whose OAuth refresh succeeds:
  ```
  2026-06-25 · 16:40:04 UTC · alice@example.com  reading /home/user/.pro/.../alice@example.com.credentials.json
  2026-06-25 · 16:40:04 UTC · alice@example.com  GET https://api.anthropic.com/api/oauth/usage  token=sk-ant-oA3Txy6P1w...  exp=expired(2d 3h ago)
  2026-06-25 · 16:40:04 UTC · alice@example.com  result: Err(HTTP transport error: HTTP 401)
  2026-06-25 · 16:40:04 UTC · refresh  alice@example.com  should_retry=true (reason: HTTP transport error: HTTP 401)
...
- For a rate-limited account with a non-expired token (refresh not triggered):
  ```
  2026-06-25 · 16:40:04 UTC · alice@example.com  GET https://api.anthropic.com/api/oauth/usage  token=sk-ant-oA3Txy6P1w...  exp=valid(1h 22m left)
  2026-06-25 · 16:40:04 UTC · alice@example.com  result: Err(HTTP transport error: HTTP 429)
  2026-06-25 · 16:40:04 UTC · refresh  alice@example.com  should_retry=false (reason: HTTP transport error: HTTP 429)
  ```
- Full `.account.use` trace when quota fetch succeeds (subprocess always dispatched; Fix(BUG-285)):
  ```
  2026-06-25 · 16:40:04 UTC · account.use  alice@home.com  reading /home/user/.pro/.../alice@home.com.credentials.json
