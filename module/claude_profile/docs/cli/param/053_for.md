...
> **REMOVED (Feature 064):** The `for::` parameter has been removed from the unified parameter set on `.accounts` and `.usage`. Its functionality is absorbed into the `active::` parameter — the `USER@MACHINE` value is now passed directly as the `active::` value.
>
> **Migration:**
> - `assign::1 name::X for::USER@MACHINE` → `active::USER@MACHINE name::X`
> - `assign::1 name::X` (current machine) → `active::$USER@$HOSTNAME name::X`
>
