...
[feature/075](../../feature/075_account_tags.md): `tags` shows the account's comma-joined tag set; `role` remains a legacy toggle until per-account lazy migration erases the stored field.
...
cols::+tags                    -> show the account tag set
```
