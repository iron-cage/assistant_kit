...
- When `claude_paths` is available (normal runtime case), `account::save()` writes the new credentials directly to `{credential_store}/{account}.credentials.json` and updates companion files. `~/.claude/.credentials.json` is NOT written — the user's live session is preserved throughout the batch. (See BUG-165, BUG-221.)
...
| # | User Story | Persona |
|---|------------|---------|
| 1 | [Account Rotation](../user_story/001_account_rotation.md) | Transparent token refresh during account switch |
| 2 | [Multi-Account Quota Monitoring](../user_story/003_quota_monitoring.md) | Silent recovery keeps quota table populated |
