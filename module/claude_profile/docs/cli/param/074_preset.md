...
- **Constraints:** `kimi` (case-insensitive) is the only recognized value; any other non-empty value is a usage error
- **Purpose:** Collapse the fixed, provider-constant parts of adding a Kimi-compatible account (`backend::redirect`, Moonshot's base URL, `inference_provider::kimi`) into a single flag, while leaving account-specific values (`api_key::`, `redirect_model::`) always explicit.
...
**Behavior:** Explicit parameters always win over `preset::kimi`'s defaults — passing `base_url::` or `inference_provider::` directly stores exactly that value, never the preset's. The `base_url`/`inference_provider` defaults are gated on the *resolved* `backend` value (after `preset::kimi`'s own `backend` default is applied), not on `preset::kimi` being present in isolation — so `preset::kimi backend::anthropic` never force-fills redirect-only fields onto an anthropic-backend save; the account saves via the ordinary OAuth-capture path exactly as if `preset::` had not been given. `preset::` never defaults `api_key::` or `redirect_model::` — these remain always-explicit, per-account values (see [feature/073](../../feature/073_kimi_provider_preset.md)).
...
**Error cases:**
- `preset::bogus` → exit 1 with stderr naming the only recognized value: `kimi`

**Notes:**
- `preset::kimi` also drives `switch_account()`'s Kimi-tier `settings.json` env var writes indirectly, by filling `inference_provider::kimi` — see [feature/073](../../feature/073_kimi_provider_preset.md) and [schema/006](../../schema/006_settings_json.md).
- Scoped to exactly one recognized value today — `kimi` — by design; not a general multi-provider registry (see [feature/073](../../feature/073_kimi_provider_preset.md)'s Out of Scope).
...
| # | Feature | Role |
|---|---------|------|
| 1 | [Kimi Provider Preset](../../feature/073_kimi_provider_preset.md) | Full feature specification |
