...
|-------|-------------------|-----------------|
| `auto` (default) | `claude-haiku-4-5-20251001` (default); `claude-sonnet-5` when `son_idle=true` OR `son_available=true` | Haiku for general session activation; Sonnet whenever Sonnet window idle (opens all dimensions simultaneously) OR window active with > 20% remaining (avoid wasting quota). Fix(BUG-289, BUG-290, BUG-301) |
| `sonnet` | `claude-sonnet-5` | Always Sonnet, regardless of quota |
| `opus` | `claude-opus-4-8` | Always Opus, regardless of quota |
...
imodel::auto     → --model claude-haiku-4-5-20251001 (default)
                   --model claude-sonnet-5 (when son_idle=true OR son_available=true)
imodel::sonnet   → always --model claude-sonnet-5
imodel::opus     → always --model claude-opus-4-8
