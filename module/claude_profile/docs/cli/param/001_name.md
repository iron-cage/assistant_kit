...
- **Prefix resolution:** When the supplied value contains no `@`, an exact local-part match is tried first — if exactly one saved account's local part (the part before `@`) equals the value, that account is used even when other accounts share the same prefix (Fix(BUG-264); e.g. `i1` resolves to `i1@example.com` exactly even when `i11@example.com` also exists). Otherwise the value is matched as a prefix against all saved account names: exactly one match resolves to that account; zero matches exits 2 (`account 'X' not found`); two or more matches exits 1 (ambiguous prefix).
