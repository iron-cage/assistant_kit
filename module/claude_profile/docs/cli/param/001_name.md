...
- **Default:** **(required)** on `.account.use`, `.account.delete`; **inferred** on `.account.save` (reads per-machine `_active` marker from the credential store; exits 1 if absent); **optional** on `.accounts` (omit to list all), `.account.limits` (omit for active account), and `.account.relogin` (omit for active account)
