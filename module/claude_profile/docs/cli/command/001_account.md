...
5. Render in `format::` — rows always alphabetical by name (`account::list()` sorts at the store layer); no `.usage`-style sort/filter pipeline runs (see Notes: Inert parity params)
...
- **Inert parity params** — 20 registered params are accepted but never read by `.accounts`: `refresh::`, `touch::`, `imodel::`, `effort::`, `set_model::`, `sort::`, `desc::`, `prefer::`, `count::`, `offset::`, `only_active::`, `only_next::`, `min_5h::`, `min_7d::`, `only_valid::`, `exclude_exhausted::`, `get::`, `live::`, `interval::`, `jitter::`. Feature 037 unified the `.accounts`/`.usage` param set at the registration level (`src/registry.rs:96-118`), but `accounts_routine()` reads only `name::`, `format::`, `dry::`, `trace::`, `force::`, `assignee::`, `owner::`, `lock::`, `reserve::`, `tags::`, `cols::`, `no_color::` (12 of the 32). Passing an inert param is silently accepted — no error, no effect. `.accounts` is always local-only (never fetches quota); rows are always name-sorted. For live quota, sorting, filtering, `get::` extraction, or monitor mode, use `.usage`. Three further `.usage`-only params (`rotate::`, `who::`, `solo::`) are likewise registered on `.accounts` (accepted, unread) but sit outside the documented 32-param surface and are hidden from `.accounts.help`. Citation-backed analysis: Referenced Command Group below.
- `assignee::`, `owner::`, `lock::`, and `reserve::` are also available on `.usage` (same behavior, unified param set). `assign::`, `unclaim::`, `for::`, and `active::` are REMOVED_TOGGLE params — any invocation exits 1 with a migration message. `next::` is likewise REMOVED (→ `sort::`) and exits 1 identically on `.accounts` and `.usage` (audit 2026-08 — `.accounts` formerly accepted it silently). See [Feature 065](../../feature/065_assignee_param_redesign.md) and [Feature 064](../../feature/064_active_marker_and_owner_redesign.md).
...
- Claim-locked and reserved accounts are marked by default: text mode adds presence-driven `Lock:    yes` / `Reserve: yes` lines only for accounts carrying the flag (like `Tags:` — no `Lock: no`/`Reserve: no` noise); `format::table` appends ` 🔒` to the locked Account cell (no reserve glyph — soft flag); `format::json` includes `"claim_lock"` and `"reserve"` on every account object. See [Feature 070](../../feature/070_account_claim_and_reservation_control.md) AC-19.
- `no_color::1` strips the ✓/🔒 glyphs from text/table output (🔒 → `(locked)`, ✓ → `*`); `format::json` is exempt (machine format — replacement would rewrite user data inside JSON values). See [param/047](../param/047_no_color.md).
...
Evaluated against `.usage` under the strict [command_group](../command_group/readme.md) identity test (same dispatch function, same parameter set) — does not qualify in substance, though criterion (b) is technically satisfied at the literal-registration level: both `.accounts` and `.usage` are registered to the same function, `accounts_view_routine()` (`src/commands/accounts.rs:402`), a thin shim that branches on command name and delegates to `accounts_routine()` (`src/commands/accounts.rs:70`) for `.accounts` or `usage_routine()` (`src/usage/api.rs:78`) for `.usage` — the two branches share no logic beyond that check, and `accounts_routine()` itself has zero cross-calls with `usage_routine()`. The unified registration (Feature 037) is parity at the literal-registration level only, not implemented behavior — the command summary, param table, and the **Inert parity params** note above now record this directly: `accounts_routine()` registers `refresh::`, `touch::`, `imodel::`, `effort::`, and 16 further `.usage` pipeline params (`src/registry.rs:96-118`) but never reads any of them anywhere in its body — `grep -n "refresh\|touch\|fetch" src/commands/accounts.rs` returns no matches — and never calls `fetch_quota_for_list()` (`src/usage/fetch.rs:63`), `apply_refresh()` (`src/usage/refresh.rs:76`), or `apply_touch()` (`src/usage/touch.rs:133`), all of which are called only from `usage_routine()`. The `assignee::`/`owner::`/`lock::`/`reserve::` mutation sharing (line above) is a real shared-helper case, already covered by the `.accounts`/`.usage` rows in [`command_group/readme.md`](../command_group/readme.md) Evaluated, Not Qualifying — see that table for the full citation-backed analysis of both this claim and the mutation-dispatch one.
...
-- **Exit:** 0 (success — including the redirect skip: an omitted `backend::` on a stored-redirect target carrying no mutation writes nothing and reports `save skipped`; see the Redirect skip gate below) | 1 (usage: invalid name or no active account set; empty `inference_provider::` value; invalid `backend::` value; unrecognized `preset::` value (only `kimi`/`deepseek` are recognized); `backend::redirect` missing one of `base_url::`/`api_key::`/`redirect_model::`; any of those three present with `backend::anthropic` or omitted `backend::`; `role::` used (REMOVED — exit 1 with `tags::` migration message); invalid `tags::` item; `host::`/`tags::`/`inference_provider::` given with an omitted `backend::` on a stored-redirect target — an explicit `backend::` is required to disambiguate) | 2 (runtime: credentials unreadable)
...
clp .account.save name::deepseek preset::deepseek api_key::"$DEEPSEEK_API_KEY" redirect_model::deepseek-v4-pro
                                                        # preset::deepseek fills backend::/base_url::/inference_provider:: for DeepSeek
```
...
| `preset::` | `string` | *(omit)* | Named provider preset pre-filling `backend::`/`base_url::`/`inference_provider::` when omitted; `kimi` and `deepseek` recognized; see [param 074](../param/074_preset.md) |
...
**Preset resolution (before step 1):** `preset::` is resolved first, before any other parameter validation. An unrecognized `preset::` value (anything but `kimi` or `deepseek`) exits 1 immediately, naming both recognized values. Otherwise: `backend` defaults to `redirect` when `backend::` was omitted; then, using that resolved `backend` value, `base_url` and `inference_provider` default per the matched preset — each only when the corresponding parameter was omitted AND the resolved `backend` is `redirect`:

| `preset::` value | `base_url` default | `inference_provider` default |
|---|---|---|
| `kimi` | `https://api.moonshot.ai/anthropic` | `kimi` |
| `deepseek` | `https://api.deepseek.com/anthropic` | `deepseek` |

Explicit `backend::`/`base_url::`/`inference_provider::` values always take precedence over these defaults. See [feature/073](../../feature/073_kimi_provider_preset.md) (`kimi`) and [feature/078](../../feature/078_deepseek_provider_preset.md) (`deepseek`).

**Redirect skip gate (after preset resolution, before step 1):** when `backend::` was omitted (and `preset::` supplied none) AND the resolved target's stored `backend` is `redirect`, the call never reaches the algorithm below. A redirect account's credential is a static key, so the live-OAuth-session capture steps 2–4 perform has nothing to snapshot. What happens next depends on whether the call carries a mutation ([feature/071](../../feature/071_redirect_backend_accounts.md) AC-19; BUG-549, BUG-554):

- **Carries no mutation** — `name::`/`dry::`/`trace::` only, the watchdog's per-tick shape: exit 0 without touching either store file. stdout reports `'{name}' is a redirect account (static credentials) — nothing to snapshot; save skipped`; `trace::1` adds `skipped (reason: redirect backend — static credentials, nothing to snapshot)`.
- **Carries a mutation** — exit 1, never silently skipped. `base_url::`/`api_key::`/`redirect_model::` fall through to the redirect-only rejection and `role::` to its REMOVED message, each keeping its own precise wording; `host::`/`tags::`/`inference_provider::` have no such rejection of their own and are refused by the gate itself, naming the parameter and requiring an explicit `backend::`.

Explicit `backend::redirect …` re-saves and explicit `backend::anthropic` re-backends (AC-15) are deliberate acts and bypass this gate entirely.
...
clp .account.save name::deepseek preset::deepseek api_key::"$DEEPSEEK_API_KEY" redirect_model::deepseek-v4-pro
# saved redirect-backend account 'deepseek'   (backend='redirect', base_url='https://api.deepseek.com/anthropic', inference_provider='deepseek')

clp .account.save name::kimi preset::bogus api_key::"$KIMI_API_KEY" redirect_model::kimi-k3
# error: preset:: invalid value 'bogus' — valid values: kimi, deepseek
```
...
- **Metadata refresh:** Re-running `.account.save` for an existing *anthropic-backend* name refreshes the unified `{name}.json` and re-fetches endpoint 005 — this is the canonical way to refresh cached org identity without re-login. It does **not** apply to a redirect account: a bare re-save on a stored-redirect target is a no-op skip (see the Redirect skip gate above), and there is no cached org identity to refresh — a redirect account never calls endpoint 005. `{name}.json` is updated via read-merge (not full overwrite): the `oauthAccount` key is replaced but all other keys (e.g., `_renewal_at` set by `.account.renewal`) are preserved.
...
- **Redirect backend:** `backend` is fixed per save call — re-running `.account.save name::X` with a different *explicit* `backend::` value rewrites the account from scratch per that backend's own path (not a partial update). An *omitted* `backend::` behaves differently and is never an implicit re-backend: on a stored-redirect target it is skipped or rejected per the Redirect skip gate above, so the destructive rewrite requires the operator to name `backend::anthropic` deliberately. Pre-existing accounts saved before Feature 071 have no `backend` key and are treated as `anthropic`. See [Feature 071](../../feature/071_redirect_backend_accounts.md).
...
- **DeepSeek provider preset:** `preset::deepseek` pre-fills `backend::redirect`, `base_url::https://api.deepseek.com/anthropic`, and `inference_provider::deepseek` — same gating/precedence rules as `preset::kimi` above. Tagging an account `inference_provider::deepseek` (whether via the preset or explicitly) also drives `switch_account()`'s 6 additional DeepSeek-tier `settings.json` env vars on `.account.use` — see [Feature 078](../../feature/078_deepseek_provider_preset.md).
...
| 11 | [DeepSeek Provider Preset](../../feature/078_deepseek_provider_preset.md) | `preset::deepseek` convenience default-filling for `backend::`/`base_url::`/`inference_provider::` |
...
Atomically overwrites `~/.claude/.credentials.json` with the named account's credentials (write-then-rename), updates the active marker (`_active_{hostname}_{user}`), and best-effort patches `~/.claude.json["oauthAccount"]` from the saved snapshot — preserving all machine-global keys untouched. When `touch::1` (default), fetches quota for the target account and spawns an isolated subprocess to activate its idle 5h session window if `five_hour.resets_at` is absent. Guarded by G5 (ownership) and G9 (`claim_lock`) — both bypassable via `force::1`. When the target account is `backend: redirect`, this command additionally writes `settings.json`'s `env.ANTHROPIC_BASE_URL`/`env.ANTHROPIC_AUTH_TOKEN`/`env.ANTHROPIC_MODEL` (clearing those same keys when switching back to a `backend: anthropic` account instead), and skips the quota/touch step entirely — there is no Anthropic quota to fetch for a foreign backend (see [feature/071](../../feature/071_redirect_backend_accounts.md)). When the target account additionally carries `inference_provider: "kimi"`, 7 more Kimi-tier `env.*` variables are written alongside the three above (and all 10 are cleared together on switch-away) — see [feature/073](../../feature/073_kimi_provider_preset.md). When it instead carries `inference_provider: "deepseek"`, 6 more DeepSeek-tier `env.*` variables are written alongside the three base ones instead (and all 9 are cleared together on switch-away); switching directly between a kimi-tagged and a deepseek-tagged redirect account clears the outgoing provider's tier bundle in the same call that writes the incoming one's — see [feature/078](../../feature/078_deepseek_provider_preset.md).
...
5. `(when dry::0)` Write or clear `settings.json`'s `env.*` keys per target `backend`: `redirect` → write `env.ANTHROPIC_BASE_URL`/`env.ANTHROPIC_AUTH_TOKEN`/`env.ANTHROPIC_MODEL` from `base_url`/`accessToken`/`redirect_model`, plus — when `inference_provider == "kimi"` — 7 more Kimi-tier vars (`ANTHROPIC_DEFAULT_OPUS_MODEL`/`_SONNET_MODEL`/`_HAIKU_MODEL`/`_FABLE_MODEL`/`CLAUDE_CODE_SUBAGENT_MODEL` mirroring `redirect_model`, `CLAUDE_CODE_EFFORT_LEVEL` fixed `"max"`, `CLAUDE_CODE_AUTO_COMPACT_WINDOW` sized by whether `redirect_model` starts with `kimi-k3`) — or, when `inference_provider == "deepseek"` instead, 6 more DeepSeek-tier vars (`ANTHROPIC_DEFAULT_OPUS_MODEL`/`_SONNET_MODEL` mirroring `redirect_model`, `ANTHROPIC_DEFAULT_HAIKU_MODEL`/`CLAUDE_CODE_SUBAGENT_MODEL` fixed `"deepseek-v4-flash"`, `CLAUDE_CODE_EFFORT_LEVEL` fixed `"max"`, `CLAUDE_CODE_AUTO_COMPACT_WINDOW` fixed `"786432"`) — each provider's tier bundle also clears the other's if present, so switching directly between a kimi-tagged and a deepseek-tagged redirect account never leaves both bundles coexisting; `anthropic` → remove all keys (the three base plus whichever tier bundle, if any, was present — up to 10 for kimi or 9 for deepseek — removing `env` entirely if it becomes empty), preserving any other `env`/`settings.json` key unchanged. See [feature/073](../../feature/073_kimi_provider_preset.md), [feature/078](../../feature/078_deepseek_provider_preset.md), and [schema/006](../../schema/006_settings_json.md).
...
9. `(when ≥1 running claude process detected)` Print stderr advisory `warning: N running claude session(s) keep(s) using the previous account — restart to pick up '{name}'` — scan via `find_claude_processes()` (proc root overridable with `CLR_PROC_DIR`); exit code and stdout unchanged (Feature 004 AC-12/AC-13)
...
- **DeepSeek-tier env vars:** a `backend: redirect` target tagged `inference_provider: "deepseek"` (whether via explicit `inference_provider::deepseek` or via `preset::deepseek`, see [feature/078](../../feature/078_deepseek_provider_preset.md)) gets 6 additional `env.*` variables beyond the 3 above — independently shaped from Kimi's 7 (no Fable var; `ANTHROPIC_DEFAULT_HAIKU_MODEL`/`CLAUDE_CODE_SUBAGENT_MODEL` are a fixed `"deepseek-v4-flash"` rather than mirroring `redirect_model`; `CLAUDE_CODE_AUTO_COMPACT_WINDOW` is a flat `"786432"` with no model-name branching). Switching away from a deepseek target clears all 9, not just the 3 base vars. Switching directly between a kimi-tagged and a deepseek-tagged target clears the outgoing provider's tier bundle in the same call that writes the incoming provider's — neither bundle is ever left stale alongside the other.
- **Running-session advisory:** the switch affects future `claude` launches only — running sessions hold the previous token in memory and keep billing the previous account until restarted. When ≥1 `claude` process is detected after a successful switch, a `warning: … restart to pick up '{name}'` line goes to stderr (never stdout; exit stays 0). No advisory in `dry::1` mode or when the proc root is unreadable. See [feature/004](../../feature/004_account_use.md) AC-12/AC-13.
...
| 10 | [DeepSeek Provider Preset](../../feature/078_deepseek_provider_preset.md) | 6 additional DeepSeek-tier `env.*` vars written/cleared alongside the base 3, gated on `inference_provider == "deepseek"`; cross-provider clearing between the two tier bundles |
...
-- **Exit:** 0 (success: credentials refreshed and saved) | 1 (usage: invalid name value; G7 ownership violation unless `force::1`; target has `backend: redirect`) | 2 (runtime: name omitted and no active account; account not found; or Claude spawn failed) | 3 (timeout or login abandoned: claude exited without updating credentials)
...
3. Redirect-backend refusal — read the stored `backend` field; exit 1 when it is `redirect`. A redirect account's credential is a static API key, so there is no OAuth session to re-authenticate; rotate the key with `.account.save name::X backend::redirect api_key::<new-key>` instead. Evaluated before both the `dry::1` check and step 4 so a dry-run surfaces the refusal, and so the live session is never switched (and no browser login ever spent) on a target the command cannot serve (BUG-556). `force::1` does not bypass this — it governs the ownership gate only
4. Snapshot the current active account name (for restoration after login)
5. `switch_account(name)` — makes the named account active in `~/.claude/`
6. Spawn `claude` with inherited TTY (stdin/stdout/stderr connected — NOT isolated subprocess) — Claude detects empty or invalid credentials and opens the browser login page
7. Wait for `claude` to exit; if `~/.claude/.credentials.json` changed → `account::save(name)` propagates fresh credentials to credential store — always as `backend: anthropic`, which is why step 3 must refuse a redirect target rather than letting it reach here
8. `switch_account(original_active)` — restore the prior active account
