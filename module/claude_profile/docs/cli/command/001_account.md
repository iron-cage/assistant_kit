...
- Claim-locked accounts are marked by default: text mode adds a presence-driven `Lock:    yes` line only for locked accounts (like `Tags:` — no `Lock: no` noise); `format::table` appends ` 🔒` to the Account cell; `format::json` includes `"claim_lock"` and `"reserve"` on every account object. See [Feature 070](../../feature/070_account_claim_and_reservation_control.md) AC-19.
