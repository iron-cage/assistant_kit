...
- Claim-locked accounts are marked by default: text and `format::tsv` append ` 🔒` to the account-name cell (suffix — name-prefix matching stays intact); `no_color::1`/`format::plain` render it as `(locked)`; `format::json` includes `"claim_lock"` on every row. See [feature/070](../../feature/070_account_claim_and_reservation_control.md) AC-18.
