...
| `lock::` | `bool` | *(omit)* | `lock::1`/`lock::0`: set/clear `claim_lock` via `write_claim_lock()`; ungated write (no ownership check); `name::` required (comma-list supported), or batch-applies to the filtered set when absent. See [param/067](../param/067_lock.md) |
| `reserve::` | `bool` | *(omit)* | `reserve::1`/`reserve::0`: set/clear `reserve` via `write_reserve()`; ungated write (no ownership check); `name::` required (comma-list supported), or batch-applies to the filtered set when absent. See [param/068](../param/068_reserve.md) |
| `force::` | `bool` | `0` | Bypass G8 ownership gate for `owner::0` and `owner::USER@MACHINE`; bypass G9 claim-lock gate for `assignee::` target-side. No effect on `lock::`/`reserve::` writes themselves (ungated) |
| `rotate::` | `bool` | `0` | After quota fetch and table render, switch to the recommended account (active `sort::` strategy winner); Gate 8 foreign-owned exclusion applies inside `find_next_for_strategy()` (`force::1` bypasses); Gate 9 claim-lock exclusion also applies there but is unconditional — never bypassed by `force::1`; mutually exclusive with `live::1`; `dry::1` previews without switching |
...
4. `fetch_quota_for_list()`: call `GET /api/oauth/usage` per account; call `GET /api/oauth/account` in parallel thread. `(when backend == redirect)` skip both HTTP calls entirely — produce an `Err("redirect backend — no Anthropic quota")` placeholder row instead, checked before the non-owned gate. `(when solo::1)` only the current+owned account gets live HTTP fetch; all others get `approximate_quota()` (historical cached data via dedicated function — no direct cache file reads)
...
- Status emoji column (`●`): composite of 5h and 7d status — `🟢` = both available (`5h Left > 15%` and `7d Left > 3%`); `🟡` = h-exhausted (`5h Left ≤ 15%`, 7d available) or weekly-exhausted (`7d Left ≤ 3%`, any 5h — including both-exhausted; 7d is binding, per BUG-321 fix); `🔴` = error or cancelled subscription (`billing_type = "none"`). Per-column emoji also embedded in `5h Left` (🟢/🟡 at ≤15% threshold) and `7d Left` (🟢/🟡 at ≤3% threshold). No JSON equivalent.
...
- `5h Reset` shows an exact countdown (`in Xh Ym`, no `~`) from the server's `five_hour.resets_at`. When that field is `null` the account has no server-reported 5h window, and the cell shows either `—` (genuinely idle) or a `~`-prefixed **projected** countdown (`~in Xh Ym`) when a corroborated local touch record exists for the account — projected as `floor_to_10_minutes( last_touch_at ) + 5h`, since Anthropic's 5h windows start on a 10-minute boundary. A touch the cache refutes (quota fetched >300s later, still no window) is not corroborated and yields `—`. Same `~`-marks-an-estimate convention as `~Renews`. See [feature/024_session_touch.md AC-20, AC-21](../../feature/024_session_touch.md).
...
- `backend: redirect` accounts render identically to a failed quota fetch — `—` for all quota columns with reason `redirect backend — no Anthropic quota` in the last visible column — but no HTTP call is ever attempted for that row (`fetch_quota_for_list()`'s redirect-bypass gate, checked before the non-owned gate). Other `backend: anthropic` rows in the same listing are unaffected. See [feature/071_redirect_backend_accounts.md](../../feature/071_redirect_backend_accounts.md).
...
- Claim-locked accounts are marked by default: text and `format::tsv` append ` 🔒` to the account-name cell (suffix — name-prefix matching stays intact); `no_color::1`/`format::plain` render it as `(locked)`; `format::json` includes `"claim_lock"` and `"reserve"` on every row. See [feature/070](../../feature/070_account_claim_and_reservation_control.md) AC-18.
- `no_color::1` covers the `get::` single-value path (e.g. `get::status` emits `ok`, not 🟢) and maps the `●` status header to `status`; `format::tsv`/`format::json` are exempt (machine formats). See [param/047](../param/047_no_color.md).
