...
| `name::` | `string` | *(omit)* | Restrict mutation (`active::`, `owner::`) to the named account; comma-list `X,Y,Z` supported for `owner::` batch operations; when absent, `active::` unassigns marker, `owner::0` batch-clears all owned accounts |
| `dry::` | `bool` | `0` | Preview mutation result without writing to disk; G8 gate still runs on `owner::` mutations |
| `active::` | `string` (`USER@MACHINE`) | *(omit)* | When `name::` present: write per-machine marker `_active_{machine}_{user}` = `{name}`. When `name::` absent: clear marker for the given identity. Value sanitized per `active_marker_filename()` rules (Feature 064) |
...
2. `(when active:: or owner:: present)` Mutation dispatch: `(when active::USER@MACHINE + name::X)` write per-machine marker; `(when active::USER@MACHINE, no name::)` clear per-machine marker; `(when owner::0)` release ownership per-account with G8 gate (batch-clear when `name::` absent); `(when owner::USER@MACHINE)` set ownership per-account with G8 gate; comma-list `name::` supported for `owner::` operations; `(when dry::1)` print planned changes without writing
