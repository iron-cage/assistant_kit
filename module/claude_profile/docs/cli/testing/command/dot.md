...
Integration test planning for the `.` command. See [commands.md](../../commands.md#command--1-) for specification.
...
**Source:** [commands.md — .](../../commands.md#command--1-)
...
**Goal:** Confirm the `.` output includes all 6 visible commands.
**Setup:** No special state required. Default environment with `clp` binary on PATH.
**Command:** `clp .`
**Expected Output:** Output contains all 6 visible command names: `.account.list`, `.account.save`, `.account.switch`, `.account.delete`, `.token.status`, `.paths`.
**Verification:**
- Capture stdout
...
**Pass Criteria:** Exit 0; all 6 visible command names present in output.
**Source:** [commands.md — .](../../commands.md#command--1-)
...
**Goal:** Confirm the hidden commands `.` and `.help` are excluded from the command listing in the output.
**Setup:** No special state required. Default environment with `clp` binary on PATH.
**Command:** `clp .`
**Expected Output:** The "Commands:" section lists exactly 6 entries, none of which are `.` or `.help`.
**Verification:**
- Capture stdout
- Extract lines from the "Commands:" section (indented command entries)
- Assert no extracted line starts with `.help` as a command name
- Assert no extracted line matches a bare `.` as a standalone command entry (distinct from `.account.*`, `.token.*`, `.paths`)
- Assert exactly 6 command entries are listed
**Pass Criteria:** Exit 0; neither `.` nor `.help` appears as a listed command; exactly 6 entries.
**Source:** [commands.md — .](../../commands.md#command--1-)
