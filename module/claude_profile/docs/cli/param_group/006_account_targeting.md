...
**Purpose:** Provides account-level metadata (machine/user context, tag set, inference provider) that is stored in `{name}.json` at `.account.save` time and displayed via column projection in `.usage`/`.accounts`. The legacy `role::` label is removed — superseded by tags ([feature/075](../../feature/075_account_tags.md)).
...
| 2 | [`.accounts`](../command/001_account.md#command-3-accounts) | `host::` display toggle — opt-in boolean; `inference_provider` — default identity column, no dedicated toggle (opt-out via `cols::-inference_provider`); `tags::` — subset row filter |
| 3 | [`.account.tag`](../command/001_account.md#command-25-accounttag) | `tags::` replace mode — dedicated tag mutation command |
...
- [../param/082_tags.md](../param/082_tags.md) — `tags::` parameter specification
