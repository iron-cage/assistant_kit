...
**Measurable threshold:** `grep -rn 'billing_type == "none"' src/usage/*.rs` MUST return zero call sites that (a) intend to answer "no active subscription" and (b) omit an accompanying `result.is_err()` (or equivalent shared-predicate call) in the same guard expression. As of BUG-332's filing, this crate had one call site where the conjunction was correctly enforced by construction (see Enforcement below) and multiple call sites where it was not (see Non-Compliant Sites) — the 2026-07-08 fix closed the gap at all of the latter by routing them through a new shared predicate (see Enforcement Mechanism and Post-Fix Status below).

### Enforcement Mechanism — Data-Layer Override Plus Shared Predicate

A named function, `AccountQuota::is_no_subscription(&self) -> bool` (`src/usage/types.rs:226-229`), now encodes the conjunction directly — added by the BUG-332 fix (2026-07-08) and called at all 3 previously-independent render-layer sites (see Post-Fix Status under Non-Compliant Sites below). The conjunction is also independently enforced by a **data-layer override**, evaluated earlier, at the point `result` is finalized:
...
### Non-Compliant Sites (BUG-332's Concrete Evidence, As Filed — Pre-Fix)
...
| `src/usage/format.rs:499` | `status_emoji()` — gates `billing_type=="none"` to 🔴 before quota thresholds | No (by design — see Related Cases) | Distinct symptom (emoji color), same broken premise |
...
**Post-Fix Status (2026-07-08):** The first 3 rows above (`render.rs:108`, `render.rs:374`, `render_tsv.rs:72`) are no longer non-compliant — each now calls the shared `AccountQuota::is_no_subscription()` predicate (`types.rs:226-229`) instead of re-deriving `billing_type == "none"` independently, so all 3 now combine with `result` by construction. `mre_bug332_renews_shown_for_billing_none_with_ok_result` (`tests/usage/render_tests_a.rs`) confirms population-2 accounts now show a real renewal date instead of `"—"`. The remaining rows (`sort.rs:47`, `sort_next.rs:36`, `format.rs:273`, `format.rs:499`, `api.rs:206`) are unaffected — they were already correctly-scoped exceptions (see Related Cases), not part of this task's fix scope.

**Related Cases — same broken single-field premise, deliberately out of this invariant's BUG-332 grounding:** `sort.rs:47`, `sort_next.rs:36`, `format.rs:499`, and `api.rs:206` all re-derive `billing_type == "none"` to answer a *different* question than "no active subscription" — namely "is this account permanently dead for classification/rotation/filtering purposes," a question BUG-317 (see pitfall/001 Pitfall 4) intentionally answers using `billing_type` alone as the dead-account signal, because a dead account should be excluded from rotation/coloring even if its last fetch happened to return `Ok`. `api.rs:206` additionally references `aq.result` in the same guard (`aq.result.is_ok() && !aq.account.as_ref().is_some_and(|a| a.billing_type == "none")`), but disjunctively — it excludes a row when *either* the fetch failed *or* `billing_type=="none"`, not the BUG-332 conjunction ("no active subscription" requires *both* to hold together). These sites are **not** BUG-332's failure mode and are not required to add a `result.is_err()` conjunct — they are listed here only because they share the same literal condition string, which is exactly the detection signal this invariant defines (see Detection below).

**Post-BUG-557 Status (2026-08-29):** all four Related Cases (`sort.rs`, `sort_next.rs`, `format.rs:499`, `api.rs:206`) now call a *second* shared predicate, `AccountQuota::is_dead_account()`, instead of re-deriving the literal. It preserves their `result`-independence exactly — `billing_type == "none"` alone still suffices — and adds one disjunct: a no-subscription verdict persisted to the quota cache ([feature/033 AC-21](../feature/033_quota_cache.md)), which is the only signal available on the cache-rendered branches where `account` is `None` by construction. `api.rs:206` keeps its disjunctive combination with `result` at the call site rather than inside the predicate. The two predicates must stay distinct: merging the Related Cases into `is_no_subscription()` would impose BUG-332's `result.is_err()` conjunct on them and readmit the entire live-cancelled population BUG-317 exists to exclude. That collapse is now blocked mechanically by `bug_557_live_cancelled_account_with_ok_result_stays_red` (`tests/usage/format_tests.rs`), which this invariant previously could only assert in prose. Distinguishing "is this the BUG-332 predicate" from "is this a deliberately `result`-independent (or disjunctively `result`-combined) classification" requires checking what logical question the call site is actually answering, not just grep-matching the literal string.
...
| `src/usage/render.rs:108,374` | Fixed (BUG-332) — now calls the shared `AccountQuota::is_no_subscription()` predicate instead of evaluating `billing_type` alone (see Post-Fix Status above) |
| `src/usage/render_tsv.rs:72` | Fixed (BUG-332) — now calls the shared `AccountQuota::is_no_subscription()` predicate, same as `render.rs:108` (see Post-Fix Status above) |
| `src/usage/types.rs:180-209` | `AccountQuota` struct — `account` and `result` are independent fields with no compiler-enforced link between them |
...
| File | Relationship |
|------|--------------|
| BUG-236 | ✅ Fixed (TSK-242): narrowed the fetch-layer override at `fetch.rs:255` to require `billing_type=="none" && result.is_err()` — the correct instance of this invariant; not propagated to render-layer call sites until BUG-332's fix |
| BUG-332 | 🎯 Verified (lifecycle state per task file header) — HIGH-confidence investigation pipeline finding with zero surviving counterexamples per the task file's own `## History`: `render.rs:108,374` and `render_tsv.rs:72` never inherited BUG-236's conjunction — this invariant's direct motivating case. Fix applied 2026-07-08 (TSK-378): `AccountQuota::is_no_subscription()` shared predicate added and called at all 3 sites (see Post-Fix Status above); state remains 🎯 Verified pending independent validation, not yet promoted to Fixed |
