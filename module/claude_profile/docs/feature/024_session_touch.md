...
- **AC-20**: When an account has a corroborated touch record (AC-21) but the server still reports `five_hour.resets_at = None`, the `5h Reset` column renders a **projected** countdown — `~in Xh Ym` — instead of `—` or any opaque state literal. The projection is `floor_to_10_minutes( last_touch_at ) + 5h` (`format::projected_window_end_secs`): Anthropic's 5h windows begin on a 10-minute boundary, so flooring the touch instant reproduces the server's own `resets_at` to within ±60s once it propagates — verified against 16 of 19 live accounts, where the unfloored `last_touch_at + 5h` matched none. The leading `~` marks the value as locally derived, matching the `~Renews` convention; a `5h Reset` value without `~` always came from the server. The table cell and `get::5h_reset` share the projection call, so the two surfaces can never disagree. `AccountQuota` carries `touched_at_secs : Option<u64>` (the touch instant) rather than a bare `touched_recently : bool`, because a boolean cannot express *when* the window ends. Fix(BUG-551).
