...
| BUG-551 🟢 Fixed | `derive_touched_recently` parsed `last_touch_at` into Unix seconds, compared it against the grace window, then discarded it into a bare `touched_recently : bool` — so the render layer knew *that* a touch happened but not *when*, and substituted the literal `(touched)` into the `5h Reset` cell. Opaque to the operator (a state name where a duration belongs), and divergent across surfaces (`get::5h_reset`, JSON, and TSV never emitted it). Fix: carry the instant as `touched_at_secs : Option<u64>` and project the window end; see AC-20. The fix initially reached only the text table and `get::5h_reset`; TSV and JSON were closed by BUG-553 |
