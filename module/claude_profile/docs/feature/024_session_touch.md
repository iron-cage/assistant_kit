...
| [BUG-309 🟢 Fixed](../../../../../task/claude_profile/bug/309_apply_touch_missing_cache_write.md) | `apply_touch` re-fetch block only set `aq.result = Ok(new_data)` — missing `write_quota_cache()`, `aq.cached = false`, `aq.cache_age_secs = None`; `{name}.json` retained stale pre-touch quota; cache-fallback accounts kept `~` markers after successful live re-fetch; fix = AC-18 |
| [BUG-310 🟢 Fixed](../../../../../task/claude_profile/bug/310_rotation_touch_stale_live_credentials.md) | Cross-feature: `api.rs:824` copies pre-touch store credentials to live; `api.rs:838` `apply_touch` may refresh token to STORE only; live session retains stale pre-refresh credentials. Primary feature: [038_usage_strategy_rotate.md AC-11](038_usage_strategy_rotate.md) — fixed by `fs::copy` at `api.rs:847` (TSK-318) |
