...
| `task/claude_profile/bug/288_account_use_touch_not_confirmed_usage_double_subprocess.md` | BUG-288 🟢 Fixed (Fix A): `apply_post_switch_touch` (Feature 027 AC-21) now re-fetches quota post-subprocess and writes via `write_quota_cache` — AC-03 re-fetch requirement satisfied for all touch paths; `.usage touch` sees updated quota and skips the just-activated account |
