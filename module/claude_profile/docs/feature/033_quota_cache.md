...
- **In Scope**: Cache write after every successful `fetch_oauth_usage` call, proactive cache-first read when cache is ≤120 s old (skips live API call entirely — prevents burst-rate flooding), cache read when fetch returns any transient error, staleness indicator in display (`~` prefix on percentages, `(Nm ago)` age suffix), model override and touch state persistence in the same cache object, `{name}.json` read-merge-write (no new files).
