...
A file written after a *definitive* failure carries two further keys alongside the preserved snapshot (AC-21):

```json
{
  "fetched_at": "2026-08-26T17:55:00Z",
  "five_hour": { "utilization": 14.0, "resets_at": "2026-08-26T21:49:00Z" },
  "last_error": "no subscription",
  "last_error_at": "2026-08-29T15:48:00Z"
}
```

The pre-BUG-557 schema also wrote a `"status"` key. It was a hardcoded `"ok"` on the only path that could reach it, no reader ever deserialized it, and no failure path could contradict it — so it asserted health for every account that had ever succeeded once. It is no longer written; files that still carry it are unaffected, since nothing reads it either way.
...
- **AC-20**: Every quota display cell (`5h Left`, `5h Reset`, `7d Left`, `7d(Son)`, `7d Reset`, `7d Son Reset`) is produced by the single aq-aware helper `format.rs::quota_cells_for( aq, data, now_secs, style )`, which applies both account-dependent display rules in one place: the cache-staleness pass (a `~` prefix on every non-em-dash cell when `cached` is true, and `(stale)` in place of a countdown for a cached `resets_at` that has already elapsed — AC-03's ladder) and the touch projection ([024_session_touch.md AC-20](024_session_touch.md)). The text table, `format::tsv`, and `get::<field>` all take their cells from it and therefore cannot disagree; `style` (`PctStyle::Emoji` / `PctStyle::Bare`) carries the one legitimate difference between them, the `🟢`/`🟡` prefix on the two `Left` columns. `format::json` is deliberately outside this helper: it emits numbers, and discloses the same two conditions through the `cached` / `cache_age_secs` fields (AC-09) and `session_5h_reset_is_estimate` ([009_token_usage.md AC-05](009_token_usage.md)) rather than through display markers, so a `~` must never appear in JSON output. Fix for BUG-553.
- **AC-21**: A **definitive** fetch failure — `billing_type == "none"` together with a failed usage fetch, the Class-A override — is persisted by `write_quota_cache_error()` as the volatile keys `last_error` (the canonical `NO_SUBSCRIPTION_REASON` string) and `last_error_at`. The write is **merge-only**: it preserves `fetched_at`, every period value and the history ring verbatim, so the age a render surface prints keeps meaning "time since last success"; with no volatile candidate to merge into, nothing is written at all and the account keeps ranking infinitely stale for `stalest::K`. A subsequent successful fetch clears both keys, since `write_quota_cache` rebuilds its object from scratch. On the three cache-rendered branches (cache-first AC-13, G1-not-owned, `approximate_quota()`) the persisted reason is restored onto `AccountQuota.fallback_reason`, which `AccountQuota::is_no_subscription()` recognises as its second disjunct — so `status_emoji`, `status_group_of` and next-account eligibility all classify a cache-rendered dead account 🔴 rather than reading it as healthy from its last successful snapshot. Only *definitive* failures qualify: a transient 5xx, timeout or 429 must never reach this writer, and `is_no_subscription()` matches the reason string exactly rather than testing `fallback_reason` for presence, so BUG-335's transient cache-fallback rows keep their ordinary quota classification. This persists a fetch **outcome**, never an identity field — `billing_type` itself remains live-fetch-only per the Out of Scope note above. Fix for BUG-557.
...
| BUG-553 🟢 Fixed | Three of the four render surfaces rebuilt quota cells from local closures, so the staleness rules this feature specifies reached only the text table — `format::tsv` referenced `cached` nowhere in its `Ok` arm (rendering a cache-fallback row as live on the one surface with no `cached` column to disclose it) and `get::<field>` dropped the `~` on every percentage and the 7d reset. The responsibility table's own claim that TSV provided the "same `~` prefix surfacing as `render.rs`" was false as shipped. Fixed per AC-20: one aq-aware `quota_cells_for()` produces every quota cell for every display surface |
...
| `src/usage/render.rs` | Staleness display (text table) — quota cells come from `format.rs`'s `quota_cells_for()`; `(Nm ago)` age label and NAME-cell fallback-reason suffix (AC-14) are this surface's own |
| `src/usage/render_tsv.rs` | Staleness display (TSV format) — takes the same `quota_cells_for()` cells as the text table (`PctStyle::Bare` drops only the emoji), so the `~`/`(stale)` markers cannot diverge; NAME-cell fallback-reason suffix (AC-14) is a standalone parenthetical — this format has no age-suffix mechanism to combine it with |
...
| `src/usage/format.rs` | `quota_cells_for()` — the one aq-aware quota-cell producer for every display surface, applying the `~`/`(stale)` staleness pass and the touch projection (AC-20); `shorten_error()` — failure-reason shortening shared by all three render formats (AC-03/AC-14); `cache_age_label()` — age-suffix formatting (AC-03); `status_emoji()` — threshold-based status coloring, cache-blind by design |
