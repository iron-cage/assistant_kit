...
| BUG-335 🔴 Open | Cache-fallback `Ok(data)` render row never calls `shorten_error()` — the original fetch-failure reason (e.g. HTTP 429) is discarded once the fallback arm converts `Err` to `Ok(cached_data)`, so text/TSV/JSON render paths show only the `~` prefix and age suffix with zero trace of why the row is stale; fix = AC-14 |
...
| `src/usage/render_tsv.rs` | Staleness display (TSV format) — same `~`/age/fallback-reason surfacing as `render.rs`, TSV-encoded |
