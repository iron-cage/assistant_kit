...
| BUG-319 | BUG-319 ✅ Fixed (premise-incorrect — superseded by BUG-321): `status_emoji()` returned 🟡 for both-exhausted accounts — `else { "🟡" }` collapsed G2+G3+G4. Fix changed `(false,false)→🔴` but incorrectly treated both-exhausted as dead. MREs: `mre_bug319_both_exhausted_status_emoji_is_red`, `test_status_emoji_and_both_at_threshold_red` — assertions will flip 🔴→🟡 when BUG-321 is fixed. |
| BUG-321 | BUG-321 🔴 Unverified: Both-exhausted accounts (5h ≤ 15% AND 7d ≤ 5%) show 🔴 and sort with dead accounts. Fix: change `( false, false ) => StatusGroup::Red` to `( false, false ) => StatusGroup::WeeklyExhausted` in `status_group_of()`, change `( false, false ) => "🔴"` to `( false, false ) => "🟡"` in `status_emoji()`. No new variant or array resize needed — both-exhausted belongs in G3 (weekly-exhausted) because 7d is the binding constraint and both-exhausted recovers identically. |
