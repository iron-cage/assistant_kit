...
(Sub column hidden by default; show with `cols::+sub`. Four-group status partition: 🟢 Green → 🟡 h-exhausted → 🟡 weekly-exhausted → 🔴 Red. Footer `Current` line identifies the `✓` account with session model/effort from `settings.json` and valid/total count. `Next` line shows the recommended account with metric from `→ Next` column and `opus` because carol's 7d(Son)=12% < 15% — session model override to Opus would fire on switch. Both lines use `·` delimiter with column-aligned padding.)
...
- **AC-18**: Every table row has a composite status emoji in the `●` column (second column, after flag) using AND logic: `🟢` when `result` is `Ok` and subscription active and `5h Left > 15%` and `7d Left > 5%`, `🟡` when `result` is `Ok` and subscription active and exactly one dimension is exhausted (`5h Left ≤ 15%` XOR `7d Left ≤ 5%`), `🔴` when `result` is `Err` OR when `billing_type = "none"` (cancelled subscription — permanently unusable regardless of quota values; Fix BUG-317) OR when both dimensions are exhausted (`5h Left ≤ 15%` AND `7d Left ≤ 5%`; Fix BUG-319). `account = None` (API fetch failed) is NOT classified as cancelled — absent data is ambiguous. The emoji appears on every row including the synthetic current-session row.
...
| `task/claude_profile/bug/319_both_exhausted_status_emoji_returns_yellow.md` | BUG-319 ✅ Fixed: `status_emoji()` returned 🟡 for both-exhausted accounts (5h ≤ 15% AND 7d ≤ 5%) — `else { "🟡" }` collapsed G2+G3+G4. Fix: 3-arm match `(true,true)→🟢, (false,false)→🔴, _→🟡` in `format.rs`. MREs: `mre_bug319_both_exhausted_status_emoji_is_red`, `test_status_emoji_and_both_at_threshold_red` |
