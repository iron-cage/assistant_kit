...
6. Render results as a table using `data_fmt`:
   - **Default columns:** flag (`✓`/`*`/`@`/ blank, priority `✓` > `*` > `@` > blank), status (`🔴`/`🟡`/`🟢`, header `●`), Account, 5h Left, 5h Reset, 7d Left, 7d Reset, Expires, ~Renews, → Next
   - **Hidden-by-default columns:** Sub, 7d(Son), 7d Son Reset — available via `cols::+sub`, `cols::+7d_son`, `cols::+7d_son_reset` (per AC-22, BUG-334 — `7d(Son)` hidden by default since its `seven_day_sonnet` data source became permanently unavailable). The example outputs below include `7d(Son)` via `cols::+7d_son` for illustration, since several captions reference its value directly — omit that modifier to see the true default column set.
