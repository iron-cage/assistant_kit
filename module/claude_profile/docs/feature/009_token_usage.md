...
(Sub column hidden by default; show with `cols::+sub`. Four-group status partition: 🟢 Green → 🟡 h-exhausted → 🟡 weekly-exhausted → 🔴 Dead. Footer `Current` line identifies the `✓` account with session model/effort from `settings.json` and valid/total count. `Next` line shows the recommended account with metric from `→ Next` column — `opus/max` because carol's 7d(Son)=12% < 15% (Opus model-derived, effort always `"max"` for Opus — TSK-335). Both lines always show `model/effort` with `·` delimiter and column-aligned padding.)
