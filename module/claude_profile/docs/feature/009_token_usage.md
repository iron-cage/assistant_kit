...
6. Render results as a table using `data_fmt`:
   - **Default columns:** flag (`✓`/`*`/`@`/ blank, priority `✓` > `*` > `@` > blank), status (`🔴`/`🟡`/`🟢`/`⚪`, header `●`), Account, 5h Left, 5h Reset, 7d Left, 7d Reset, Expires, ~Renews, Owner, → Next — `Owner` shows the `owner` field from `{name}.json` (`USER@MACHINE` when owned, `—` when unowned); default-visible since Feature 037 (AC-19). See [037_accounts_usage_param_unification.md](037_accounts_usage_param_unification.md).
   - **Hidden-by-default columns:** Sub, 7d(Son), 7d Son Reset, Host, Role — available via `cols::+sub`, `cols::+7d_son`, `cols::+7d_son_reset`, `cols::+host`, `cols::+role` (per AC-22, BUG-334 — `7d(Son)` hidden by default since its `seven_day_sonnet` data source became permanently unavailable). The example outputs below include `7d(Son)` via `cols::+7d_son` for illustration, since several captions reference its value directly, but omit `Owner`, `Host`, and `Role` — omit `cols::+7d_son` and mentally add `Owner` after `~Renews` to see the true default column set.
   ...
   - **Composite status emoji column (`●`):** placed between the flag and Account columns; populated on every row; uses AND logic of 5h and 7d:
     - `⚪` — redirect-backend account (Feature 071): checked before the error tier; the row's `result` carries the canonical placeholder (`is_redirect_backend()`), which is not an error state — the account simply has no Anthropic quota semantics.
     ...
     - `🟡` — valid token, active subscription, at least one quota exhausted (`5h Left ≤ 15.0%` or `7d Left ≤ 3.0%` or both; `result` is `Ok`). Recoverable by waiting for quota resets — includes h-exhausted (G2), weekly-exhausted (G3), and both-exhausted (also G3; 7d is the binding constraint).
     - `🟢` — valid token, active subscription, both `5h Left > 15.0%` and `7d Left > 3.0%` (both quotas healthy; `result` is `Ok`)
     - No JSON equivalent — the status is a display-only column derived from existing fields
   - **Per-column emoji:** `5h Left` and `7d Left` column values embed an individual 🟢/🟡 emoji based on their own threshold: `5h Left` uses ≤15% (`🟢 86%` when > 15%, `🟡 12%` when ≤ 15%); `7d Left` uses ≤3% (`🟢 65%` when > 3%, `🟡 2%` when ≤ 3%). This provides drill-down visibility beyond the composite `●`.
   - `Expires`: "in Xh Ym" when `expires_in_secs > 0`; "EXPIRED" when `expires_in_secs == 0`; `static` for redirect-backend rows (Feature 071 — the account's API key has no OAuth expiry; the raw arithmetic would misreport the unset `expires_at_ms = 0` as EXPIRED)
   ...
   - `5h Reset` / `7d Reset`: countdown formatted via `format_duration_secs` (capped to 2 significant units); sourced from `five_hour.resets_at` / `seven_day.resets_at` (ISO-8601 UTC string → Unix seconds via `iso_to_unix_secs`). `5h Reset` additionally falls back to a **projected** countdown when `five_hour.resets_at` is absent but a corroborated local touch record exists — `~in Xh Ym`, derived as `floor_to_10_minutes( last_touch_at ) + 5h`; the `~` prefix marks it as locally derived, the same convention `~Renews` uses. `—` when neither source is available. See [024_session_touch.md AC-20, AC-21](024_session_touch.md).
...
7. Append footer when ≥2 accounts with valid quota data exist. Footer has two `·`-delimited, column-aligned lines: (1) `Current · <name> · <model>/<effort> · N/N` identifying the `✓` account, its session model and effort from `~/.claude/settings.json`, and the valid/total count; (2) `Next (<strategy>) · <name> · <model>/<effort> · <metric>` showing the recommendation for the active `sort::` strategy — effort is model-derived: `"max"` for `opus`, `"high"` for `sonnet`; always shown unconditionally (TSK-335 Fix H3). When no `✓` account is detected (credentials unreadable), fall back to `Valid: N / M   session: <model>  effort: <level>` on line 1. Omit footer when 0 or 1 valid account.
...
Current      · alice@example.com · sonnet/high · 3/5
Next (renew) · carol@example.com · opus/max    · in 2d 11h +7d
```

(Sub column hidden by default; show with `cols::+sub`. Four-group status partition: 🟢 Green → 🟡 h-exhausted → 🟡 weekly-exhausted → 🔴 Dead. Footer `Current` line identifies the `✓` account with session model/effort from `settings.json` and valid/total count. `Next` line shows the recommended account with metric from `→ Next` column — `opus/max` because carol's 7d(Son)=12% < 10% (Opus model-derived, effort always `"max"` for Opus — TSK-335). Both lines always show `model/effort` with `·` delimiter and column-aligned padding.)
...
Current      · alice@example.com · sonnet/high · 3/3
Next (renew) · carol@example.com · sonnet/high · in 5d 1h +7d
```

(`*` = active marker points here, but live credentials belong to `alice@example.com`. carol is the only eligible account. `sonnet` because carol's 7d(Son)=54% ≥ 10% — no override.)
...
Current          · (current session)   · sonnet/high · 2/3
Next (renew)     · alice@example.com   · sonnet/high · in 3h 47m $ren
```
...
- **AC-03**: Accounts with expired or missing tokens show `—` in quota columns and a shortened error reason in the final visible quota data column (`5h Left` through `7d Reset` range). Metadata columns `Expires`, `Sub`, and `~Renews` are populated from their respective non-quota sources (`expires_at_ms`, `OauthAccountData`) and are not overwritten by the error reason. The error label is context-aware: HTTP 429 + `billing_type == "none"` (confirmed cancelled subscription) → `"no subscription"`; HTTP 429 with active subscription or unknown billing state → `"rate limited (429)"`. (**BUG-220 ✅ Closed** 2026-05-30; **BUG-231 ✅ Fixed** 2026-06-03) Redirect-backend placeholder rows (`is_redirect_backend()`) are a carve-out: the last quota column carries the compact `(redirect)` marker rather than the full backend descriptor (the 40-char sentence stays correct for `trace::1` output but would widen the column for the whole table), and `~Renews` shows `—` (renewal is inapplicable for an account with no Anthropic billing org, not unknown). Non-redirect Err rows are unaffected. (**BUG-538 ✅ Fixed** 2026-08-19)
- **AC-04**: Table output is rendered by `data_fmt`.
- **AC-05**: `format::json` returns a valid JSON array with one object per account; each object includes `expires_in_secs`, `is_current` (bool), `is_active` (bool), `is_occupied_elsewhere` (bool), `is_owned` (bool — Feature 036), `claim_lock` (bool — Feature 070), `reserve` (bool — Feature 070), `billing_type` (string or `null`), `has_max` (bool or `null`), `renewal_secs` (u64 or `null`), `renewal_is_estimate` (bool or `null`), `next_event_type` (string or `null`), and `next_event_secs` (u64 or `null`); successful rows also include `session_5h_left_pct`, `weekly_7d_left_pct`, and `weekly_7d_sonnet_left_pct` (all remaining, not consumed), plus `session_5h_resets_in_secs` (u64 or `null`) paired with `session_5h_reset_is_estimate` (bool or `null`); `weekly_7d_sonnet_left_pct` is `null` when Sonnet quota data is absent from the API response; `billing_type`, `has_max`, `renewal_secs`, `renewal_is_estimate`, `next_event_type`, and `next_event_secs` are `null` when the account fetch failed.

  `session_5h_reset_is_estimate` follows the same convention as `renewal_is_estimate`: `true` when `session_5h_resets_in_secs` is a countdown projected locally from a corroborated touch record rather than reported by the API (AC-08; [024_session_touch.md AC-20](024_session_touch.md)), `false` when the API reported the reset, and `null` exactly when `session_5h_resets_in_secs` is `null`. JSON carries numbers, not display strings, so it cannot use the `~` prefix the text surfaces use to mark a locally-derived value — the companion boolean is how the same distinction is expressed here. A consumer that ignores the flag reads a projected countdown as if the API had reported it. (Fix(BUG-553))
- **AC-06**: Missing credential store exits 2 with an actionable error message.
- **AC-07**: The `Expires` column shows token TTL ("in Xh Ym") for valid tokens and "EXPIRED" for tokens whose `expiresAt` is in the past; sourced from the credential file without an API call. Redirect-backend rows show `static` instead (Feature 071 AC-17) — a static API key has no OAuth expiry to count down.
- **AC-08**: `5h Left` and `7d Left` show remaining quota percentage (100 − consumed); `7d(Son)` shows remaining Sonnet-only weekly quota (100 − consumed) or `—` when absent; `5h Reset` and `7d Reset` show independent reset countdowns as separate columns; all quota data sourced from `claude_quota::fetch_oauth_usage()` → `OauthUsageData`. When `five_hour.resets_at` is absent, `5h Reset` renders a `~`-prefixed projected countdown from a corroborated touch record, or `—` when none exists — never an opaque state literal (Fix(BUG-551); see [024_session_touch.md AC-20](024_session_touch.md)).

  Every quota cell — `5h Left`, `5h Reset`, `7d Left`, `7d(Son)`, `7d Reset`, `7d Son Reset` — is produced by one shared helper that takes the account, so the text table, `format::tsv`, and `get::<field>` cannot disagree about any of them. That covers the two rules whose value depends on the account rather than the API response: cache staleness (the `~` prefix, and `(stale)` for an already-elapsed cached reset — see [033_quota_cache.md](033_quota_cache.md)) and the touch projection above. `format::json` expresses both through dedicated fields (`cached`/`cache_age_secs`, `session_5h_reset_is_estimate` — AC-05) rather than display markers, keeping its numbers parseable. (Fix(BUG-553))
...
- **AC-18**: Every table row has a composite status emoji in the `●` column (second column, after flag) using AND logic: `🟢` when `result` is `Ok` and subscription active and `5h Left > 15%` and `7d Left > 3%`, `🟡` when `result` is `Ok` and subscription active and at least one quota dimension is exhausted (`5h Left ≤ 15%` or `7d Left ≤ 3%` or both; G2 h-exhausted and G3 weekly-exhausted both show 🟡 — G3 covers accounts where both quotas are exhausted), `⚪` when the row is a redirect-backend placeholder (`is_redirect_backend()`, checked before the error tier — Feature 071 AC-17), `🔴` when `result` is `Err` OR when `billing_type = "none"` (cancelled subscription — permanently unusable regardless of quota values; Fix BUG-317). Fix(BUG-321): the former code returned 🔴 for accounts with both `5h ≤ 15%` AND `7d ≤ 3%`, conflating recoverable quota exhaustion with unrecoverable dead accounts. Both-exhausted accounts are G3 weekly-exhausted and display 🟡. `account = None` (API fetch failed) is NOT classified as cancelled — absent data is ambiguous. The emoji appears on every row including the synthetic current-session row.
- **AC-19**: The exhaustion boundary for composite `●` is exclusive for `🟢` and inclusive for `🟡`: 5h dimension uses 15% (`5h Left = 15%` → `🟡`; `> 15%` needed for `🟢`), 7d dimension uses 3% (`7d Left = 3%` → `🟡`; `> 3%` needed for `🟢`).
...
- **AC-21**: `5h Left` and `7d Left` column values each embed a per-column emoji prefix using their respective thresholds: `5h Left` shows `🟢` when `> 15%`, `🟡` when `≤ 15%`; `7d Left` shows `🟢` when `> 3%`, `🟡` when `≤ 3%`. This provides individual-dimension visibility beyond the composite `●`.
- **AC-22**: `Sub` column is hidden by default; shown via `cols::+sub`. `7d Son Reset` column is hidden by default; shown via `cols::+7d_son_reset`. `7d(Son)` column is hidden by default; shown via `cols::+7d_son` — its underlying `seven_day_sonnet` data source has been permanently unavailable since Anthropic's 2026-06-25 API restructuring (BUG-334, fix applied 2026-07-08 — `ColsVisibility::default_set()` now sets `d7_son: false`, matching this criterion; state 🎯 Verified pending independent validation).
...
- **AC-31**: When `OauthAccountData.billing_type == "none"` (confirmed cancelled subscription via a successful account fetch), the account's per-fetch result is overridden to `Err("no subscription")` — the `GET /api/oauth/usage` HTTP response is discarded regardless of its status code. This makes the result semantically correct at the data layer and removes the need for context-aware display logic in `render.rs`. `~Renews` shows `"—"` for these accounts (AC-27). When `billing_type` is unknown (`OauthAccountData` fetch failed), the raw usage fetch result and standard error mapping apply. Trace behavior: when `trace::1`, the timestamped `result:` diagnostic line is emitted AFTER the Class A override, so the trace correctly reflects the final stored result (not the raw API response). Render-layer and TSV-renderer display logic (`render.rs:108,374`, `render_tsv.rs:72`) that independently re-derives `billing_type == "none"` without also checking whether `result` was `Ok` before this override applied MUST NOT assume "no active subscription" from `billing_type` alone — non-stripe (team/enterprise) accounts can carry `billing_type == "none"` with a genuinely successful usage fetch (`result = Ok(...)`), and this override only ever discards the result when the raw fetch also failed (`r.is_err()`). Any renderer needing this conclusion should read the already-overridden `result` field rather than re-checking `billing_type` in isolation (see [invariant/011](../invariant/011_shared_predicate_consistency.md)). (**BUG-233 ✅ Fixed** 2026-06-03; **BUG-234 ✅ Fixed** 2026-06-03 — trace ordering; BUG-332, fix applied 2026-07-08, state 🎯 Verified pending independent validation — see invariant/011 for the render-layer consistency requirement)
...
| `tests/usage/render_tests_b.rs` | FT-33: sessions table flags a marker naming a since-deleted account with `(stale)`, leaves a live account's marker unflagged (AC-33, BUG-347) |
