...
| BUG-319 | BUG-319 ✅ Fixed: `status_emoji()` returned 🟡 for both-exhausted accounts (5h ≤ 15% AND 7d ≤ 5%) — `else { "🟡" }` collapsed G2+G3+G4. Fix: 3-arm match `(true,true)→🟢, (false,false)→🔴, _→🟡` in `format.rs`. MREs: `mre_bug319_both_exhausted_status_emoji_is_red`, `test_status_emoji_and_both_at_threshold_red` |
