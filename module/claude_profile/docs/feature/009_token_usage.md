...
- **AC-03**: Accounts with expired or missing tokens show `—` in quota columns and a shortened error reason in the final visible quota data column (`5h Left` through `7d Reset` range). Metadata columns `Expires`, `Sub`, and `~Renews` are populated from their respective non-quota sources (`expires_at_ms`, `OauthAccountData`) and are not overwritten by the error reason. (**BUG-220 🔴 Open**: `render_text()` and `render_tsv()` currently overwrite `~Renews` with the error reason for 429-errored accounts — fix tracked in BUG-220.)
