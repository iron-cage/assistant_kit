...
- **AC-03**: `owner::VALUE name::X` sets the `owner` field to `VALUE` via `write_owner()` ([Feature 063](063_explicit_ownership_claim.md)). `unclaim::` is not registered on `.account.save` (exits 1 on unknown parameter). Ownership release is via `.accounts unclaim::1`. `.accounts assign::1` is marker-only — does NOT call `write_owner()`, does NOT touch the `owner` field in `{name}.json`.
...
- **AC-14**: `account_save_routine()` passes `owner: None` to `save()` — ownership-neutral, preserves existing `owner` field. Background refresh callers also pass `owner: None`. The `.accounts unclaim::1` path calls `write_owner(name, store, "")` to clear ownership. The `.accounts owner::VALUE name::X` path calls `write_owner(name, store, VALUE)` to set ownership ([Feature 063](063_explicit_ownership_claim.md)). The `.accounts assign::1` path does NOT call `write_owner()` and does NOT modify the `owner` field. Both `unclaim::1` and `owner::` share the G8 gate and `force::` bypass.
