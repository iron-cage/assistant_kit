...
| `src/commands/accounts.rs` | `accounts_routine()` assign path — writes `_active_{machine}_{user}` for any `USER@MACHINE` pair without credential rotation; assign usage block path — reads own `_active_*` marker to show current active account in live usage block (Feature 037: absorbed from deleted `account_assign.rs`) |
