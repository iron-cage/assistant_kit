...
| [BUG-297 🟢 Fixed (TSK-307)](../../../../../task/claude_profile/bug/297_touch_fires_after_refresh_none.md) | `apply_refresh` else-continue branch sets `aq.result = Err("refresh token expired")` before `continue;` — prevents redundant `apply_touch` subprocess for RT-expired accounts; fix = AC-30 |
| [BUG-298 🟢 Fixed](../../../../../task/claude_profile/bug/298_refresh_trace_reason_ok_for_cached_owned_account.md) | `apply_refresh` trace emits constant `reason: ok` for cached+owned+expired accounts — `fetch.rs` cache fallback converts Err→Ok destroying original error; fix: `else if aq.cached` branch added before `aq.result.err()` at trace reason computation in `refresh.rs` — emits `"cached-expired"` when token is locally expired, `"cached"` when token is still valid (rate-limited) |
| [BUG-303 🟢 Fixed (TSK-310)](../../../../../task/claude_profile/bug/303_refresh_fires_for_occupied_elsewhere_accounts.md) | `should_refresh()` G2 gate at `refresh_predicate.rs:32` checked `!aq.is_owned` only — no `is_occupied_elsewhere` guard; fix: `!aq.is_owned || aq.is_occupied_elsewhere` guard added; fix = AC-31 |
| [BUG-316 🟢 Fixed (TSK-321)](../../../../../task/claude_profile/bug/316_refresh_live_creds_stale_is_active_toctou.md) | `refresh_token_with_live_path` stale `is_active` guard — boolean computed once before `run_isolated`, reused in race-recovery block 35s later; concurrent `switch_account("B")` during subprocess window causes B's live credentials to be written into A's store slot; fix = AC-33 re-read at race-recovery site |
