...
| [BUG-298 🟢 Fixed](../../../../../task/claude_profile/bug/298_refresh_trace_reason_ok_for_cached_owned_account.md) | `apply_refresh` trace emits constant `reason: ok` for cached+owned+expired accounts — `fetch.rs` cache fallback converts Err→Ok destroying original error; fix: `else if aq.cached { "cached-expired" }` branch added before `aq.result.err()` at trace reason computation in `refresh.rs` |
