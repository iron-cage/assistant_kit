...
| `task/claude_profile/bug/303_refresh_fires_for_occupied_elsewhere_accounts.md` | BUG-303 (Unverified): `should_refresh()` G2 gate at `refresh_predicate.rs:32` checks `!aq.is_owned` only — no `is_occupied_elsewhere` guard; all four trigger paths reachable for owned+`@` accounts causing credential mutation. Fix: `if !aq.is_owned || aq.is_occupied_elsewhere { return false; }`. |
