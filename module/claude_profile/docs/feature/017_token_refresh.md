...
| `claude_profile_core/src/account/refresh.rs` | `refresh_account_token()` — `read credentials → run_isolated → write credentials → save` lifecycle; sole authorized caller of `run_isolated()` (invariant 008) |
