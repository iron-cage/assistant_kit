...
`claude_profile` must provide `account::auto_rotate()` as a one-call rotation primitive:

1. Call `account::list()` to enumerate all accounts.
2. Filter to inactive accounts (`is_active == false`).
3. Select the account with the highest `expires_at_ms`.
4. Call `switch_account()` on the selected account.
...
2. List candidates:       account::list() → Vec<Account>
3. Select best:           filter is_active==false, pick max expires_at_ms
4. Switch:                account::switch_account(name) → writes credentials + _active
5. Confirm (caller):      invoke claude_runner with minimal message to verify
```

### Acceptance Criteria

- **AC-01**: `auto_rotate()` returns the name of the inactive account with the highest `expires_at_ms`.
- **AC-02**: `auto_rotate()` returns `NotFound` when no inactive accounts exist.
- **AC-03**: After `auto_rotate()`, `~/.claude/.credentials.json` contains the selected account's credentials.
