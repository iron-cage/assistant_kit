...
| BUG-286 | BUG-286 ✅ Fixed: `override_session_model_to_opus` gate extended with `\|\| current == "claude-opus-4-8" \|\| current == "claude-opus-4-6"` — full-ID opus in settings.json is now re-normalized to shorthand `"opus"` (`claude_profile_core/src/account/session_settings.rs:50`); reproducer `bug_reproducer(BUG-286)` in `claude_profile_core/tests/account_session_settings_test.rs` |
