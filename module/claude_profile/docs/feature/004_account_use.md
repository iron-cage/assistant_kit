...
| bug | `task/claude_profile/bug/254_switch_account_skips_email_patch_when_metadata_file_absent.md` | BUG-254: `switch_account()` skips emailAddress patch when `{name}.json` absent — stale emailAddress causes `save()` to cross-contaminate credential files |
