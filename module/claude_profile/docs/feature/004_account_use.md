...
6. Best-effort restore model preference: read `model` from `{credential_store}/{name}.settings.json`; if present, set `model` in `~/.claude/settings.json` to that value (creating or patching the file); if `{name}.settings.json` is absent or lacks a `model` field, delete the `model` key from `~/.claude/settings.json` if it exists (clearing stale model from prior account). All other keys in `~/.claude/settings.json` are preserved untouched. Missing or malformed `settings.json` files at either location are silently skipped.

**BUG-219 ✅ Fixed (TSK-221):** `organizationName` and `organizationUuid` inside `oauthAccount` are now overridden from `{name}.roles.json` after the BUG-217 `emailAddress` insert. Best-effort: silently skipped if `roles.json` is absent or malformed. `displayName` and `accountUuid` are left as-is (not in `roles.json`; low display impact).
...
**Model preference restore (BUG-222 fix):** `switch_account()` reads `model` from `{credential_store}/{name}.settings.json` and updates `~/.claude/settings.json` accordingly. When the snapshot file is present with a `model` field, that value is written into `~/.claude/settings.json`, restoring the per-account preference. When the snapshot is absent or lacks `model`, the `model` key is removed from `~/.claude/settings.json` to prevent the prior account's model from persisting. All other keys in `~/.claude/settings.json` are preserved.
...
- **AC-07**: `.account.use name::alice@acme.com` patches `~/.claude.json oauthAccount.emailAddress` to `'alice@acme.com'` regardless of what the `alice@acme.com.claude.json` snapshot contains — the account name always wins over stored snapshot data. Additionally, `oauthAccount.organizationName` and `oauthAccount.organizationUuid` are overridden from `alice@acme.com.roles.json` when present and non-empty (BUG-219 fix — prevents stale cross-account org identity from propagating to `~/.claude.json`).
- **AC-08**: `.account.use name::alice@acme.com` when `{credential_store}/alice@acme.com.settings.json` contains `{"model": "sonnet"}` writes `"model": "sonnet"` into `~/.claude/settings.json` while preserving all other keys; when `alice@acme.com.settings.json` is absent or lacks `model`, removes the `model` key from `~/.claude/settings.json` (clearing stale model preference from prior account).
...
| bug | `task/claude_profile/bug/219_switch_account_stale_oauthaccount_org_fields.md` | BUG-219 ✅ Fixed by TSK-221: `switch_account()` now reads `{name}.roles.json` and overrides `organizationName` + `organizationUuid` after the BUG-217 `emailAddress` insert |
| bug | `task/claude_profile/bug/222_switch_account_model_preference_not_restored.md` | BUG-222 ✅ Fixed (TSK-234): `switch_account()` now reads `{name}.settings.json` and restores/clears `model` in `~/.claude/settings.json` (step 6) |
