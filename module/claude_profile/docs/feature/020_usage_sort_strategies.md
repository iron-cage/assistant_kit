...
- **AC-10**: `sort::` and `desc::` work correctly with `live::1` — sort order is stable within each refresh cycle. Status group order is fixed across cycles. <!-- BUG-330 ../../../task/claude_profile/bug/330_ordinary_invocation_row_order_stability_undocumented.md — this guarantee is scoped only to live::1; no equivalent guarantee is documented for ordinary invocations -->
