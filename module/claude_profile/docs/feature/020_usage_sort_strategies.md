...
**Algorithm:**
1. Sort all accounts by `renewal_secs(aq.renewal_at, aq.org_created_at, now)` ascending. **Known Limitation (BUG-341):** the current implementation instead reads the estimate leg via the stale path `aq.account.as_ref().map(|a| a.org_created_at.as_str())` — for accounts refreshed from cache (`account` field `None`), this silently falls back to `u64::MAX` instead of the intended top-level `aq.org_created_at` estimate, in place of the `renewal_secs(aq.renewal_at, aq.org_created_at, now)` call shown here. Not yet fixed as of this writing.
2. Accounts without subscription data (no `renewal_at` and no `org_created_at` billing day) score `u64::MAX` and appear last — cache-refreshed accounts with real `org_created_at` data are currently misclassified into this group too (BUG-341).
...
**Known Limitation (BUG-341):** the `<metric>` field is computed by `strategy_metric()` (`sort_next.rs`), which shares the same stale `aq.account.as_ref().map(|a| a.org_created_at.as_str())` read as the sort keys above — not the top-level `aq.org_created_at` field the table's own `→ Next` column reads. For accounts refreshed from cache, this footer metric can therefore show `—` (or a stale/absent renewal event) even when the table's `→ Next` column for the same row correctly shows an estimated renewal — a visible divergence between the two, both nominally documented as using "the same format." Not yet fixed as of this writing.

### Acceptance Criteria

- **AC-01**: `sort::renew` (default) sorts rows by `min(7d Reset, ~Renews)` countdown ascending within each status group; tiebreak is `prefer_weekly` ascending; final tiebreak is alphabetical name. `sort::name` sorts alphabetically. When `sort::` is omitted, `renew` is used. **Known Limitation (BUG-341):** for accounts refreshed from cache (no live `account` object attached), the `~Renews` leg of `min(7d Reset, ~Renews)` incorrectly evaluates to absent (`u64::MAX`) instead of the real `org_created_at`-estimated renewal, because the current code reads via a stale gated path rather than the top-level `aq.org_created_at` field added by BUG-327 — see Strategy 2/3 Algorithm above. Not yet fixed as of this writing. See BUG-341 in the Bugs section below.
- **AC-02**: `sort::renews` sorts by subscription renewal timer ascending; accounts without subscription data are placed last; tiebreak is alphabetical name. **Known Limitation (BUG-341):** same estimate-leg defect as AC-01 — see above. Not yet fixed as of this writing.
...
- **AC-12**: Four-group status partition is applied universally before any sort strategy. Accounts are partitioned into: 🟢 Green (`5h Left > 15%` and `7d Left > 3%`), 🟡 h-exhausted (`5h Left ≤ 15%` and `7d Left > 3%`), 🟡 weekly-exhausted (`7d Left ≤ 3%` — any 5h, including both-exhausted), 🔴 Dead (error or cancelled `billing_type="none"`). Sort strategy applies within each status group. See [dictionary](../cli/002_dictionary.md#status-groups).
...
| BUG-324 | BUG-324 ✅ Fixed: `find_first_eligible()` eligibility gate used `prefer_weekly(aq, prefer) > 5.0` — green accounts with `7d(Son) ≤ 3%` blocked from rotation under `prefer::any`. Fix: `sort_next.rs` changed to `seven_day_left(aq) > WEEKLY_EXHAUSTION_THRESHOLD`. Same class as BUG-299. |
| BUG-330 | <!-- BUG-330 — ordinary-invocation row-order/is_current stability undocumented --> BUG-330 ✅ Fixed: AC-10's `live::1`-only stability scope now documented explicitly (Known Limitation above); ordinary invocations have no cross-invocation row-order / `is_current`-placement guarantee — documentation/specification gap closed, no code change needed |
| BUG-341 | BUG-341 🎯 Open (unfixed): `sort::renew`/`sort::renews` `sub_renewal_secs` estimate leg (`sort.rs:138-152`, `sort.rs:179-187`) and the footer `Next` metric (`sort_next.rs:127-131`, `sort_next.rs:146-150`) read the stale gated path `aq.account.as_ref().map(\|a\| a.org_created_at.as_str())` instead of the top-level `aq.org_created_at` field added by BUG-327 — cache-refreshed accounts silently score `u64::MAX` instead of their real estimated renewal, misordering rows and hiding/diverging the footer metric. See AC-01, AC-02, Strategy 2/3 Algorithm, and Recommendation (footer) above. |
