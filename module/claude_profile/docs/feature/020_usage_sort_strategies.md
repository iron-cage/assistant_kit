...
**Eligibility:** non-current, non-active, non-occupied, not h-exhausted (`5h Left > 15%`), not weekly-exhausted (`seven_day_left > WEEKLY_EXHAUSTION_THRESHOLD`), no recorded no-subscription verdict (live or cached — see [033_quota_cache.md](033_quota_cache.md) AC-21), valid quota data, `expires_in_secs > 0`. When no eligible account exists, the footer recommendation line is omitted.

**Headroom preference:** eligibility admits any account above the 3% exhaustion floor, but that floor answers "is this account spent?", not "is it worth switching to?". Selection therefore runs in two passes over the same sort order: pass 1 admits only accounts with `seven_day_left > ROTATION_HEADROOM_THRESHOLD` (15%); pass 2 repeats at the bare `WEEKLY_EXHAUSTION_THRESHOLD` floor and runs **only if pass 1 found nothing**. Headroom filters the candidate set, never reorders it — the active strategy's ordering is preserved exactly within each pass. Applies to every strategy. The fallback pass is what keeps a fully-depleted fleet rotating: a single raised gate would make `rotate::1` report `"no eligible account to rotate to"` ([038 AC-03](038_usage_strategy_rotate.md)) whenever every account sat below 15%.
...
| BUG-341 | BUG-341 ✅ Fixed: `sort::renew`/`sort::renews` `sub_renewal_secs` estimate leg (`sort.rs`) and the footer `Next` metric (`sort_next.rs`) read the stale gated path `aq.account.as_ref().map(\|a\| a.org_created_at.as_str())` instead of the top-level `aq.org_created_at` field added by BUG-327/TSK-368 — cache-refreshed accounts silently scored `u64::MAX` instead of their real estimated renewal, misordering rows and hiding/diverging the footer metric. Fix: both call sites now read `aq.org_created_at.as_deref()` directly via `renewal_secs(aq.renewal_at.as_deref(), aq.org_created_at.as_deref(), now_secs)`, matching `render.rs`'s existing call sites. |
