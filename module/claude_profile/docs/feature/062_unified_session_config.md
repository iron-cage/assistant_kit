...
Previously, the recommended session model was computed in two places using the same 10% Sonnet threshold:
...
1. Calls `apply_model_override(winner_data, paths)` — bidirectional model AND effort correction: writes `"opus"` + `effortLevel: "max"` when Sonnet left < 10% (Fix BUG-322, values updated TSK-335); writes `"sonnet"` + `effortLevel: "high"` when Sonnet left >= 10% or tier absent (Fix BUG-311, values updated TSK-335). Effort is written unconditionally (TSK-335 — Fix H2) — the BUG-312 init guard is now effectively unreachable but retained as safety fallback (writes `"high"` if absent).
...
- **AC-05**: After `rotate::1` switch succeeds and winner `result = Ok(data)`, `apply_model_override(data, paths)` is called for the winner. When winner's Sonnet left < 10%, settings.json `model` becomes `"opus"`.
...
- **AC-09**: When `apply_model_override()` is called and `seven_day_sonnet` left < 10% (Opus branch), `set_session_effort(paths, "max")` is called unconditionally regardless of `overrode`. (Fix BUG-322; value updated TSK-335: `"high"` → `"max"`)
- **AC-10**: When `apply_model_override()` is called and `seven_day_sonnet` left ≥ 10% or tier absent (Sonnet branch), `set_session_effort(paths, "high")` is called unconditionally regardless of `overrode`. (Fix BUG-322; value updated TSK-335: `"low"` → `"high"`)
