...
| File | Relationship |
|------|--------------|
| `claude_profile_core/src/account/` | `Account` struct org fields; `save()` writes org identity to `{name}.json` (feature-gated); `delete()` removes `{name}.json` best-effort; `list()` reads org identity from `{name}.json` |
