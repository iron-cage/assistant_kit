...
| [075_account_tags.md](075_account_tags.md) | Supersedes the `role` label with the tag set — `role::` param REMOVED, lazy field migration; `host::` unaffected |
...
| File | Relationship |
|------|--------------|
| `claude_profile_core/src/account/store.rs` | `save()` — `{name}.json` host/role write; host/role capture |
