...
- **Responsibility**: Documents the `rotate::1` parameter on `.usage`: eligibility filtering, ownership gate, strategy selection via the active `next::` algorithm, dry-run preview, and touch reuse from the already-fetched `AccountQuota`.
- **In Scope**: `rotate::` boolean param, G5 ownership gate on rotate path, strategy selection via `find_next_for_strategy()` (same `next::` that drives `→`), post-switch touch from pre-fetched quota, `dry::1` preview, `force::1` bypass, mutual exclusion with `live::1`, no-eligible-account error.
- **Out of Scope**: The recommendation display (→ marker, footer) — that is Feature 023. Sort order — that is Feature 020. The deprecated `auto_rotate()` API and `.account.rotate` command — see [008_auto_rotate.md](008_auto_rotate.md) (deprecated).
...
The former `.account.rotate` used `max_by_key(expires_at_ms)` — the account with the longest-lived OAuth token. `.usage rotate::1` defaults to `next::renew` — the account whose quota renewal event fires soonest. This is intentional: the new behavior is operationally superior for quota-management automation. To approximate the old behavior, use `clp .usage rotate::1 sort::expires next::endurance`.

### Acceptance Criteria

- **AC-01**: `rotate::1` switches to the account selected by the active `next::` strategy (the `→` winner). The table shows `→` on that account; the output ends with `switched to '{name}'`.
- **AC-02**: `rotate::1 dry::1` previews the target account with `[dry-run] would switch to '{name}'`; no credentials are written; exit 0.
- **AC-03**: When no eligible candidate exists for the active `next::` strategy (all accounts are current, active, occupied, h-exhausted, or non-owned when `force::0`), `rotate::1` exits 1 with `"no eligible account to rotate to"`. The table is still rendered.
...
- **AC-07**: `rotate::1 next::endurance` switches to the endurance strategy winner.
- **AC-08**: `rotate::1 next::drain` switches to the drain strategy winner.
- **AC-09**: `rotate::1 format::json` still executes the switch; JSON output is unchanged (no `"switched_to"` field added to JSON).
- **AC-10**: Post-switch touch is applied using the winner's `AccountQuota` already in memory from the main fetch — no additional `GET /api/oauth/usage` call for the winner account.
- **AC-11**: Exit codes: 0 = switch succeeded (or dry-run); 1 = usage error (no eligible account, mutual exclusion, ownership violation); 2 = runtime error (credential store unreadable, switch I/O failure).

### Migration from `.account.rotate`

| Old | New | Notes |
|-----|-----|-------|
| `clp .account.rotate` | `clp .usage rotate::1` | Default `next::renew` (soonest renewal). Former default was `max_by_key(expires_at_ms)`. |
...
| *(no equivalent)* | `clp .usage rotate::1 next::endurance` | New: rotate to account with most 5h session quota. |
| *(no equivalent)* | `clp .usage rotate::1 next::drain` | New: rotate to account closest to quota exhaustion. |
...
| [cli/param/032_next.md](../cli/param/032_next.md) | `next::` controls which strategy selects the rotation target |
...
| File | Relationship |
|------|--------------|
| [023_next_account_strategies.md](023_next_account_strategies.md) | Strategy selection algorithms (`find_next_for_strategy`) reused for rotation target |
