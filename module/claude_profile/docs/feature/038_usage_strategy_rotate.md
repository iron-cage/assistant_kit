...
- **AC-01**: `rotate::1` switches to the account selected by the active `sort::` strategy (the recommended account). The footer shows the recommended account; the output ends with `switched to '{name}'`. Selection applies the headroom preference of [020](020_usage_sort_strategies.md): among accounts the strategy would otherwise admit, one holding `7d Left > ROTATION_HEADROOM_THRESHOLD` always wins over one merely clearing the 3% exhaustion floor, regardless of how much sooner the latter's quota resets. Rotating onto a nearly-spent account is worse than not rotating: the caller's own anti-flap cooldown then blocks the corrective rotation for its full window. Fix for BUG-558.
- **AC-02**: `rotate::1 dry::1` previews the target account with `[dry-run] would switch to '{name}'`; no credentials are written; exit 0.
- **AC-03**: When no eligible candidate exists for the active `sort::` strategy (all accounts are current, active, occupied, h-exhausted, recorded as having no subscription, or non-owned when `force::0`), `rotate::1` exits 1 with `"no eligible account to rotate to"`. The table is still rendered. AC-01's headroom preference never contributes to this outcome — it is a preference with a fallback pass at the bare exhaustion floor, so a fleet whose every account sits below `ROTATION_HEADROOM_THRESHOLD` still rotates to its best remaining option rather than failing here.
