...
- **In Scope**: Quota fetch for the target account using its saved credential file; `resolve_model()` + `resolve_effort()` reuse from `usage.rs`; `run_isolated()` subprocess spawn (always fires when quota fetch succeeds — BUG-285 fix); `touch::` (default `1`), `refresh::` (default `1`), `imodel::`, `effort::`, `trace::` (default `0`) parameter registration on `.account.use`; graceful skip when quota fetch fails; OAuth token refresh when locally expired and `refresh::1`; `[trace] account.use` lines covering credential read, quota fetch, model/effort resolution, subprocess dispatch, and refresh attempt.
- **Out of Scope**: `.usage` subprocess control (→ 026_subprocess_model_effort.md); credential rotation mechanics (→ 004_account_use.md).

### Design

After `switch_account()` succeeds, `.account.use` fetches quota data for the target account and spawns the subprocess to activate its 5h session window whenever the fetch succeeds.
...
3. Subprocess dispatch: `pre_switch_touch_ctx()` returns `NeedTouch` for every successful quota fetch. `resets_at` state does not gate subprocess dispatch — it reflects server-side Anthropic quota state (any session, any machine) and cannot serve as a local-process identity oracle. (BUG-285 fix: removed wrong oracle that caused false `AlreadyActive` classification.)
...
6. If quota fetch succeeded (step 2): call `resolve_model(quota, imodel_param)` → `IsolatedModel`; call `resolve_effort(&model, effort_param)` → `Option<&str>`; spawn `run_isolated()` with `["--print", "."]` plus optional `--model` and `--effort` flags.
...
[trace] account.use  {name}  model override: sonnet→opus (7d(Son) left=5%)           ← fetch OK + 7d(Son) < 20% + snapshot was Sonnet (any form) or full-ID opus; omitted otherwise
[trace] account.use  {name}  model: {model}  effort: {effort}                        ← fetch OK path only
[trace] account.use  {name}  subprocess: spawned                                      ← fetch OK path only; no "already active" skip (BUG-285 fix)
```
...
- **AC-01**: `clp .account.use name::alice@home.com` (default `touch::1`) fetches quota for the target account, switches credentials, and spawns `run_isolated` when the quota fetch succeeds (regardless of `five_hour.resets_at` state — BUG-285 fix).
...
- **AC-03**: `clp .account.use` against an account with `resets_at.is_some()` spawns the subprocess and exits 0 — `resets_at` reflects server-side quota state and does not gate subprocess dispatch. (BUG-285 fix: previous behaviour wrongly skipped subprocess for these accounts.)
...
- **AC-12**: No idle-check trace line is emitted — `pre_switch_touch_ctx()` does not classify accounts as idle/active after BUG-285 fix. The trace sequence jumps from `quota fetch: OK` directly to the model-override line (when applicable) then `model:` then `subprocess: spawned`.
- **AC-13**: When `trace::1` and `touch::1` and quota fetch succeeded: emits `[trace] account.use  {name}  model: {model}  effort: {effort}` (using the resolved model and effort strings) — appears before `subprocess: spawned`. Omitted only when fetch failed.
- **AC-14**: When `trace::1` and `touch::1`: emits `[trace] account.use  {name}  subprocess: spawned` when quota fetch succeeded, or `subprocess: skipped (reason: fetch failed)` when fetch failed. No `subprocess: skipped (reason: already active)` line exists after BUG-285 fix.
...
- **AC-18**: When quota fetch succeeded (step 2) and `seven_day_sonnet` remaining < 20% and the just-restored session model contains `"sonnet"` (matches both shorthand `"sonnet"` and full ID `"claude-sonnet-4-6"`) or is absent or equals `"claude-opus-4-6"` (full-ID opus normalized to `"opus"` shorthand): overwrites `~/.claude/settings.json` model with `"opus"` before the subprocess step. (Fix for BUG-225; alias fix for BUG-257; full-ID normalization fix for BUG-286.)
...
| `task/claude_profile/bug/286_switch_account_model_id_not_normalized_to_shorthand.md` | BUG-286 🔴 Open: `override_session_model_to_opus` gate `contains("sonnet") \|\| is_empty()` does not match `"claude-opus-4-6"` — full-ID opus not normalized to `"opus"` shorthand when 7d(Son) < 20% |
