...
2026-06-25 · 16:40:04 · account.use  {name}  model override: sonnet→opus (7d(Son) left=5%)           ← fetch OK + 7d(Son) < 15% + snapshot was Sonnet (any form) or full-ID opus; omitted otherwise
...
| BUG-286 | BUG-286 🟢 Fixed (TSK-261): `override_session_model_to_opus` gate extended to match `"claude-opus-4-6"` (full-ID opus) in addition to Sonnet forms; normalized to `"opus"` shorthand when 7d(Son) < 15% — see AC-18 |
