...
2. If `dry::1`: print `[dry-run] would switch to '{name}'` and return immediately (no files changed, no quota fetch, no subprocess).
3. Fetch quota for the target account from `{credential_store}/{name}.credentials.json` — one HTTP call to `/api/oauth/usage`. If fetch fails (network error, expired token), record failure and continue to step 4.
...
4b. If quota fetch succeeded (step 3): call `apply_model_override()` which is now bidirectional (Fix BUG-311). When `seven_day_sonnet` is `Some` and remaining < 10%: overwrite `~/.claude/settings.json` model with `"opus"` (Fix BUG-225; alias fix BUG-257; normalization fix BUG-286). When `seven_day_sonnet` is `Some` and remaining >= 10%: overwrite model with `"sonnet"` if currently in any `"opus"` form (Fix BUG-311). When `seven_day_sonnet` is absent (`None`): overwrite model with `"sonnet"` conservatively — absent tier is treated as unknown, not exhausted (Fix BUG-300; conservative sonnet restore extended by Fix BUG-311). When `touch_ctx` is absent (fetch failed — see BUG-226 limitation), this step is skipped and the snapshot model is installed as-is.
5. If quota fetch succeeded (step 3): call `resolve_model(quota, imodel_param)` → `IsolatedModel`; call `resolve_effort(&model, effort_param)` → `Option<&str>`; call `claude_profile_core::account::refresh_account_token()` with the resolved model and effort args (Feature 017 AC-34 / invariant 008) — which internally applies `expiresAt=1` manipulation (Feature 017 AC-32, RT rotation) and live credential sync for the current account (Feature 017 AC-33, race safety), then invokes `run_isolated` with `["--print", "."]`. The subprocess is idempotent — exits immediately if account is already active. After subprocess completes: re-fetch quota via `fetch_oauth_usage` unconditionally using the saved OAuth token (best-effort; failure does not abort the switch). The re-fetch ensures the account's session state is visible to any subsequent command (e.g., `.usage`) that also evaluates quota timers. The touch record itself (`mark_touched`) is written only when `refresh_account_token` returned `Some` — a subprocess that failed outright records no touch, since a stamp would then suppress the corrective re-touch for `TOUCH_GRACE_SECS` (see [024_session_touch.md AC-22](024_session_touch.md)). The re-fetch above is deliberately *not* part of that gate: it runs seconds after the subprocess, well inside `TOUCH_PROPAGATION_SECS`, so requiring it to report a 5h window would suppress the stamp in exactly the propagation-lag case the flag exists for. (Fix(BUG-285): idle check removed; `AlreadyActive` variant removed from `PreSwitchOutcome`. Fix(BUG-288): post-subprocess re-fetch added. Fix(BUG-552): the touch stamp gated on the refresh result.)
...
2026-06-25 · 16:40:04 · account.use  {name}  model override: sonnet→opus (7d(Son) left=5%)  effort→max  ← fetch OK + 7d(Son) < 10% + snapshot was Sonnet (any form) or full-ID opus; omitted otherwise
...
- **AC-19**: When `trace::1` and the model override fires (AC-18): emits `... · account.use  {name}  model override: sonnet→opus (7d(Son) left={N}%)  effort→max` before the `model:` line when upgrading to Opus. Emits `... · account.use  {name}  model override: opus→sonnet (7d(Son) left={N}%)  effort→high` when restoring to Sonnet (Sonnet tier present and left >= 10%, Fix BUG-311). Omitted when no model change occurs (model already correct, tier absent, or quota fetch failed).
...
- **AC-22**: `clp .account.use name::kimi` (target `backend: redirect`, default `touch::1`) skips quota-fetch and the touch subprocess entirely — exits 0 with zero HTTP calls attempted; `pre_switch_touch_ctx()` returns before any of steps 3/4b/5 execute.
