...
| `task/claude_profile/bug/286_switch_account_model_id_not_normalized_to_shorthand.md` | BUG-286 🟢 Fixed (TSK-261): `override_session_model_to_opus` gate extended to match `"claude-opus-4-6"` (full-ID opus) in addition to Sonnet forms; normalized to `"opus"` shorthand when 7d(Son) < 20% — see AC-18 |
