...
- **AC-01**: `.accounts` accepts all active parameters from the unified set. Unknown parameters exit 1. REMOVED migration stubs exit 1 with a migration message: `next::` (→ `sort::`), `assign::`/`for::`/`active::` (→ `assignee::`), `unclaim::` (→ `owner::0`), and the 14 legacy field toggles (→ `cols::`). `next::` rejection matches `.usage` exactly (audit 2026-08 — formerly silently accepted on `.accounts` only).
...
- **AC-08**: `.accounts assignee::devuser@devbox name::X` writes `{credential_store}/_active_devbox_devuser` = X. Exits 0. `~/.claude/.credentials.json`, `~/.claude.json`, `{name}.json` NOT touched. (Formerly `active::devuser@devbox name::X` — Feature 064; `active::` is now REMOVED_TOGGLE — Feature 065.)
...
- **AC-10**: `.accounts assignee::devuser@devbox` (no `name::`) clears `{credential_store}/_active_devbox_devuser`. Exits 0. No credentials or `{name}.json` touched. (Formerly `active::devuser@devbox` — Feature 064.)
...
- **AC-15** *(revised — audit 2026-08; the original "same algorithm as `.usage`" criterion was never implemented)*: `.accounts refresh::1` and `.accounts touch::1` are accepted without error but inert — `accounts_routine()` never invokes the fetch/touch pipeline, output is identical to the default local-only invocation, and no ` · fetch`/` · touch` trace lines appear with `trace::1`. The live pipeline remains `.usage`-only; the full inert param set is enumerated in [001_account.md](../cli/command/001_account.md) Notes: Inert parity params.
- **AC-16**: `.usage owner::0 name::X` clears owner field — identical to `.accounts owner::0 name::X`.
- **AC-17**: `.usage assignee::devuser@devbox name::X` writes marker — identical to `.accounts assignee::devuser@devbox name::X`. (Formerly `active::devuser@devbox name::X` — Feature 064.)
...
- **AC-23**: `.accounts owner::devuser@devbox name::X,Y,Z` sets ownership for X, Y, and Z in one invocation. Each account evaluated against G8 independently.
