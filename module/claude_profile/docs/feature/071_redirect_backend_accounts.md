...
- **AC-19**: `clp .account.save name::kimi` (no `backend::`, no `preset::`) when `kimi.json` holds `backend: redirect` exits 0 WITHOUT touching either store file — stdout reports the skip (`'kimi' is a redirect account (static credentials) — nothing to snapshot; save skipped`), `trace::1` adds `skipped (reason: redirect backend — static credentials, nothing to snapshot)`. Rationale: a redirect account's credential is a static key already in the store; the live-OAuth-session snapshot a bare save performs does not apply, and defaulting the absent `backend::` to anthropic would ride AC-15's delete-and-rewrite, destroying `base_url`/`redirect_model`/`inference_provider`/`claim_lock` (BUG-549 — triggered automatically by the watchdog's per-tick bare save whenever a redirect account is active). Explicit `backend::redirect ...` re-saves and explicit `backend::anthropic` re-backends (AC-15) are deliberate acts and remain unaffected. Same backend-driven unconditional-skip family as AC-16.
