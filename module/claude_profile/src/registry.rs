...
pub fn register_commands( registry : &mut unilang::registry::CommandRegistry )
{
  ...
  reg_cmd( registry, ".accounts",       "List all saved accounts with identity column control",
    vec![
      ...
      reg_arg_opt( "sort",              Kind::String  ).with_description( "Row ordering strategy: `name` (default), `renew`, `endurance`, `drain`, `next`" ),
      ...
      reg_arg_opt( "next",              Kind::String  ).with_description( "Recommendation strategy: `renew` (default), `endurance`, `drain`" ),
