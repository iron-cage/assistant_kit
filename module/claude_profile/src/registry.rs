...
pub fn register_commands( registry : &mut unilang::registry::CommandRegistry )
{
  ...
  reg_cmd( registry, ".account.save", "Save current credentials as a named account profile",
    vec![
      ...
      reg_arg_opt( "preset",         Kind::String ).with_description( "Named provider preset pre-filling backend::/base_url::/inference_provider:: for a known foreign provider; explicit params always override the preset default. `kimi` and `deepseek` are recognized today; see docs/cli/param/074_preset.md" ),
