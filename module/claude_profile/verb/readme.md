...
`build`, `clean`, `install`, `test`, `test_only`, and `verify` forward all arguments after `--dry-run` straight through to the underlying command — `cargo build`/`cargo clean`/`cargo install`/`will .test`/`cargo nextest run --all-features` respectively (`test` and `test_only` forward via `runbox .live` into their `*.d/l1` layer, which itself forwards on). This is what makes `will`-style `key::value` overrides work end-to-end, e.g. `./verb/test scope::subtree` or `./verb/verify level::5` — the destination tool validates and rejects anything it doesn't recognize; the wrapper never judges argument validity itself. `test_only` still requires at least one argument, since a bare invocation with none would silently run the full untargeted suite — that's a check on its own contract (did you give us a filter at all), not a guess about what the filter should look like. `run` and `lint` are passthrough dispatchers too — they forward all arguments to the underlying `cargo run` / `cargo clippy` invocation by design.
