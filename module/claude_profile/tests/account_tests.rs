...
fn save_creates_credential_store_when_missing()
{
  ...
  account::save( "alice@acme.com", &credential_store, &paths, true, None, None, None ).expect( "save" );
...
fn save_overwrites_existing_entry()
{
  ...
  let paths = ClaudePaths::new().expect( "HOME set" );
  account::save( "alice@acme.com", &credential_store, &paths, true, None, None, None ).expect( "first save" );
  ...
  account::save( "alice@acme.com", &credential_store, &paths, true, None, None, None ).expect( "second save" );
...
fn save_rejects_empty_name()
{
  let ( _dir, credential_store ) = setup_home( CREDENTIALS );
  let paths = ClaudePaths::new().expect( "HOME set" );
  let err = account::save( "", &credential_store, &paths, true, None, None, None ).expect_err( "empty name must fail" );
...
fn save_rejects_name_with_slash()
{
  let ( _dir, credential_store ) = setup_home( CREDENTIALS );
  let paths = ClaudePaths::new().expect( "HOME set" );
  let err = account::save( "work/home", &credential_store, &paths, true, None, None, None ).expect_err( "slash must fail" );
  assert_eq!( err.kind(), std::io::ErrorKind::InvalidInput );
}
...
fn list_marks_active_account_via_active_marker()
{
  ...
  let ( _dir, credential_store ) = setup_home( CREDENTIALS );
  let paths = ClaudePaths::new().expect( "HOME set" );
  account::save( "alice@acme.com", &credential_store, &paths, true, None, None, None ).expect( "save alice@acme.com" );
  account::save( "alice@home.com", &credential_store, &paths, true, None, None, None ).expect( "save alice@home.com" );
...
fn list_returns_accounts_sorted_by_name()
{
  ...
  let ( _dir, credential_store ) = setup_home( CREDENTIALS );
  let paths = ClaudePaths::new().expect( "HOME set" );
  account::save( "zebra@acme.com", &credential_store, &paths, true, None, None, None ).expect( "save zebra" );
  account::save( "alpha@acme.com", &credential_store, &paths, true, None, None, None ).expect( "save alpha" );
...
fn delete_removes_credential_file()
{
  ...
  let ( _dir, credential_store ) = setup_home( CREDENTIALS );
  let paths = ClaudePaths::new().expect( "HOME set" );
  account::save( "alice@oldco.com", &credential_store, &paths, true, None, None, None ).expect( "save" );
...
fn test_bug174_mre_switch_preserves_machine_global_commands()
{
  ...
  account::save( "a@x.com", &credential_store, &paths, true, None, None, None ).expect( "save A" );
  ...
  account::save( "b@y.com", &credential_store, &paths, true, None, None, None ).expect( "save B" );
