...
///
/// `$PRO` is a *storage root* — a directory. If `$PRO` resolves to an existing
/// file on disk, it is unusable as a root and must be transparent: the code must
/// fall through to `$HOME` rather than returning a path that hangs under a file.
///
/// Root cause of the bug: `path.exists()` returns `true` for files, so a file path
/// would be accepted as root, producing `<file>/persistent/claude_profile` — an
/// unusable `base()` that makes `ensure_exists()` fail with ENOTDIR.
