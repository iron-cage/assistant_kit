...
Structural compliance validation cases for the `docs/feature/` collection (42 files). Validates that all feature doc instances conform to the per-type H3 section format required by doc.rulebook.md § Feature Documentation : Rule 9a.
...
| DT-04 | `### Sources` section present in all 39 feature docs | Structure |
...
- **Scope:** All 42 `docs/feature/*.md` files
...
- **Scope:** All 42 `docs/feature/*.md` files (023 is deprecated and legitimately omits `### Sources`)
- **Command:** `grep -l "### Sources" docs/feature/*.md | wc -l`
- **Expected:** `41`
- **Failure:** Count less than 41 — at least one non-deprecated feature doc is missing `### Sources`
- **Note:** Every active feature doc references source files; `### Sources` must be present. Deprecated feature 023 is excluded.

---

### DT-05: Tests section present in expected files

- **Scope:** Feature docs that reference test files (approximately 22 of 42)
