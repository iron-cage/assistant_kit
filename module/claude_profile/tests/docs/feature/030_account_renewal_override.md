...
Feature behavioral requirement test cases for `docs/feature/030_account_renewal_override.md`. Each FT case maps to one acceptance criterion. Command-level tests (IT-N) are in [cli/command/014_account_renewal.md](../cli/command/014_account_renewal.md).
...
- **Source fn:** ⏳ (in `tests/cli/account_mutations_test.rs` and `tests/cli/usage_test.rs`)
