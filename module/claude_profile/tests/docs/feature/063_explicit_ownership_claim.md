...
|----|------|----------|--------|
| FT-01 | `ft_owner_sets_owner_field` | AC-01: `owner::devuser@devbox name::X` writes owner field | ✅ |
...
| FT-12 | `f63_ft12_usage_owner_mirrors_accounts` | AC-12: `.usage owner::` same behavior as `.accounts owner::` | ✅ |

**Total:** 12 test cases (12 FT)

---

### FT-01: `owner::devuser@devbox name::X` writes owner field

- **Given:** `alice@corp.com.json` exists in credential store. Account is unowned (`"owner": ""`).
- **When:** `clp .accounts owner::devuser@devbox name::alice@corp.com`
- **Then:** Exits 0. `alice@corp.com.json` contains `"owner": "devuser@devbox"`. stdout contains `owned alice@corp.com by devuser@devbox`. Credentials file unchanged.
- **Exit:** 0
- **Source fn:** `ft_owner_sets_owner_field` (in `account_owner_param_test.rs`)
...
- **When:** `clp .accounts owner::devuser@devbox` (no `name::`)
- **Then:** Exits 1. stderr contains usage error indicating `name::` is required when setting a non-`0` owner identity.
- **Exit:** 1
- **Source fn:** `ft_owner_requires_name` (in `account_owner_param_test.rs`)
...
- **Exit:** 1
- **Source fn:** `ft_owner_g8_blocks_non_owner` (in `account_owner_param_test.rs`)

---

### FT-04: Unowned account passes G8; `owner::` write succeeds

- **Given:** `alice@corp.com.json` has `"owner": ""` (unowned). Caller is `devuser@devbox`.
- **When:** `clp .accounts owner::devuser@devbox name::alice@corp.com`
- **Then:** Exits 0. G8 passes because account is unowned. `alice@corp.com.json` contains `"owner": "devuser@devbox"`.
- **Exit:** 0
- **Source fn:** `ft_owner_unowned_passes_g8` (in `account_owner_param_test.rs`)
...
- **When:** `clp .accounts owner::devuser@devbox unclaim::1 name::alice@corp.com`
...
- **Exit:** 1
- **Source fn:** `ft_owner_mutual_exclusion_unclaim` (in `account_owner_param_test.rs`)
...
- **When:** `clp .accounts owner::devuser@devbox name::alice@corp.com dry::1`
- **Then:** Exits 0. stdout contains `[dry-run] would set owner of alice@corp.com to devuser@devbox`. `alice@corp.com.json` is NOT modified — still contains `"owner": ""`.
- **Exit:** 0
- **Source fn:** `ft_owner_dry_run_preview` (in `account_owner_param_test.rs`)

---

### FT-07: `force::1` bypasses G8 for other-owned account

- **Given:** `alice@corp.com.json` has `"owner": "other@remote"`. Caller is `devuser@devbox` (G8 would block without force).
- **When:** `clp .accounts owner::devuser@devbox name::alice@corp.com force::1`
- **Then:** Exits 0. G8 bypassed by `force::1`. `alice@corp.com.json` contains `"owner": "devuser@devbox"`.
- **Exit:** 0
- **Source fn:** `ft_owner_force_bypasses_g8` (in `account_owner_param_test.rs`)
...
- **When:** `clp .accounts owner::devuser@devbox name::alice@corp.com trace::1`
- **Then:** Exits 0. Owner field written. stderr contains a diagnostic line referencing the G8 gate evaluation result.
- **Exit:** 0
- **Source fn:** `ft_owner_trace_emits_diagnostic` (in `account_owner_param_test.rs`)
...
- **When:** `clp .accounts owner::devuser@devbox name::alice`
- **Then:** Exits 0. Prefix `"alice"` resolves to `alice@corp.com`. `alice@corp.com.json` contains `"owner": "devuser@devbox"`.
- **Exit:** 0
- **Source fn:** `ft_owner_prefix_resolution` (in `account_owner_param_test.rs`)
...
- **Exit:** 1
- **Source fn:** `ft_owner_empty_value_rejected` (in `account_owner_param_test.rs`)
...
- **When-1:** `clp .accounts owner::devuser@devbox name::alice@corp.com` (caller = devuser@devbox)
- **When-2:** `clp .accounts owner::other@remote name::alice@corp.com` (different caller, no force)
- **Then-1:** Exits 0. `alice@corp.com.json` owner set to `"devuser@devbox"`.
- **Then-2:** Exits 1. G8 enforces the newly set owner — `other@remote` is blocked.
- **Exit:** 0 (step 1), 1 (step 2)
- **Source fn:** `ft_owner_gates_respect_new_value` (in `account_owner_param_test.rs`)
...
- **When-A:** `clp .accounts owner::devuser@devbox name::alice@corp.com`
- **When-B:** `clp .usage owner::devuser@devbox name::alice@corp.com`
- **Then-A:** Exits 0. `alice@corp.com.json` contains `"owner": "devuser@devbox"`.
- **Then-B:** Exits 0. Identical result — `owner::` mutation works the same on `.usage` and `.accounts`.
- **Exit:** 0 both cases
- **Source fn:** `f63_ft12_usage_owner_mirrors_accounts` (in `tests/cli/usage_feature_test.rs`) — corrected name and file path
