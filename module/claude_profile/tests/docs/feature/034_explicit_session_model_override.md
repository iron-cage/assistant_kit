...
### FT-10: `set_session_model()` preserves existing `settings.json` keys

- **Given:** `settings.json` contains `{"theme": "dark", "autoUpdaterStatus": "disabled"}`.
- **When:** `set_session_model(&paths, Some("claude-opus-4-6"))` is called.
- **Then:** `settings.json` contains `{"theme": "dark", "autoUpdaterStatus": "disabled", "model": "claude-opus-4-6"}`. No keys removed.
- **Exit:** Ok(())
...
### FT-11: `settings.json` created when absent

- **Given:** `~/.claude/settings.json` does not exist.
- **When:** `set_session_model(&paths, Some("claude-opus-4-6"))` is called.
- **Then:** `settings.json` is created and contains `{"model":"claude-opus-4-6"}`.
- **Exit:** Ok(())
