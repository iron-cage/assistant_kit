...
Feature behavioral requirement test cases for `docs/feature/022_org_identity_snapshot.md` (FR-22). Each FT case maps to one acceptance criterion. Parameter edge cases are in [cli/param/030_org_uuid.md](../cli/param/030_org_uuid.md) and [cli/param/031_org_name.md](../cli/param/031_org_name.md).

### AC Coverage Index

| FT | Criterion | AC | Category |
|----|-----------|----|---------|
| FT-01 | `save` writes `{name}.roles.json` when endpoint 005 succeeds | AC-01 | Lifecycle |
| FT-02 | Endpoint 005 failure → `roles.json` absent; `save` still exits 0 | AC-02 | Best-Effort |
| FT-03 | Re-`save` overwrites `roles.json` with fresh data | AC-03 | Idempotency |
| FT-04 | `delete` removes `roles.json`; absent `roles.json` causes no error | AC-04 | Lifecycle |
...
| FT-07 | `org_uuid::1` on `.credentials.status` shows `Org ID:` from active `roles.json` | AC-07 | Field Presence |
| FT-08 | `org_name::1` on `.credentials.status` shows `Org:` from active `roles.json` | AC-08 | Field Presence |
...
| FT-11 | Null workspace fields in `roles.json` → empty string in `Account`, `N/A` in text | AC-11 | Personal Account |

### Test Case Index

| ID | Test Name | AC | Category |
|----|-----------|----|---------|
| FT-01 | save writes roles.json when endpoint 005 returns valid response | AC-01 | Lifecycle |
| FT-02 | endpoint 005 failure → no roles.json written; save still exits 0 | AC-02 | Best-Effort |
| FT-03 | re-save overwrites roles.json with fresh org data | AC-03 | Idempotency |
| FT-04 | delete removes roles.json; absent file causes no error | AC-04 | Lifecycle |
...
| FT-07 | org_uuid::1 shows Org ID on .credentials.status from active roles.json | AC-07 | Field Presence |
| FT-08 | org_name::1 shows Org on .credentials.status from active roles.json | AC-08 | Field Presence |
...
| FT-11 | null workspace fields in roles.json render as empty string and N/A | AC-11 | Personal Account |

**Total:** 11 FT cases

---

### FT-01: `save` writes `{name}.roles.json` when endpoint 005 returns valid response
...
### FT-02: Endpoint 005 failure → no `roles.json` written; `save` still exits 0

- **Given:** An account credential file with an invalid `accessToken` value (causes endpoint 005 to return HTTP 401); no pre-existing `{name}.roles.json` in the credential store.
- **When:** `clp .account.save`
...
### FT-03: Re-`save` overwrites `roles.json` with fresh org data

- **Given:** Account `alice@example.com` has an existing `alice@example.com.roles.json` in the credential store (stale content); the active `accessToken` is valid; endpoint 005 returns an updated response on the second call.
...
### FT-04: `delete` removes `roles.json`; absent file causes no error

- **Given:** Account `alice@acme.com` whose credential store contains a pre-seeded `alice@acme.com.roles.json` fixture. Account `bob@acme.com` whose credential store does NOT have `bob@acme.com.roles.json`.
...
- **Given:** Two accounts `alice@acme.com` and `bob@acme.com`; `alice@acme.com.roles.json` exists in the credential store with `"organization_uuid":"aaaaaaaa-1111-cccc-dddd-eeeeeeeeeeee"`; `bob@acme.com.roles.json` does NOT exist.
...
- **Given:** Two accounts `alice@acme.com` and `bob@acme.com`; `alice@acme.com.roles.json` exists with `"organization_name":"Acme Corp"`; `bob@acme.com.roles.json` does NOT exist.
...
### FT-07: `org_uuid::1` shows `Org ID:` on `.credentials.status` from active `roles.json`

- **Given:** Active account `alice@acme.com`; `_active` marker points to alice; `{credential_store}/alice@acme.com.roles.json` exists with `"organization_uuid":"aaaaaaaa-1111-cccc-dddd-eeeeeeeeeeee"`.
...
### FT-08: `org_name::1` shows `Org:` on `.credentials.status` from active `roles.json`

- **Given:** Active account `alice@acme.com`; `_active` marker points to alice; `{credential_store}/alice@acme.com.roles.json` exists with `"organization_name":"Acme Corp"`.
...
- **Given:** Active account `alice@acme.com` with `alice@acme.com.roles.json` containing all 5 org fields (`organization_uuid`, `organization_name`, `organization_role`, `workspace_uuid`, `workspace_name`). `org_uuid::` and `org_name::` params are NOT passed.
...
### FT-11: Null workspace fields in `roles.json` render as empty string and `N/A`

- **Given:** Account `alice@personal.com` with `{credential_store}/alice@personal.com.roles.json` containing `"workspace_uuid":null,"workspace_name":null` (personal account with no workspace membership); file also contains valid `organization_uuid` and `organization_name`.
