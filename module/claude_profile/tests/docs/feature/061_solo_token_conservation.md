...
- **Then:** The solo gate fires before G2. Account B's result is unchanged (original 401 error preserved). Without solo, the same account would be refreshed — `should_refresh` returns true (401+expired), `refresh_account_token` returns None (empty store), result becomes `Err("token refresh failed")` (label per BUG-539). Converted from gag-based trace capture to behavioral test.
