...
`active::USER@MACHINE name::X`. Tests verify the current interface.
...
**Migration:** `assign::1 name::X` (Feature 037) → `active::USER@MACHINE name::X` (Feature 064).
...
- **When:** `clp .accounts active::testuser@testmachine name::alice@acme.com` (formerly `assign::1 name::alice@acme.com`)
...
- **When:** `clp .accounts active::testuser@testmachine name::nobody@acme.com`
...
- **Then:** Exit 1. Migration message: "REMOVED — use `active::USER@MACHINE name::X`". No marker file written.
- **Exit:** 1
