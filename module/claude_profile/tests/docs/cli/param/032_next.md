...
| EC-3 | `next::bogus` exits 1 naming both valid values | Invalid Value |
...
### EC-3: `next::bogus` exits 1 naming both valid values

- **Given:** Any environment (empty credential store).
- **When:** `clp .usage next::bogus`
- **Then:** Exits 1. Stderr contains each of the two valid values: "endurance", "drain".
- **Exit:** 1
...
- **Then:** Exits 1. Stderr contains "endurance" and "drain" (the two valid values). Does NOT contain "all", "session", or "reset". TSK-184 reduced `NextStrategy` from 5 variants to 2; "all" is no longer a recognized value.
- **Exit:** 1
- **Source:** [feature/023_next_account_strategies.md AC-05](../../../../docs/feature/023_next_account_strategies.md)
