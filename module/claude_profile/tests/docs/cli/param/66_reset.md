...
| EC-05 | `ec5_reset_1_idempotent_when_no_preference_set` | `~/.clr/prefs.json` exists but has no `subprocess_model` key → `reset::1` | exits 0 without error; idempotent no-op | ✅ |
| EC-06 | `ec6_reset_1_idempotent_when_prefs_file_absent` | `~/.clr/prefs.json` does not exist → `reset::1` | exits 0 without error; no file created | ✅ |
| EC-07 | `ec7_reset_1_preserves_other_prefs_keys` | `~/.clr/prefs.json` has `subprocess_model` plus unrelated keys → `reset::1` | `subprocess_model` removed; other keys preserved; exit 0 | ✅ |
...
| EC-10 | `ec10_reset_1_stdout_message` | `~/.clr/prefs.json` has `"subprocess_model": "claude-sonnet-5"` → `reset::1` | stdout contains `model.select: (reset to default)`; exit 0 | ✅ |

**Total:** 10 edge case tests

---

### EC-01: `reset::1` removes `subprocess_model` — observable file-state change

- **Given:** `~/.clr/prefs.json` contains `{"subprocess_model": "claude-opus-4-8"}`
- **When:** `clp .model.select reset::1`
- **Then:** Exits 0. `~/.clr/prefs.json` no longer contains the `subprocess_model` key. `clr run/ask/isolated/refresh` subsequently uses `ISOLATED_DEFAULT_MODEL`.
- **Exit:** 0
...
- **Then:** Exits 0. `~/.clr/prefs.json` is unchanged — still contains `"subprocess_model": "claude-opus-4-8"`. Mode is get, determined by `id::` absence, not by the `reset::0` value itself.
- **Exit:** 0
...
- **Given:** Any `~/.clr/prefs.json` state
- **When:** `clp .model.select reset::1 id::claude-opus-4-8`
- **Then:** Exits 1. stderr contains `id:: and reset::1 are mutually exclusive`. `~/.clr/prefs.json` is unchanged.
- **Exit:** 1
...
- **Given:** `~/.clr/prefs.json` exists and contains `{"other_key": "value"}` — no `subprocess_model` key present.
- **When:** `clp .model.select reset::1`
- **Then:** Exits 0 without error. `~/.clr/prefs.json` still contains `{"other_key": "value"}`, unchanged. Operation is idempotent.
- **Exit:** 0
- **Source:** [param/066_reset.md](../../../../docs/cli/param/066_reset.md)

---

### EC-06: `reset::1` idempotent when `prefs.json` file is absent

- **Given:** `~/.clr/prefs.json` does not exist on disk.
- **When:** `clp .model.select reset::1`
...
### EC-07: `reset::1` preserves other keys in `prefs.json`

- **Given:** `~/.clr/prefs.json` contains `{"subprocess_model": "claude-opus-4-8", "other_key": "unrelated_value"}`
- **When:** `clp .model.select reset::1`
- **Then:** Exits 0. `~/.clr/prefs.json` contains `{"other_key": "unrelated_value"}` only — `subprocess_model` removed, all other keys preserved verbatim.
- **Exit:** 0
- **Source:** [param/066_reset.md](../../../../docs/cli/param/066_reset.md)
...
- **Given:** `~/.clr/prefs.json` contains `{"subprocess_model": "claude-sonnet-5"}`
- **When:** `clp .model.select reset::1`
