...
| EC-7 | Missing `roles.json` snapshot → `Org ID: N/A` | Edge Case |
...
- **Given:** Active account with `roles.json` containing organization_uuid
...
- **Given:** Active account with `roles.json` containing `organization_uuid="org-xyz"`
