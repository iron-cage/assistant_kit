...
| EC-9 | `from_now::+30d` writes `_renewal_at` ~30 days in the future | Behavioral Divergence |
| EC-10 | `from_now::+365d` writes `_renewal_at` ~365 days in the future | Behavioral Divergence |
...
### EC-9: `from_now::+30d` writes `_renewal_at` ~30 days in the future

- **Behavioral Divergence:** Two valid positive deltas produce different computed `_renewal_at` timestamps. `+30d` yields a date ~30 days from now; `+365d` yields a date ~365 days from now. The resulting `.usage` `~Renews` countdown differs correspondingly.
- **Given:** Account `test@example.com` exists.
...
### EC-10: `from_now::+365d` writes `_renewal_at` ~365 days in the future

- **Behavioral Divergence:** (pair with EC-9)
- **Given:** Account `test@example.com` exists.
