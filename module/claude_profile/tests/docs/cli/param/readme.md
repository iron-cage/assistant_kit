...
| 64_id.md | Edge cases for `id::` parameter (originally `.model.select`'s set-mode model pin, Feature 069; `.model.select` retired by Feature 035/Task 465 — `id::` now live only on `.provider.select`) |
| 65_offline.md | Edge cases for `offline::` parameter (`.models` static catalog vs live API toggle — Feature 068) |
| 66_reset.md | Edge cases for `reset::` parameter (originally `.model.select`'s subprocess-model-pin clear, Feature 069; `.model.select` retired by Feature 035/Task 465 — `reset::` now live only on `.provider.select`) |
