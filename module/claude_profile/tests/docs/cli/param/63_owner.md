...
| EC-03 | `ec3_owner_and_unclaim_removed_toggle` | `owner::devuser@devbox unclaim::1 name::X` | exit 1 — `unclaim::1` is REMOVED_TOGGLE (Feature 064) | ✅ |
| EC-04 | `ec4_owner_missing_name_exits_1` | `owner::devuser@devbox` (no name::) | exit 1 with usage error | ✅ |
...
| EC-07 | `ec7_owner_dry_no_file_writes` | `owner::devuser@devbox name::X dry::1` | `[dry-run]` message, no `{name}.json` change | ✅ |
| EC-08 | `ec8_owner_overwrite_existing` | account already owned by caller → `owner::new@identity` | overwrites to new identity | ✅ |
| EC-09 | `ec9_owner_idempotent_same_value` | `owner::devuser@devbox` when already `owner: "devuser@devbox"` | no-op write, exit 0 | ✅ |
...
| EC-13 | `ec13_owner_set_comma_list_batch_set` | `owner::devuser@devbox name::X,Y,Z` | sets owner for X, Y, Z independently; G8 per account; exits 0 | ✅ |
...
### EC-03: `owner::devuser@devbox unclaim::1 name::X` — REMOVED_TOGGLE exit 1

- **Given:** Account `X` exists
- **When:** `clp .accounts owner::devuser@devbox unclaim::1 name::X`
...
### EC-04: `owner::devuser@devbox` without `name::` exits 1

- **Given:** Any account state
- **When:** `clp .accounts owner::devuser@devbox` (no `name::` provided)
...
- **When:** `clp .accounts owner::devuser@devbox name::X`
...
- **When:** `clp .accounts owner::devuser@devbox name::X force::1`
- **Then:** Exits 0. `{X}.json` contains `"owner": "devuser@devbox"`. G8 bypassed by `force::1`.
- **Exit:** 0
...
- **When:** `clp .accounts owner::devuser@devbox name::X dry::1`
...
- **Given:** `{X}.json` has `"owner": "devuser@devbox"` (caller owns it)
...
- **Given:** `{X}.json` has `"owner": "devuser@devbox"`. Caller identity is `devuser@devbox`.
- **When:** `clp .accounts owner::devuser@devbox name::X`
...
### EC-13: `owner::devuser@devbox name::X,Y,Z` — batch-set via comma-list
...
- **Then:** Exits 0. All three `.json` files contain `"owner": "devuser@devbox"`. G8 evaluated independently for each. Stdout contains `owned {name} by devuser@devbox` for each.
- **Exit:** 0
...
- **Then:** Exits 0. stdout contains `[dry-run] would clear owner of alice@corp.com`. `alice@corp.com.json` is NOT modified — still contains `"owner": "devuser@devbox"`. No files written.
- **Exit:** 0
...
- **Given:** `alice@corp.com.json` exists with `"owner": ""` (already unowned). Caller identity is `devuser@devbox`.
- **When:** `clp .accounts owner::0 name::alice@corp.com`
...
- **Given:** `alice@corp.com.json` has `"owner": "other@remote"`. Caller identity is `devuser@devbox`. No `force::1`.
- **When:** `clp .accounts owner::0 name::alice@corp.com`
...
- **Given:** `alice@corp.com.json` and `bob@corp.com.json` both have `"owner": "other@remote"`. Caller identity is `devuser@devbox`. No `name::`.
