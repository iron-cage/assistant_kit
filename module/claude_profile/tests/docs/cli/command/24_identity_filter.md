...
- **Covers**: AC-01 through AC-08, AC-15 (📋 planned — no tests exist yet)

### Test Cases

| IT | AC | Scenario | Source fn |
|----|----|----------|-----------|
| IT-01 | AC-01 | Get with no filter file → `include=[] exclude=[] (permit-all)`, exit 0 | `t01_filter_get_permit_all` |
| IT-02 | AC-02 | `include::a,b` writes sorted deduplicated include, empty exclude | `t02_filter_include_write` |
| IT-03 | AC-03 | Each given side fully replaces; both sides in one call | `t03_filter_both_sides_replace` |
| IT-04 | AC-04 | `include ∩ exclude ≠ ∅` → exit 1 naming overlap, nothing written | `t04_filter_overlap_exits_1` |
| IT-05 | AC-05 | Invalid tag in either set → exit 1 naming it, nothing written | `t05_filter_invalid_tag_exits_1` |
| IT-06 | AC-06 | `clear::1` deletes; idempotent; with `include::`/`exclude::` → exit 1 | `t06_filter_clear_idempotent_and_exclusive` |
| IT-07 | AC-07 | `identity::USER@MACHINE` targets that seat; malformed → exit 1 | `t07_filter_identity_targeting` |
| IT-08 | AC-08 | Include matching zero tagged accounts → stderr warning, exit 0 | `t08_filter_typo_guard_warns` |
| IT-09 | AC-15 | Get with `format::json`; unsupported format → exit 1 | `t15_identity_commands_json` |
| IT-10 | — | `.identity.filter` appears in `clp .help` after registration | `dot04_all_visible_commands_present` (extend on implementation) |

### Notes

- **📋 Planned — implementation pending.** Source fn names are prescriptive for `tests/cli/identity_filter_test.rs` (IT-10: `tests/cli/dot_test.rs`); none exist yet. Correct drifted names here when implementation lands.
...
- Gate 11 selection behavior (`rotate::1`, auto-switch, footer `Next`) is out of this command's IT scope — covered by FT-09/FT-11/FT-12/FT-13 in the FT spec and `tests/docs/cli/command/06_usage.md`'s suite on implementation.
