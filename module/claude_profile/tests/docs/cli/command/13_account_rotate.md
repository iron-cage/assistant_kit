...
| IT-3 | Rotates to best inactive account | Two accounts; active has lower expiry; checks `_active` changed | 0 |
| IT-4 | Selects highest `expiresAt` among multiple inactives | Three accounts with differing expiry; verifies selected name | 0 |
| IT-5 | `dry::1` shows candidate without switching | Two accounts present; `_active` unchanged after command | 0 |
