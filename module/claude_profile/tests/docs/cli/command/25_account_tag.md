...
- **Covers**: AC-03, AC-05 through AC-10 (📋 planned — no tests exist yet)

### Test Cases

| IT | AC | Scenario | Source fn |
|----|----|----------|-----------|
| IT-01 | AC-05 | `add::a,b` unions into the existing set (dedup, sorted) | `t05_tag_add_unions_set` |
| IT-02 | AC-06 | `remove::a` removes; absent tag is a no-op success | `t06_tag_remove_idempotent` |
| IT-03 | AC-07 | `tags::a,b` replaces the whole set; combining ops exits 1 | `t07_tag_replace_and_mutual_exclusion` |
| IT-04 | AC-08 | No operation given → exit 1 naming `add::`/`remove::`/`tags::` | `t08_tag_no_operation_exits_1` |
| IT-05 | AC-09 | First tag write (incl. `remove::`) migrates legacy `role` to tag, deletes field | `t09_first_tag_write_migrates_role` |
| IT-06 | AC-10 | Ungated (non-owner OK); `name::X,Y` batch; `dry::1` touches nothing | `t10_tag_ungated_batch_dry` |
| IT-07 | AC-03 | Invalid tag in any operation → exit 1 naming it, no write | `t03_invalid_tag_exits_1_no_write` (extend with `.account.tag` assertion) |
| IT-08 | — | Unknown account name → exit 2, nothing written | `t19_account_tag_unknown_account_exits_2` |
| IT-09 | — | Missing required `name::` → exit 1 | `t20_account_tag_missing_name_exits_1` |
| IT-10 | — | `.account.tag` appears in `clp .help` after registration | `dot04_all_visible_commands_present` (extend on implementation) |

### Notes

- **📋 Planned — implementation pending.** Source fn names are prescriptive for `tests/cli/account_tag_test.rs` (IT-10: `tests/cli/dot_test.rs`); none exist yet. Correct drifted names here when implementation lands.
...
- **Exit:** 1
- **Source fn:** `t03_invalid_tag_exits_1_no_write` *(planned; extend with `.account.tag` path assertion)*
...
- **Exit:** 2
- **Source fn:** `t19_account_tag_unknown_account_exits_2` *(planned)*
...
- **Exit:** 1
- **Source fn:** `t20_account_tag_missing_name_exits_1` *(planned)*
- **Source:** [001_account.md Command 25](../../../../docs/cli/command/001_account.md)
