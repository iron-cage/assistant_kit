...
| IT-08 | AC-08 | `id::VALUE` on existing `config.toml` (seeded with `.model.select`'s `model` key) → both keys preserved | `it08_set_preserves_model_select_key` |
...
- Backing store is `~/.clr/config.toml`'s `provider` key, sharing the same tiered flat-TOML file as `.model.select`'s `model` key via `claude_core::toml_io` — the two keys never interact (IT-08 asserts this directly).
- Unlike `.model.select`'s get-mode `(unset)` sentinel, `.provider.select`'s get mode always resolves to an effective value (`anthropic` when never selected) — the global provider is a standing config scalar, never "nothing selected" (AC-07, AC-16).
...
- IT-08: seed `config.toml` with `model = "claude-opus-4-8"` (from `.model.select`); call `id::kimi`; verify both `model` and `provider` keys present with their respective values.
...
### IT-08: `id::VALUE` preserves `.model.select`'s key
...
- **Variant:** When never selected → `{"provider":"anthropic"}` (never `null` — distinct from `.model.select`'s unset-is-null behavior).
- **Exit:** 0
