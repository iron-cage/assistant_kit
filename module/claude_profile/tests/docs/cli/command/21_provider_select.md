...
| IT-06 | AC-12 | `reset::1` with no `config.toml` → exits 0 idempotently | *(no test — coverage gap; `t09` always seeds a config first)* |
...
- **Coverage gaps (no backing test as of this writing):** IT-04 (`id::moonshot`), IT-06 (`reset::1` with no `config.toml` present), and IT-08 (both keys asserted present simultaneously after a set over a pre-existing `model` key) — see the Test Case Index above and each case's own note.
...
> **Coverage gap** — no test exercises this scenario. `t09_provider_select_reset_preserves_model_key` is the only `reset::1` test and always seeds `config.toml` first (`.model scope::subprocess` then `id::kimi`); no test calls `reset::1` against a directory where `~/.clr/config.toml` was never created.
