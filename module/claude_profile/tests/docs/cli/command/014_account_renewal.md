...
Set or clear a billing renewal timestamp override (`_renewal_at`) stored in `{name}.claude.json`.

| # | Test | Conditions | Exit |
|---|------|-----------|------|
| IT-1 | `at::` writes `_renewal_at` to `{name}.claude.json` | Account exists; `at::2026-06-29T21:00:00Z`; verifies file contains field | 0 |
| IT-2 | `from_now::+1h` writes future `_renewal_at` | Account exists; `from_now::+1h`; verifies ISO-8601 string is present | 0 |
| IT-3 | `clear::1` removes `_renewal_at` from `{name}.claude.json` | Account has `_renewal_at`; after `clear::1`, field absent | 0 |
...
| IT-9 | `name::all` updates every saved account | Two accounts; `from_now::+0m`; both `.claude.json` updated | 0 |
