...
{
  let pairs = read_all_settings_typed( path )?;
  Ok( pairs.into_iter().map( |( k, v, _ )| ( k, v ) ).collect() )
}
...
/// Return the value for a single key only if it is stored as a plain JSON
/// string.  Returns `None` if the key is absent, or if its value is a
/// number, bool, null, or nested object/array — use [`get_setting`] instead
/// if the stringified form is wanted regardless of the underlying JSON type.
///
/// # Errors
///
/// Returns `Err(NotFound)` if the file does not exist.
/// Returns `Err(InvalidData)` if the file is not valid JSON.
#[ inline ]
pub fn get_string_setting( path : &Path, key : &str ) -> Result< Option< String >, io::Error >
{
  let pairs = read_all_settings_typed( path )?;
  Ok(
    pairs.into_iter()
    .find( |( k, _, _ )| k == key )
    .and_then( |( _, v, stored_as )| ( stored_as == StoredAs::Str ).then_some( v ) )
  )
}

/// Read all key-value settings from a JSON file, preserving each value's
/// inferred [`StoredAs`] type.
///
/// # Errors
///
/// Returns `Err(NotFound)` if the file does not exist.
/// Returns `Err(InvalidData)` if the file is not valid JSON.
#[ inline ]
fn read_all_settings_typed( path : &Path ) -> Result< Vec< ( String, String, StoredAs ) >, io::Error >
{
...
pub fn set_env_var( path : &Path, key : &str, value : &str ) -> Result< (), io::Error >
{
  ...
  let mut env_pairs : Vec< ( String, String ) > = match env_idx
  {
    Some( idx ) => json_parse_flat_object( &pairs[ idx ].1 )?.into_iter().map( |( k, v, _ )| ( k, v ) ).collect(),
    None        => vec![],
  };
...
pub fn remove_env_var( path : &Path, key : &str ) -> Result< (), io::Error >
{
  ...
  let mut env_pairs : Vec< ( String, String ) > = json_parse_flat_object( &pairs[ idx ].1 )?
  .into_iter().map( |( k, v, _ )| ( k, v ) ).collect();
...
/// Parse a JSON object `{key: value, ...}` into `Vec<(String, String, StoredAs)>`.
///
...
fn json_parse_flat_object( src : &str ) -> Result< Vec< ( String, String, StoredAs ) >, io::Error >
{
  ...
  while pos < chars.len()
  {
    ...
    // Parse value
    skip_ws( &chars, &mut pos );
    let ( value, value_type, next ) = parse_json_value( &chars, pos )?;
    pos = next;

    result.push( ( key, value, value_type ) );
...
fn parse_json_value( chars : &[ char ], start : usize ) -> Result< ( String, StoredAs, usize ), io::Error >
{
  if start >= chars.len()
  {
  ...
  match chars[ start ]
  {
    '"' =>
    {
      ...
      Ok( ( s, StoredAs::Str, end ) )
    }
    '{' | '[' =>
    {
      let ( s, end ) = consume_balanced( chars, start )?;
      Ok( ( s, StoredAs::Raw, end ) )
    }
    't' =>
    {
      let ( s, end ) = check_literal( chars, start, &[ 't', 'r', 'u', 'e' ], "true" )?;
      Ok( ( s, StoredAs::Bool, end ) )
    }
    'f' =>
    {
      let ( s, end ) = check_literal( chars, start, &[ 'f', 'a', 'l', 's', 'e' ], "false" )?;
      Ok( ( s, StoredAs::Bool, end ) )
    }
    'n' =>
    {
      let ( s, end ) = check_literal( chars, start, &[ 'n', 'u', 'l', 'l' ], "null" )?;
      Ok( ( s, StoredAs::Raw, end ) )
    }
    c if c == '-' || c.is_ascii_digit() =>
    {
      ...
      Ok( ( num_str, StoredAs::Number, end ) )
    }
    other =>
    {
      Err( io::Error::new(
