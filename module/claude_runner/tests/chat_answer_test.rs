//! `clr chat` reads the answer from the transcript, not the terminal.
//!
//! # Why this exists
//!
//! `clr chat` promises print-mode output: a prompt goes in, an answer comes out.
//! What physically arrives over the pty is not an answer — it is Claude Code's
//! interface, redrawn frame by frame, with the words somewhere inside it. Printing
//! that was the observed defect: a smoke run answered `pineapple` correctly and
//! displayed box rules, a `❯` prompt, a status bar, and spinner frames around it.
//!
//! The answer is instead read from the session's own transcript, which Claude Code
//! writes as it goes and keys by the same conversation id the daemon already holds.
//! These tests pin the reading, because a wrong read is not a crash — it is a
//! plausible-looking answer with the wrong words in it.
//!
//! # What is not tested here
//!
//! A real turn. That needs a real model call, which lives in the manual smoke
//! (`tests/manual/readme.md`). The fixtures below are hand-built to the shape a
//! real transcript actually has — the required-field list comes from
//! `claude_storage_core`'s own parser, and the block mix (thinking, tool use, tool
//! result, text) from a recorded session.
//!
//! # Coverage
//!
...
use claude_runner::{ answer_since, transcript_mark, transcript_path };
...
///
/// Not decoration: `claude_storage_core` rejects a line missing any of them, and a
/// rejected line is skipped rather than reported. A fixture that drops one would
/// silently test an empty file. `cwd` here is the session's own directory recorded
/// inside each entry — nothing under test reads it.
...
fn ca1_an_assistant_text_block_is_the_answer()
{
  ...
  let answer = answer_since( &transcript.path(), 0, SHORT_GRACE );
...
fn ca2_thinking_and_tool_blocks_are_not_the_answer()
{
  ...
  let answer = answer_since( &transcript.path(), 0, SHORT_GRACE ).expect( "no answer" );
...
fn ca3_the_mark_excludes_the_previous_turn()
{
  ...
  // Exactly what `dispatch_chat` does between turns: mark, then send.
  ...
  let answer = answer_since( &transcript.path(), mark, SHORT_GRACE ).expect( "no answer" );
...
fn ca4_a_missing_transcript_falls_back_rather_than_failing()
{
  ...
  assert!
  (
    answer_since( &path, 0, SHORT_GRACE ).is_none(),
    "a missing transcript must report nothing, so `chat` falls back to the terminal"
  );
}
...
fn ca6_the_grace_period_waits_for_a_transcript_that_is_still_being_written()
{
  ...
  let answer = answer_since( &path, 0, Duration::from_secs( 2 ) );
  ...
  (
    ...
    "the turn ends before the transcript is flushed — giving up at that instant prints a terminal dump"
  );
}
...
{
  // No env is set here — this crate's tests do not mutate process-global state
  // (see `gate_unit_test.rs`). Whatever HOME the suite runs under is enough: the
  // claim under test is the shape of the path, not where its root is.
  let Some( path ) = transcript_path( Path::new( "/tmp/work" ), SESSION ) else
  {
    ...
    // case `chat` treats as "fall back to the terminal".
    return;
  };
  ...
  (
    ...
    "the transcript is the conversation id plus .jsonl — the whole reason the daemon's id is enough to find it"
  );
  assert_eq!
  (
...
fn ca8_non_conversation_lines_are_neither_counted_nor_printed()
{
  ...
  &[
    ...
    // conversation entry. `claude_storage_core` skips them; the mark must agree,
    // or the slice would start in the wrong place on the next turn.
