...
| IN-6 | `KNOWN_SUBCOMMANDS` contains all 10 commands; no entry begins with `--` | Naming Invariant | ✅ |
...
- **Exit:** 0
- **Source:** [invariant/003_command_naming.md](../../../docs/invariant/003_command_naming.md), [command/02_help.md](../../../docs/cli/command/02_help.md)
...
### IN-6: `KNOWN_SUBCOMMANDS` contains all 10 commands; no entry begins with `--`

- **Given:** static analysis of `guard_unknown_subcommand()` dispatch in `src/cli/mod.rs`
- **When:** inspect `KNOWN_SUBCOMMANDS` constant
- **Then:** `KNOWN_SUBCOMMANDS` contains exactly 10 entries: `run`, `ask`, `isolated`, `refresh`, `help`, `ps`, `kill`, `tools`, `scope`, `query`; none starts with `--` or `-`; all are bare words. `topic` is not yet a registered subcommand — dispatch wiring is pending implementation (task 521)
- **Exit:** 0
- **Source:** [invariant/003_command_naming.md](../../../docs/invariant/003_command_naming.md)
