...
| IN-13 | Slot owned by a dead PID whose number is occupied by a live non-leader thread → owner reads as dead, slot reclaimed, caller admitted | Invariant Hold |
| IN-14 | Queued-waiter gate file keyed to a live non-leader thread TID → not rendered, self-heal-deleted | Invariant Hold |
| IN-15 | Slot record carrying a start time that mismatches the live occupant of the same PID number → owner reads as dead, slot reclaimed | Invariant Hold |
| IN-16 | Legacy slot record without a start-time field, owner a live thread-group leader → still denied `HeldByLive` (compatibility boundary) | Invariant Hold |

## Test Coverage Summary

- Invariant Hold: 15 tests (IN-1, IN-2, IN-3, IN-4, IN-5, IN-6, IN-7, IN-8, IN-9, IN-11, IN-12, IN-13, IN-14, IN-15, IN-16)
- Documented Residual: 1 test (IN-10)

**Total:** 16 invariant test cases (minimum for `invariant` doc type is 2; this spec exceeds it to cover the fresh-claim burst case, the single-generation reclaim race, the multi-generation orphaned-ticket chain walk, the fallback-scan starvation guard, the ticket-winner self-collision guard, the multi-generation chain-walk regression guard, the opt-in live-owner staleness threshold, the claim-vs-content atomicity of `claim_slot_file()` itself, the fresh-claim arbitration regression guard, the explicitly accepted pre-existing-corruption residual boundary, and both consumers of the shared owner-liveness predicate — reclaim and queued-waiter display — for its zombie clause (BUG-479), its thread-group-leader and same-incarnation clauses (BUG-488), and the legacy-record compatibility boundary of the incarnation binding)

## Architectural Constraint

All cases except IN-12 and IN-14 are integration tests split across `tests/concurrency_gate_test.rs`, `tests/concurrency_gate_ext_test.rs`, and `tests/concurrency_gate_ext2_test.rs` (see Implementation Notes below for the exact file per case) — the atomicity guarantee lives entirely inside `acquire_slot()` and its call site in `wait_for_session_slot()`, and can only be observed by launching real `clr` subprocesses (or, for IN-1/IN-2/IN-9, real concurrent racers) against a shared `CLR_GATE_DIR` fixture. IN-12 and IN-14 live in `tests/ps_command_test.rs`: they exercise the queued-waiter display consumer of the same shared `pid_alive()` predicate (BUG-479, BUG-488), so their natural home is beside the other `clr ps` queued-table cases (IT-10/IT-13); IN-14 is the same case the `ps` command spec registers as IT-47. IN-13/IN-14's thread fixture (`spawn_parked_helper_thread()` in `tests/cli_binary_test_helpers.rs`) spawns a parked helper thread inside the test process and reads its TID via `/proc/thread-self` — a live non-leader thread ID is readdir-invisible (`ls /proc` never lists it) yet `/proc/{tid}/stat` is readable with state ∉ {`Z`} and `/proc/{tid}/status` reports `Tgid != tid`, exactly the occupancy shape that masked a dead waiter in the wild (BUG-488); using the test's own thread makes the collision deterministic instead of waiting for a host PID wrap. IN-15's fixture records a live leader PID but a deliberately wrong start time (any value ≠ `/proc/{pid}/stat` field 22 of the occupant); IN-16's fixture writes the pre-BUG-488 record shape with no start-time field at all, proving the incarnation binding is additive for records written by older binaries. IN-11/IN-12's zombie fixture spawns a real immediately-exiting process and deliberately never `wait()`s on it — the test process stays its parent, so the child remains state `Z` with a live `/proc/{pid}` entry for the test's duration (spawn-and-reap, as IN-2 does, produces an ABSENT PID instead, which is a different liveness state). IN-3 and IN-6 exercise the chain-walking loop added for BUG-402: when a reclaim ticket's own recorded claimant is also dead (never reached `rename()`), `acquire_slot()` advances to the next-generation ticket keyed by that dead claimant's own `(pid, since)` instead of treating the existing ticket as permanent defeat, rechecking the slot's current owner at each generation in case a concurrent caller completed a rename while this call was still walking the chain. IN-5 exercises the self-collision cleanup added for BUG-405 via `CLR_GATE_FORCE_TMP_CLAIM_FAIL_ONCE`, a test-only injection point mirroring the existing `reclaim_test_delay()` idiom used to make an otherwise-narrow race deterministically reproducible. IN-7 exercises the opt-in staleness threshold added for BUG-400 via `CLR_GATE_STALE_SECS`: unlike IN-2/IN-3/IN-6, the pre-seeded owner is a genuinely alive real child process (not a spawned-and-reaped dead one) — the sub-case split (threshold unset vs. set) is what proves the fix is additive rather than a behavior change for existing callers. IN-8/IN-9/IN-10 exercise `claim_slot_file()`'s own claim-vs-content atomicity added for BUG-407: IN-8 and IN-9 are direct-correctness/regression guards (both pass identically pre-fix and post-fix in the absence of an injected crash — no red/green transition is claimed or required for either); IN-10 is a Documented Residual category (distinct from Invariant Hold) — it does not assert the invariant holds for pre-existing corruption, it asserts the KNOWN absence of that guarantee is stable across the fix, so a future change cannot silently narrow or widen the boundary unnoticed.
...
| IN-3 | `t17_orphaned_reclaim_ticket_does_not_permanently_block_slot` | `tests/concurrency_gate_ext_test.rs` | ✅ |
| IN-4 | `t18_gate_tries_other_free_index_when_count_derived_index_is_live_held` | `tests/concurrency_gate_ext_test.rs` | ✅ |
| IN-5 | `t21_ticket_winner_that_fails_own_admission_does_not_self_deny_forever` | `tests/concurrency_gate_ext_test.rs` | ✅ |
| IN-6 | `t22_acquire_slot_walks_multi_generation_reclaim_ticket_chain` | `tests/concurrency_gate_ext_test.rs` | ✅ |
| IN-7 | `t21_stale_alive_owner_becomes_reclaimable_when_threshold_set` | `tests/concurrency_gate_ext2_test.rs` | ✅ |
| IN-8 | `t22_claim_slot_file_content_valid_immediately_after_call` | `tests/concurrency_gate_ext2_test.rs` | ✅ |
| IN-9 | `t23_concurrent_racers_still_yield_exactly_one_winner` | `tests/concurrency_gate_ext2_test.rs` | ✅ |
| IN-10 | `t24_preexisting_empty_slot_file_remains_a_documented_residual` | `tests/concurrency_gate_ext2_test.rs` | ✅ |
...
| IN-13 | `t42_thread_tid_masked_dead_slot_owner_reclaimed_and_caller_admitted` | `tests/concurrency_gate_test.rs` | ✅ |
| IN-14 | `it_47_thread_tid_waiter_filtered_out_and_self_healed` (= IT-47 in the `ps` command spec) | `tests/ps_command_test.rs` | ✅ |
| IN-15 | `t43_mismatched_start_time_slot_owner_reclaimed_and_caller_admitted` | `tests/concurrency_gate_test.rs` | ✅ |
| IN-16 | `t44_legacy_record_without_start_time_live_leader_still_held` | `tests/concurrency_gate_test.rs` | ✅ |
...
### IN-13: Slot owned by a dead PID whose number is occupied by a live non-leader thread → owner reads as dead, slot reclaimed, caller admitted

- **Given:** a parked helper thread spawned inside the test process, its TID read via `/proc/thread-self` (live occupancy: `/proc/{tid}/stat` readable, state ∉ {`Z`}, but `/proc/{tid}/status` reports `Tgid != tid` — no process with that PID exists; all three properties fixture-verified before the run); `slot_0.json` pre-seeded recording that TID as owner in the legacy record shape (no start-time field — isolates the thread-group-leader clause); empty proc dir; `--max-sessions 1`; `CLR_GATE_STALE_SECS` unset
- **When:** a fresh `clr --print --max-sessions 1` invocation calls `acquire_slot()` against the thread-masked slot
- **Then:** the owner fails the thread-group-leader clause of the liveness check, the slot is reclaimed via the normal ticket-arbitrated handoff, and the caller is admitted promptly (exit 0) with zero `slot held by another session` denials on stderr
- **Note:** `bug_reproducer(BUG-488)` — pre-fix, the thread's readable stat masks the dead owner as alive and the caller exhausts its gate-wait budget (BUG-479's starvation consequence with no zombie involved). The gate-reclaim half; IN-14 covers the display half
- **Source:** [invariant/012_gate_slot_atomicity.md](../../../docs/invariant/012_gate_slot_atomicity.md) pid_alive contract clause (c); Provenance : BUG-488
...
### IN-15: Slot record carrying a start time that mismatches the live occupant of the same PID number → owner reads as dead, slot reclaimed

- **Given:** `slot_0.json` pre-seeded recording a genuinely alive leader PID (the test process itself — nextest runs each test in its own process, so it is a live leader for the test's whole duration) but a deliberately wrong start time (the occupant's real `/proc/{pid}/stat` field 22, plus one); empty proc dir; `--max-sessions 1`; `CLR_GATE_STALE_SECS` unset
- **When:** a fresh `clr --print --max-sessions 1` invocation calls `acquire_slot()` against the mismatched record
- **Then:** the occupant fails the same-incarnation clause — the record's writer is provably not the current occupant — the slot is reclaimed via the normal ticket-arbitrated handoff, and the caller is admitted promptly (exit 0)
- **Note:** `bug_reproducer(BUG-488)` — the full-PID-recycling half of the fix: clause (c) alone cannot catch a recycled *leader* PID, only the start-time binding can. Deterministic stand-in for a host PID wrap: fabricating the mismatch in the record is equivalent to the number having been recycled to a different process
- **Source:** [invariant/012_gate_slot_atomicity.md](../../../docs/invariant/012_gate_slot_atomicity.md) pid_alive contract clause (d); Provenance : BUG-488

---

### IN-16: Legacy slot record without a start-time field, owner a live thread-group leader → still denied `HeldByLive` (compatibility boundary)

- **Given:** `slot_0.json` pre-seeded in the pre-BUG-488 record shape — `{"pid":N,"since":N}`, no start-time field — recording a genuinely alive leader PID (the test process itself — nextest runs each test in its own process); empty proc dir; `--max-sessions 1`; `CLR_GATE_STALE_SECS` unset
- **When:** a fresh `clr --print --max-sessions 1` invocation calls `acquire_slot()` against the legacy-shaped record
- **Then:** the invocation is denied `HeldByLive` and exhausts its gate-wait budget (`"session gate timed out"` on stderr) — a live leader owner recorded by an older binary is NOT treated as dead merely because its record predates the incarnation binding
- **Note:** compatibility boundary of the incarnation binding — proves clause (d) is additive: records without the field keep current semantics until rewritten by a post-fix binary, so a mid-upgrade mixed fleet never mass-reclaims slots held by live sessions. The implementing test additionally pins the record plumbing post-fix: the denied caller exits via the exhaustion path (skipping the drop guard), and its surviving `{pid}.json` waiter file must carry the writer's start-time field
- **Source:** [invariant/012_gate_slot_atomicity.md](../../../docs/invariant/012_gate_slot_atomicity.md) pid_alive contract clause (d), "legacy records" sentence; Provenance : BUG-488
