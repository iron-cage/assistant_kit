...
| EC-03 | exactly 19 lines start with `"  clr "` (usage forms) | Section Split |
...
### EC-03: exactly 19 lines start with `"  clr "` (usage forms)

- **Command:** `clr --help`
- **Expected behavior:** splitting stdout by newline and counting lines that start with `"  clr "` (two-space indent) yields exactly 19 — one per command, less the slot `run` shares with the bare `clr <command>` form, plus `query`'s second PID-dispatch line
- **Exit:** 0
- **Source:** `tests/cli_args_ext_test.rs::ec03_help_has_nineteen_usage_forms`
