...
- **Command:** `clr ps --help`
- **Expected behavior:** Exit 0; stdout contains both the symbol and the name of all 9 session flags in canonical order — 👈 This session, 🖨 Print mode, 🔌 Query mode, ⚡ Active, 🕰 Ancient, 🐘 High RAM, 🧟 Odd state, ⚠ Dead metrics, 🐳 Container
- **Exit:** 0
...
- **Note:** guards the drift that shipped 🔌 in `FLAG_LEGEND` while `--help` listed only 7 flags — the legend rendered under the table and the legend `--help` promises must name the same set
