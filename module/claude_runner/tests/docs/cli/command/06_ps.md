...
| IT-26 | `clr ps --wide` shows all 11 columns including State, Mode, Command, Binary | Wide output |
...
| IT-47 | Thread-TID gate file (dead PID number occupied by live non-leader thread) filtered out + self-healed | BUG-488 repro |
| IT-48 | 🧟 flag for a SIGSTOPped session (kernel state `T`, not ⚠) | Flags — odd state |
| IT-49 | `clr ps --help` lists every session flag, symbol and name | Flags — help listing |
| IT-50 | 🆕 flag for a PID absent from the previous snapshot | Flags — new since last check |
| IT-51 | 🆕 absent when no prior snapshot exists (first invocation) | Flags — new since last check absent |
| IT-52 | Ended Since Last Check table for a session no longer running | Ended table — present |
| IT-53 | Ended Since Last Check table absent on first invocation | Ended table — absent |
| IT-54 | `CLR_PS_STATE_DIR` override is honored | Snapshot — state dir override |
| IT-55 | 🏠 flag for a PID the session daemon reports as hosted | Flags — daemon-hosted |
...
- BUG-479 repro: 1 test (IT-46)
- BUG-488 repro: 1 test (IT-47)
- Flags — odd state: 1 test (IT-48)
- Flags — help listing: 1 test (IT-49)
- Flags — new since last check: 1 test (IT-50)
- Flags — new since last check absent: 1 test (IT-51)
- Ended table — present: 1 test (IT-52)
- Ended table — absent: 1 test (IT-53)
- Snapshot — state dir override: 1 test (IT-54)
- Flags — daemon-hosted: 1 test (IT-55)
...
- **Setup:** temp dir created as fake `$PRO`; topic directory `my/project` created within it; fake `claude` ELF spawned with `current_dir = $PRO/my/project`; `PRO` set to temp dir path when running `clr ps`
...
- **Setup:** ≥1 fake `claude` process running
- **Command:** `clr ps --wide`
- **Expected behavior:** Exit 0; stdout contains `State`, `Mode`, `Command`, and `Binary` headers (plus the 7 default headers)
- **Exit:** 0
- **Source:** [command/06_ps.md](../../../../docs/cli/command/06_ps.md)
...
- **Setup:** Fake `claude` ELF spawned in a topic directory of `$HOME` (inside HOME, not ancient, not high-RAM, state not R, interactive mode, metrics readable); `CLR_PS_ANCIENT_SECS=999999` (prevent ancient false positive); `CLR_PS_HIGH_RAM_MB=999999` (prevent high-RAM false positive)
- **Command:** `clr ps`
...
### IT-46: Zombie-PID gate file filtered out + self-healed (BUG-479)

- **Setup:** temp dir used as `CLR_GATE_DIR`; spawn a real immediately-exiting child and deliberately never `wait()` on it (stays state `Z` with a live `/proc/{pid}` entry); write `{zombie_pid}.json` with valid gate JSON; empty `CLR_PROC_DIR`
- **Command:** `clr ps` (with `CLR_GATE_DIR=<temp_dir>` in env)
- **Expected behavior:** Exit 0; stdout does NOT contain `Queued` (zombie waiter not rendered); stdout does NOT contain the zombie PID; the waiter `.json` file is deleted by self-healing cleanup
- **Exit:** 0
- **Source:** [command/06_ps.md](../../../../docs/cli/command/06_ps.md) self-heal contract; `task/claude_runner/bug/479_zombie_blind_pid_liveness.md`
- **Note:** test_kind: bug_reproducer(BUG-479). IT-13's twin for the OTHER dead-PID state: IT-13 covers an absent PID, this covers exited-but-unreaped (state `Z`) — pre-fix, bare `/proc/{pid}` existence rendered zombies as live waiters and self-heal never fired

---

### IT-47: Thread-TID gate file filtered out + self-healed (BUG-488)

- **Setup:** temp dir used as `CLR_GATE_DIR`; spawn a helper thread inside the test process and park it (`spawn_parked_helper_thread()` in `tests/cli_binary_test_helpers.rs`); read the thread's TID via `/proc/thread-self` (a live non-leader thread ID: `/proc/{tid}/stat` is readable and state ∉ {`Z`}, but `/proc/{tid}/status` reports `Tgid != tid` and readdir of `/proc` never lists it — all three fixture-verified before the run); write `{tid}.json` with valid gate JSON; empty `CLR_PROC_DIR`
- **Command:** `clr ps` (with `CLR_GATE_DIR=<temp_dir>` in env)
- **Expected behavior:** Exit 0; stdout does NOT contain `Queued` (thread-masked waiter not rendered); stdout does NOT contain the TID; the waiter `.json` file is deleted by self-healing cleanup
- **Exit:** 0
- **Source:** [command/06_ps.md](../../../../docs/cli/command/06_ps.md) self-heal contract; [invariant/012](../../../../docs/invariant/012_gate_slot_atomicity.md) pid_alive contract clauses (c)+(d); `task/claude_runner/bug/488_pid_liveness_thread_id_blind.md`
- **Note:** test_kind: bug_reproducer(BUG-488). Completes the dead-PID triad — IT-13: PID absent; IT-46: PID present as zombie; IT-47: PID number present only as a live non-leader thread of another process (the record's owner is dead, the number is merely occupied). Spec added at BUG-488 filing; implemented as `it_47_thread_tid_waiter_filtered_out_and_self_healed` in `tests/ps_command_test.rs` (task 489). Pre-fix, `pid_alive()`'s bare stat-readability clause rendered the thread-masked waiter as a live `Queued` row and self-heal never fired (observed live: 76h phantom row masked by a dockerd startup thread; RED-phase run confirmed the exact symptom)

---

### IT-48: 🧟 Odd state flag for a stopped session

- **Setup:** spawn a fake `claude` process, `make_proc_dir(&[pid])` for discovery, then `kill -STOP <pid>` so it sits in kernel state `T`; assert `/proc/{pid}/stat` field 3 is `T`/`t` before running the command; `CLR_PS_ANCIENT_SECS=999999`, `CLR_PS_HIGH_RAM_MB=999999` suppress 🕰 and 🐘; teardown sends `kill -CONT` before reaping, since a stopped process does not act on SIGKILL until resumed
- **Command:** `clr ps`
- **Expected behavior:** Exit 0; stdout contains `🧟` and the legend entry `Odd state`; stdout does NOT contain `⚠` — a stopped process's `/proc` entries still parse, so `read_process_metrics()` returns `Some` and the ⚠ branch is never reached
- **Exit:** 0
- **Source:** [command/06_ps.md](../../../../docs/cli/command/06_ps.md) Session Flags; [user_story/026](../../../../docs/cli/user_story/026_session_listing.md) AC-032
- **Note:** a zombie (`Z`) is deliberately NOT the fixture even though it matches 🧟's condition — the kernel clears `cmdline` on exit and `find_claude_processes()` keeps only processes whose argv[0] basename is `claude`, so a zombie is dropped at discovery and never reaches flag computation. A first version of this test used a killed-but-unreaped child and failed with `No active Claude Code sessions.`, which is what established the constraint

---

### IT-49: `ps --help` lists every session flag

- **Setup:** none (pure help output)
- **Command:** `clr ps --help`
- **Expected behavior:** Exit 0; stdout contains both the symbol and the name of all 11 session flags in canonical order — 👈 This session, 🏠 Daemon-hosted, 🆕 New since last check, 🖨 Print mode, 🔌 Query mode, ⚡ Active, 🕰 Ancient, 🐘 High RAM, 🧟 Odd state, ⚠ Dead metrics, 🐳 Container
- **Exit:** 0
- **Source:** [command/06_ps.md](../../../../docs/cli/command/06_ps.md) Session Flags; [user_story/026](../../../../docs/cli/user_story/026_session_listing.md) AC-031
- **Note:** guards the drift that shipped 🔌 in `FLAG_LEGEND` while `--help` listed only 7 flags — the legend rendered under the table and the legend `--help` promises must name the same set. The `expected` array in `ps_flags_test.rs` is the literal source of truth this test checks against; it must be updated by hand every time `FLAG_LEGEND` gains or loses an entry — nothing enforces the two staying in sync except this test failing when they drift

---

### IT-50: 🆕 flag for a PID absent from the previous snapshot

- **Setup:** isolated `CLR_PS_STATE_DIR`; run 1 with an empty `CLR_PROC_DIR` (0 live sessions) establishes an empty prior snapshot; run 2 spawns a fake `claude` ELF and points `CLR_PROC_DIR` at just that PID; `CLR_PS_ANCIENT_SECS=999999`, `CLR_PS_HIGH_RAM_MB=999999` on run 2 suppress 🕰/🐘
- **Command:** `clr ps` (run 1, empty proc set) then `clr ps` (run 2, one live session) — both against the same `CLR_PS_STATE_DIR`
- **Expected behavior:** Run 2 exit 0; stdout contains `🆕`; legend lists `New since last check`
- **Exit:** 0
- **Source:** [command/06_ps.md](../../../../docs/cli/command/06_ps.md) § Session Snapshot & Temporal Diff; [user_story/026](../../../../docs/cli/user_story/026_session_listing.md) AC-034

---

### IT-51: 🆕 absent when no prior snapshot exists (first invocation)

- **Setup:** freshly created, never-before-used `CLR_PS_STATE_DIR`; fake `claude` ELF spawned and visible via `CLR_PROC_DIR`; `CLR_PS_ANCIENT_SECS=999999`, `CLR_PS_HIGH_RAM_MB=999999`
- **Command:** `clr ps` (single invocation — the very first against this state dir)
- **Expected behavior:** Exit 0; stdout does NOT contain `🆕` — there is no prior snapshot to compare against yet
- **Exit:** 0
- **Source:** [command/06_ps.md](../../../../docs/cli/command/06_ps.md) § Session Snapshot & Temporal Diff "First invocation" clause; [user_story/026](../../../../docs/cli/user_story/026_session_listing.md) AC-034

---

### IT-52: Ended Since Last Check table for a session no longer running

- **Setup:** isolated `CLR_PS_STATE_DIR`; run 1 spawns a fake `claude` ELF and captures it in the snapshot; the session is then killed and `wait()`-reaped (fully gone, not a zombie); run 2 uses an empty `CLR_PROC_DIR`
- **Command:** `clr ps` (run 1, one live session) then `clr ps` (run 2, 0 live sessions) — both against the same `CLR_PS_STATE_DIR`
- **Expected behavior:** Run 2 exit 0; stdout contains `Ended Since Last Check`; stdout contains the ended session's PID
- **Exit:** 0
- **Source:** [command/06_ps.md](../../../../docs/cli/command/06_ps.md) § Session Snapshot & Temporal Diff; [user_story/026](../../../../docs/cli/user_story/026_session_listing.md) AC-035

---

### IT-53: Ended Since Last Check table absent on first invocation

- **Setup:** freshly created, never-before-used `CLR_PS_STATE_DIR`; empty `CLR_PROC_DIR`
- **Command:** `clr ps` (single invocation)
- **Expected behavior:** Exit 0; stdout does NOT contain `Ended Since Last Check` — there is no prior snapshot to diff against
- **Exit:** 0
- **Source:** [command/06_ps.md](../../../../docs/cli/command/06_ps.md) § Session Snapshot & Temporal Diff "First invocation" clause; [user_story/026](../../../../docs/cli/user_story/026_session_listing.md) AC-035

---

### IT-54: `CLR_PS_STATE_DIR` override is honored

- **Setup:** two distinct temp dirs (`target_dir`, `sibling_dir`); empty `CLR_PROC_DIR`
- **Command:** `clr ps` with `CLR_PS_STATE_DIR=<target_dir>`
- **Expected behavior:** Exit 0; `<target_dir>/last_snapshot.json` exists on disk after the run; `<sibling_dir>/last_snapshot.json` does NOT exist
- **Exit:** 0
- **Source:** [command/06_ps.md](../../../../docs/cli/command/06_ps.md) § Session Snapshot & Temporal Diff "State file location"

---

### IT-55: 🏠 flag for a PID the session daemon reports as hosted

- **Setup:** a real `claude_daemon_core::Daemon` bound to a temp socket at the same path `DaemonPaths::with_home(temp_home)` resolves to, served on a background thread (same technique as `claude_daemon_core/tests/serve_test.rs`'s `Harness`); a `Spawn` request against it produces one real hosted child (`cat`) with a known pid; `clr ps` is run with `HOME=<temp_home>` and `CLR_PROC_DIR` isolated to that same pid so `find_claude_processes()` reports it as a `claude`-shaped row
- **Command:** `clr ps` (with `HOME=<temp_home>` in env, pointing `hosted_pids()`'s `DaemonPaths::new()` lookup at the test daemon's socket)
- **Expected behavior:** Exit 0; stdout contains `🏠`; legend lists `Daemon-hosted`
- **Exit:** 0
- **Source:** [command/06_ps.md](../../../../docs/cli/command/06_ps.md) Session Flags; [user_story/026](../../../../docs/cli/user_story/026_session_listing.md) AC-036
- **Note:** the negative case — no daemon running — is already covered incidentally by IT-30 and US-18: both set `HOME` to a fresh temp dir with no daemon behind it and assert the `Flags` column is absent entirely, which would fail if `hosted_pids()` ever fired spuriously. No dedicated negative test is needed since an absent socket and a socket with no matching pid are the same code path inside `hosted_pids()` (§ `claude_runner/src/cli/daemon.rs`) — both fall through to an empty `HashSet`
