...
- **Note:** Boundary/regression coverage for the same print-mode-only counting behavior — gate triggers at exactly the limit and not below it — is additionally covered by `t01_gate_triggers_at_eight_print_mode_processes` and `t02_gate_does_not_trigger_below_eight_print_mode_processes`; `t06_max_sessions_zero_disables_gate_regardless_of_count` regression-guards US-1 against the print-mode-only counting change (all three in `concurrency_gate_test.rs`)
