...
| US-3 | Explicit `--subdir` bypasses slug generation | AC-002 | ✅ |
...
- **Given:** cwd has no `-investigate-the-flaky` subdirectory yet
...
- **Given:** a subdirectory matching the message's slug already exists (from a prior real invocation)
...
### US-3: Explicit `--subdir` bypasses slug generation

- **Given:** clean cwd
- **When:** `clr topic --dry-run --subdir auth-refactor "Start refactoring the auth module"`
- **Then:** dry-run output includes a path ending in `/-auth-refactor` exactly (no counter suffix, not derived from message text)
- **Exit:** 0
- **Verifies:** AC-002
- **Implemented by:** `topic_command_test.rs::us3_explicit_subdir_bypasses_slug_generation`

---

### US-4: First use of a topic name clones the source session

- **Given:** cwd has a qualifying `.jsonl` session file; `-fresh-topic` subdirectory does not yet exist
- **When:** `clr topic --dry-run --subdir fresh-topic "Start this"`
- **Then:** dry-run output includes a `# session-transplant:` plan line copying cwd's session into the new subdirectory's storage
- **Exit:** 0
- **Verifies:** AC-003
...
- **Given:** `-fresh-topic` subdirectory already has its own session file from a prior real (non-dry-run) `clr topic --subdir fresh-topic` call
- **When:** `clr topic --dry-run --subdir fresh-topic "Continue this"`
