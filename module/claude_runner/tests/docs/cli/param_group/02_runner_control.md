...
`--new-session`, `--dir`, `--topic`, `--max-tokens`, `--session-dir`, `--dry-run`, `--quiet`,
...
`--journal`, `--journal-dir`, `--args-file`, `--from`, `--no-stdin`, `--global`,
`--topic-mode`, `--keep-clone`. Tests validate these fifty-five flags
