...
|----|-----------|----------|
| DT-1 | Commands section lists all 11 subcommands | Completeness |
...
### DT-1: Commands section lists all 11 subcommands

- **Given:** `docs/cli/001_dictionary.md` Commands section
- **When:** inspect entries in the Commands table
- **Then:** exactly 11 entries present: `run`, `ask`, `isolated`, `refresh`, `ps`, `kill`, `tools`, `scope`, `query`, `topic`, `help`; no entry missing; no extra entries; each name matches the corresponding file under `docs/cli/command/`
