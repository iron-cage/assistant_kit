...
| EC-2 | `--retry-on-rate-limit 0 --dry-run` → exit 0; default no-retry behavior | Behavioral Divergence |
...
| EC-6 | `CLR_RETRY_ON_RATE_LIMIT=notanumber --dry-run` → silently ignored; default 0 | Validation |
...
### EC-2: --retry-on-rate-limit 0 --dry-run → exit 0; default no-retry

- **Given:** `--retry-on-rate-limit 0` and `--dry-run` set
- **When:** `clr --retry-on-rate-limit 0 --dry-run "task"`
- **Then:** Exit 0; dry-run output produced; no retry messages on stderr. **Divergence from EC-3:** value 0 is the default and disables all retry logic at configuration level; value 3 (EC-3) activates the retry wrapper code path (though in dry-run it still fires no subprocess)
- **Exit:** 0
- **Source:** [034_retry_on_rate_limit.md](../../../../docs/cli/param/034_retry_on_rate_limit.md)
- **Commands:** run, ask
...
### EC-6: CLR_RETRY_ON_RATE_LIMIT=invalid → silently ignored; default 0
