...
| EC-10 | Retry stderr line contains `result` text, NOT raw JSON blob; in summary mode `first_message()` extracts `"result"` field | Output Quality |
| EC-11 | Exhaustion stderr contains rendered `---` separator + key:val lines (render_summary applied to stdout); no raw JSON dump | Output Quality |
...
- Output Quality: 2 tests (EC-10, EC-11)
