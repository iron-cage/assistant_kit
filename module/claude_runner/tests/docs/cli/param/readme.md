...
| [090_pick.md](090_pick.md) | Edge cases for `--pick` (delegate selection policy: value, guard, and the `broadcast` rejection) | ✅ |
| [091_seed.md](091_seed.md) | Edge cases for `--seed` (reproducible draw: stability and spread, which pull opposite ways) | ✅ |
| [092_concurrency.md](092_concurrency.md) | Edge cases for `--concurrency` (the clamp on `broadcast` and `pool`; the `delegate` rejection) | ✅ |
| [093_count.md](093_count.md) | Edge cases for `--count` (target not increment — four cases, one per wrong implementation) | ✅ |
| [094_prefix.md](094_prefix.md) | Edge cases for `--prefix` (independent pools, the three rejection guards, and the round-trip rule) | ✅ |
