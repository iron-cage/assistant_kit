...
| E29 | `CLR_TOPIC=NAME` names the topic (fork mode for a fresh name) | `CLR_TOPIC` | dry-run preview contains `topic=NAME` |
...
### E29: CLR_TOPIC=NAME names the topic

- **Given:** `CLR_TOPIC=feature`; no `--topic` on CLI; no pre-existing `-feature` dir (fresh name → fork mode)
- **When:** `clr --dry-run task`
- **Then:** dry-run preview line contains `topic=feature ` (fork-mode session plan)
- **Exit:** 0
- **CLI-wins:** `clr --dry-run --topic build task` with `CLR_TOPIC=debug` → preview shows `topic=build `, NOT `topic=debug`
- **Source:** [003_env_param.md §1](../../../../docs/cli/003_env_param.md)
