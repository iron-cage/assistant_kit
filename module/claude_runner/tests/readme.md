...
| Concurrency gate (T01–T04, T06) | `concurrency_gate_test.rs` | `--max-sessions` gate end-to-end via synthetic proc-dir process simulation: triggers at the default/explicit limit, does not trigger below it, interactive invocations bypass entirely (zero wait), print-mode-only counting excludes interactive processes, `--max-sessions 0` disables the gate |
...
| `concurrency_gate_test.rs` | `--max-sessions` concurrency gate end-to-end: trigger at limit, no-trigger below limit, interactive bypass with zero wait (AF1), print-mode-only counting excludes interactive processes, `--max-sessions 0` disable (T01–T04, T06). |
