...
| Concurrency gate extended: override-tier resolution (T35–T36, T39–T41, `t_gate_*`) | `concurrency_gate_ext3_test.rs` | Extension of `concurrency_gate_test.rs`: `CLR_REMAINING_TIMEOUT_SECS` budget clamp, deadline-resolution diagnostic announcement and boundary inputs (T39–T40, BUG-481), poll-secs-zero/numeric-budget divide-by-zero guard (T41, BUG-481), and the CLI-flag/env-var/JSON-key/precedence/default matrix for `gate-poll-secs`, `gate-max-attempts`, and `gate-stale-secs` |
...
| `concurrency_gate_ext3_test.rs` | Extension of `concurrency_gate_test.rs`: `CLR_REMAINING_TIMEOUT_SECS` budget clamp (T35/T36, BUG-423), deadline-resolution diagnostic announcement and boundary-input handling (T39/T40, BUG-481), poll-secs-zero/numeric-budget divide-by-zero guard (T41, BUG-481), and the CLI-flag/env-var/JSON-key/precedence/default matrix for `gate-poll-secs`, `gate-max-attempts`, and `gate-stale-secs` (`t_gate_*`). |
