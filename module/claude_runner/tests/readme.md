...
| Bug reproducer BUG-008 | `bug_reproducers_008_test.rs` | `clp .model.select` pin honored by `dispatch_run()`; pref absent → no injection; CLI flag wins |
...
| `bug_reproducers_008_test.rs` | Bug reproducer BUG-008: `dispatch_run()` injects pinned model from `~/.clr/prefs.json`; absent pref → no injection; CLI flag wins. |
