...
| Bug reproducer BUG-539 | `bug_reproducers_539_test.rs` | Interactive journal events carry `duration_ms` (feature 002 AC-012) at both completion sites — blocking and timeout-polling |
| Journal attribution (EC-23–EC-29) | `journal_attribution_test.rs` | Task 542 attribution fields on journal events: `dir` cwd fallback vs explicit `--dir`, `agent_id` composition, `user`/`host` pinning, `CLR_ACCOUNT` override → store marker → absent hierarchy, uniform stamping across `retry`/`execution`/`interactive` |
...
| Fork-mode topics (F01–F17) | `topic_fork_test.rs` | Cache-preserving `--topic` fork mode: deterministic UUIDv5 session args (`--session-id` / `--resume --fork-session` / plain `--resume`), mode selection precedence (`--topic-mode`, `CLR_TOPIC_MODE`, existing `-<name>` dir), fork+`--from`/`--global` contradictions, `--new-session` semantics, dry-run purity, registry recording, `topics --file` parity with `claude_storage .session.path` |
| `clr topic` subcommand (T01–T11) | `topic_command_test.rs` | Auto-named `run`/`ask` alias: auto-generated `--topic` slug from message + `-2`/`-3` disambiguation, explicit `--topic` byte-identical to `ask`, unknown-flag/help handling, `--effort` passthrough, real (non-dry-run) session clone-then-continue transplant; BUG-541/542/543 reproducers (source-drift continuity, storage-aware freshness, symlink-safe canonical probe) |
...
| `bug_reproducers_539_test.rs` | Bug reproducer BUG-539: interactive journal events carry `duration_ms` (feature 002 AC-012) at both the blocking and timeout-polling completion sites. |
| `journal_attribution_test.rs` | Task 542 journal attribution (EC-23–EC-29): `dir` cwd fallback, explicit `--dir` preserved, `agent_id` composition, account hierarchy (`CLR_ACCOUNT` → active marker → absent), uniform stamping across event types. |
...
| `topic_fork_test.rs` | Fork-mode topics: first-use fresh create / fork-from-latest / repeat-resume argv shapes with the deterministic UUIDv5 session id (F01–F03), legacy `-<name>` dir coexistence and mode-selection precedence incl. `CLR_TOPIC_MODE` env and the explicit-fork-overrides-dir escape hatch (F04–F05, F10–F11), fork+`--from`/`--global` contradiction errors (F06–F07), `--new-session` repeat-error vs fresh-source-suppression split (F08–F09), dry-run registry/dir purity (F12), real-run argv passthrough + registry recording via a stubbed `claude` (F13, unix only), print-gating consistency (F14), `topics --file` core-rule parity and guards (F15–F16), merged fork/dir listing with MODE column (F17). |
| `topic_command_test.rs` | `clr topic` subcommand (task 521): auto-generated `--topic` slug from message with `-2`/`-3` disambiguation counter (T01–T02), explicit `--topic` parity with `ask` (T03), unknown-flag/help handling (T06–T07), `--effort` passthrough (T08), real non-dry-run session clone-then-continue transplant via a stubbed `claude` binary (T04–T05, unix only); reproducers `bug_reproducer(BUG-541)` T09 (repeat topic continues its own history despite cwd source drift), `bug_reproducer(BUG-542)` T10 (auto-name skips candidate with orphaned session storage), `bug_reproducer(BUG-543)` T11 (freshness probe reaches the canonical storage key through a symlinked base). |
| `config_file_test.rs` | Config-file param tier: `~/.clr/config.toml` and project-level `.clr.toml` — the 4th of 5 CLI param precedence levels. |
| `input_format_test.rs` | `--input-format` SDK protocol routing: IT-1–IT-6 (task 414; unix only). |
| `query_command_test.rs` | `clr query` PID-addressed control-session dispatch: QT-1–QT-32 (task 418). Note the `QT-` prefix is reused by `quiet_test.rs` for its own unrelated QT-1–QT-6; the two ranges are not the same series. |
| `fixtures/` | Test fixture sources compiled as helper binaries — `fake_claude_control.rs` control-session stub. |
