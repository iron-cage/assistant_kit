...
| Concurrency gate (T01–T04, T06–T11, T13–T14) | `concurrency_gate_test.rs` | `--max-sessions` gate end-to-end via synthetic proc-dir process simulation: triggers at the default/explicit limit, does not trigger below it, interactive bypass (zero wait), print-mode-only counting, `--max-sessions 0` disables the gate, gate-state-file JSON escaping (T07/T13), N-concurrent-racer TOCTOU close (T08, BUG-387), poll/max-attempts env var overrides (T09–T11), dead-owner-slot reclaim race (T14, BUG-392) |
...
| Concurrency gate extended: override-tier resolution (T35–T36, `t_gate_*`) | `concurrency_gate_ext3_test.rs` | Extension of `concurrency_gate_test.rs`: `CLR_REMAINING_TIMEOUT_SECS` budget clamp, and the CLI-flag/env-var/JSON-key/precedence/default matrix for `gate-poll-secs`, `gate-max-attempts`, and `gate-stale-secs` |
...
| `--from` param edge cases (EC-1–EC-12, US-1–US-8) | `session_from_test.rs` | `-c` injection from source session, default-to-cwd, empty source, `--session-dir` precedence, `--new-session` suppression, `--to` combo, `CLR_FROM` env var (+ old `CLR_SESSION_FROM` inert), relative path resolution, JSON config key, dry-run WYSIWYG, cross-loading user stories including bare-invocation no-op |
| Session path resolution feature (FT-6–FT-10) | `session_path_resolution_test.rs` | `clr scope` 6-var format, `--from` session resumption, `--to`+`--from` combo, `--to`/`--dir` alias equivalence, `--session-dir` precedence over `--from` |
...
| `concurrency_gate_ext3_test.rs` | Extension of `concurrency_gate_test.rs`: `CLR_REMAINING_TIMEOUT_SECS` budget clamp (T35/T36, BUG-423), and the CLI-flag/env-var/JSON-key/precedence/default matrix for `gate-poll-secs`, `gate-max-attempts`, and `gate-stale-secs` (`t_gate_*`). |
...
| `session_from_test.rs` | `--from` parameter edge cases: EC-1–EC-12 (injection, rejected old flag name, empty source, precedence, suppression, `--to` combo, env var + old-env-var rejection, relative path, JSON config key, WYSIWYG) + US-1–US-8 (session transplant user stories, including default-to-cwd and bare-invocation no-op). |
| `session_path_resolution_test.rs` | Feature 005 session path resolution: FT-6–FT-10 (scope output format, `--from` resumption, `--to` combo, `--to`/`--dir` alias, `--session-dir` precedence). |
