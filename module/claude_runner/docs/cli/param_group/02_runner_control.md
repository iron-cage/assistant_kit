...
"Is this parameter consumed by the runner, not Claude?" — YES for all 56 (55 CLI flags + 1 env-var-only).
...
|---|---------|------------|-----------------|-------|
| 1 | [`run`](../command/01_run.md) | Full | — | All 56 params apply; default command |
| 5 | [`ask`](../command/05_ask.md) | Full | — | All 56 params apply; identical behavior — pure alias for run |
| 11 | [`topic`](../command/11_topic.md) | Full | — | All 56 params apply; `--topic`'s default overridden with an auto-generated slug — see [`command_group/01_run_ask.md`](../command_group/01_run_ask.md) Default Divergence Table |
...
| [`--keep-clone`](../param/089_keep_clone.md) | bool | false | Session cross-load | Preserve an existing non-empty destination session copy on a `--from` transplant (mtime refresh only) instead of the default overwrite-with-fresh-copy |
...
| [`--from`](../param/076_from.md) | [`DirectoryPath`](../type/02_directory_path.md) | cwd | Session source | Compute session storage for this dir and use it for `-c` injection; defaults to cwd (self-copy guard suppresses no-op transplants) |
