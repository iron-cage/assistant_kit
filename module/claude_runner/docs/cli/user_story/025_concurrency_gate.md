...
- **In Scope**: `--max-sessions` limit, 30s polling interval, 50-attempt limit, `0` = unlimited semantics, `CLR_MAX_SESSIONS` env var, stderr status messages, dry-run bypass.
...
- AC-003: When 50 attempts are exhausted without a slot opening, `clr` emits an error message to stderr and exits with code 1
