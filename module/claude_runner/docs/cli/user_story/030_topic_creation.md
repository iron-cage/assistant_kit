...
- **AC-001 (Auto-named topic):** `clr topic "message"` (no `--topic`) generates a concise slug from `message`, disambiguates it against candidate topic names of the effective `--dir` via a numeric counter suffix, and uses it as `--topic`'s value — always a fresh, previously-unused name. A candidate is rejected (counter advances) when EITHER its working directory already exists on disk OR its session storage already holds a qualifying session (Fix(BUG-542)) — the second check catches a topic directory that was deleted and never recreated while its storage survived, since storage lives under `~/.claude/projects/` independent of the working directory's own filesystem lifetime.
