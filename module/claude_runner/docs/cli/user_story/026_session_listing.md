...
- **In Scope**: `clr ps` unicode-box table output, `#`/`PID`/`Started`/`CPU%`/`RAM`/`State`/`Absolute Path`/`Task` columns, no-sessions message, `clr --help` listing, typo-guard for `clr p` / `clr pss`, self-PID exclusion.
