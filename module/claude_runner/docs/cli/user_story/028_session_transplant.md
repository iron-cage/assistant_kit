...
- **AC-6 (Deprecated `--session-dir` inert):** When both `--from` and `--session-dir` are given, `--from`'s computed source storage governs exactly as if `--session-dir` were absent — the deprecated parameter is fully inert and only warns on stderr (Fix(BUG-493)).
