...
- **AC-4 (First-use default to CWD):** `clr run --to /project-b "message"` (no `--from`) against a target with NO qualifying session of its own defaults the session source to CWD — Claude runs in `/project-b`, loading the most recent session from the directory `clr` was invoked in. Equivalent to explicitly passing `--from <cwd>` for that first use only; repeat use is governed by AC-9 (Fix(BUG-541)).
...
- **AC-9 (Repeat use continues the target's own history):** `clr run --to /project-b "message"` (no `--from`) against a target that ALREADY has a qualifying session of its own uses that target's storage as the source — the run continues `/project-b`'s own established history, never re-deriving from CWD's possibly-since-changed most-recent session. Source and target storage coincide, so the self-copy guard suppresses any transplant. Applies uniformly to `--dir`/`--to` and `--topic` targets. (Fix(BUG-541); reproducer: `tests/topic_command_test.rs::transplant::t09_second_auto_topic_call_ignores_unrelated_source_session_drift`)
