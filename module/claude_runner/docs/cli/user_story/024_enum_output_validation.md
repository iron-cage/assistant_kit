...
- **Purpose**: Document `--expect` / `--expect-strategy` / `--expect-retries` as runner-native
...
- **In Scope**: `--expect "val1|val2|..."` case-insensitive matching, `fail`/`retry`/`default:<V>`
  strategies, retry re-invocation semantics, exit code 3, `CLR_EXPECT*` env vars.
...
- `--expect-strategy retry --expect-retries 2` re-invokes independently up to 2 additional times
...
- `CLR_EXPECT`, `CLR_EXPECT_STRATEGY`, `CLR_EXPECT_RETRIES` env vars apply when the corresponding
  CLI flags are absent
- Invalid strategy string (e.g., `--expect-strategy bogus`) causes `clr` to exit 1 at parse time
- `--expect-retries` value > 255 causes `clr` to exit 1 at parse time
...
|---|-----------------|------|
| 2 | [Runner Control](../param_group/02_runner_control.md) | All three `--expect-*` params are Runner Control members |
...
| 32 | [`--expect-retries`](../param/032_expect_retries.md) | Re-invocation cap for `retry` strategy |
...
| 18 | [Env-var Configuration](018_env_var_configuration.md) | `CLR_EXPECT*` vars are instances of the CLR_* env var system |
