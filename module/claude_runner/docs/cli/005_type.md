...
| 3 | `TokenLimit` | u32 | [`--max-tokens`](param/009_max_tokens.md) | Maximum output token count |
| 4 | `ModelName` | String | [`--model`](param/003_model.md) | Claude model identifier string |
| 5 | `VerbosityLevel` | u8 | [`--verbosity`](param/012_verbosity.md) | Runner diagnostic output gate (0–5) |
...
| 7 | `EffortLevel` | enum | [`--effort`](param/017_effort.md) | Reasoning effort level forwarded to the `claude` subprocess |
| 8 | `CredentialsFilePath` | String | [`--creds`](param/019_creds.md) | Path to an existing credentials JSON file |
| 9 | `TimeoutSecs` | u64 | [`--timeout`](param/020_timeout.md) | Subprocess wait limit in seconds |
...
- [`run`](001_command.md#command--1-run) (via `[MESSAGE]`)
- [`isolated`](001_command.md#command--2-isolated) (via `[MESSAGE]`)
---
...
- [`run`](001_command.md#command--1-run) (via `--dir`, `--session-dir`)
---
...
- [`run`](001_command.md#command--1-run) (via `--max-tokens`)
---
...
- [`run`](001_command.md#command--1-run) (via `--model`)
---
...
- [`run`](001_command.md#command--1-run) (via `--verbosity`)
---
...
- [`run`](001_command.md#command--1-run) (via `--system-prompt`, `--append-system-prompt`)
---
...
- [`run`](001_command.md#command--1-run) (via `--effort`)
---
...
- [`isolated`](001_command.md#command--2-isolated) (via `--creds`)
---
...
- [`isolated`](001_command.md#command--2-isolated) (via `--timeout`)
---
...
- [`run`](001_command.md#command--1-run) (via `--json-schema`)
---
...
- [`run`](001_command.md#command--1-run) (via `--mcp-config`)
---
...
- [`run`](001_command.md#command--1-run) (via `--file`)