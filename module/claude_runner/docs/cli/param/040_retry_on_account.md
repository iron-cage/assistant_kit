...
clr -p "task" --retry-on-account 0                 # disable retry for Account class
CLR_RETRY_ON_ACCOUNT=2 clr -p "task"               # env-var equivalent
```

**Note:** Default is `auto` — inherits from `--retry-default` (Tier 3 fallback,
default 2). Set to `0` to explicitly disable retry for Account regardless of
...
**Note:** Retrying a quota-exhausted error is useful for long-running tasks where
the usage period may reset between retry attempts. For immediate account
switching, see the deferred `--on-quota-exhausted` parameter in
`type/14_error_class.md`.
