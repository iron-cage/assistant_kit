...
|---|-------|------------|------------|
| 1 | [Claude-Native Flags](../param_group/01_claude_native_flags.md) | Full | `--print`, `--model`, `--verbose`, `--effort`, `--json-schema`, `--mcp-config`, `--output-format`, `--input-format`, `--max-turns`, `--allowed-tools`, `--disallowed-tools`, `--add-dir`, `--fallback-model` |
...
| 3 | [`isolated`](../command/03_isolated.md) | — | Forwarded to claude subprocess; env: `CLR_MAX_BUDGET_USD` |
