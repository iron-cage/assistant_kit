...
| `AuthError` | any nonzero + text match | `"authentication_error"`, `"Your organization does not have access to Claude"`, `"Not logged in"`, or `"Please run /login"` in stdout/stderr | fix credentials |
...
4. Scan stdout + stderr for `"Not logged in"` → `AuthError`
5. Scan stdout + stderr for `"Please run /login"` → `AuthError`
6. Scan stdout + stderr for `"API Error: "` → `ApiError`
7. Exit code == 2 → `RateLimit`
8. Exit code > 128 → `Signal`
9. Default → `Unknown`
...
The `"authentication_error"` pattern (step 3) catches the Claude CLI 401 form: `"Failed to authenticate. API Error: 401 {\"type\":\"authentication_error\",...}"` — it must appear before the `"API Error: "` catch-all at step 6.
The `"Not logged in"` (step 4) and `"Please run /login"` (step 5) patterns catch credential-absent or credential-expired messages emitted by claude; without them, classify_error() falls through to `Unknown`, suppressing the `--retry-on-auth` path (TSK-453).
