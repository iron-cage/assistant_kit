...
| I7 | Model was hardcoded string for isolated | isolated | ✅ Changed to `"opus"` alias (`ISOLATED_DEFAULT_MODEL`); `"sonnet"` alias added as `REFRESH_DEFAULT_MODEL` for refresh (S7) |
...
| S7 | `ISOLATED_DEFAULT_MODEL` → `"opus"` alias; add `REFRESH_DEFAULT_MODEL` = `"sonnet"` alias | `isolated.rs` constants | ✅ `ISOLATED_DEFAULT_MODEL = "opus"`, `REFRESH_DEFAULT_MODEL = "sonnet"` |
