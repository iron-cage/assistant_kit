...
-- **Parameters:** `--creds`, `--timeout`, `--trace`, `--dry-run`, `--no-compact-window`, `--journal`, `--journal-dir`
...
- `--model "sonnet"` alias (`REFRESH_DEFAULT_MODEL` — Sonnet is sufficient for a trivial ping)
