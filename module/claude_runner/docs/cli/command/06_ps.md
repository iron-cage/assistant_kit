...
| 59 | [`--columns`](../param/059_columns.md) | string | 8 default cols | Comma-separated list of column keys to display |
...
1. **Active Sessions** — caption: `Active Sessions · N running`; default columns:
   ...
   Optional columns (hidden by default): `Mode`, `Command`, `Binary`. Use
...
| 1 | Error (invalid `--mode` value, unknown `--columns` key, or unexpected I/O failure) |
...
# Show all 11 columns including mode, command, and binary
clr ps --wide
...
| # | Group | Params | Notes |
|---|-------|--------|-------|
| 5 | [Session Listing](../param_group/05_session_listing.md) | `--mode`, `--columns`, `--wide` | All 3 params exclusive to `ps` |
