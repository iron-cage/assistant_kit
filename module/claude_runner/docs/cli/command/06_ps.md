...
caption rule line (e.g., `─── Active Sessions · 2 running (1 interactive, 1 print) ──────`)
that names the table and shows the item count. The column header row follows the caption.

1. **Active Sessions** — caption: `Active Sessions · N running (I interactive, P print)`
   when `--mode` is `all` (the default), where `I` and `P` are counts of the active
   rows classified as interactive and print respectively (`I + P == N`; see §
   Mode Detection for the per-row classification rule). When `--mode` is
   `interactive` or `print`, every row already shares one mode, so the breakdown
