...
Two plain-style tables printed consecutively:

1. **Active Sessions** — columns: `#`, `PID`, `Elapsed`, `CPU%`, `RAM`, `State`,
   `Absolute Path`, `Task`.
2. **Queued CLR Processes** — columns: `#`, `PID`, `CWD`, `Waiting`, `Attempt`.
   Shows `clr` processes currently blocked in `wait_for_session_slot()`.
