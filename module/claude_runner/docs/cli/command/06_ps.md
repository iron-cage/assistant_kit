...
3. Read `$CLR_GATE_DIR` for gate state files; probe PID liveness (zombie-aware: `/proc/{pid}/stat` state ∉ {`Z`}, not bare existence); delete orphaned files.
...
`build_queued_table()` checks liveness for every gate file's PID before rendering a row, via the shared zombie-aware `gate::pid_alive()` predicate: `/proc/{pid}/stat` must be readable AND its state field must not be `Z`. Gate files whose PID is gone **or exited-but-unreaped (zombie)** are filtered out and deleted (self-healing cleanup) — this prevents orphaned gate files from SIGKILL, crash, or `exit(1)`-under-a-non-reaping-supervisor appearing as phantom waiters.
