...
| topic (concept) | A named, isolated line of work with its own working directory and its own session storage. The canonical term for this concept — never "subdir". For the two commands of the same name see [Commands](#commands) above |
| topic name | The bare name identifying a topic (`auth-refactor`). A single directory-name component — a value containing `/` is rejected. Carried by `--topic` / `CLR_TOPIC` / JSON `"topic"` |
| topic directory | The on-disk directory backing a topic: the topic name prefixed with `-` (topic name `auth-refactor` → directory `-auth-refactor`), placed under the base. The `-` prefix is what makes it a topic directory and what `clr topics` recognizes when listing; a plain directory of the same name is not a topic |
| topic session | A Claude session that runs inside a topic directory. Distinct from the directory itself — a topic directory can exist with zero topic sessions, which is what a `SESSIONS` count of `0` in `clr topics` output means |
| base | The directory that topic directories are created and listed under. Resolved by precedence, highest first: `--dir <PATH>`, then `--global`, then the current working directory. Single source of truth: `topic_path::topic_base()` |
