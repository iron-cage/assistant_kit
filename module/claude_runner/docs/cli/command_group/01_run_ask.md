...
| 1 | `topic` | `--topic` | `.` (identity — no topic directory appended) | Auto-generated slug from `MESSAGE` (lowercase, hyphenated, truncated, disambiguated via counter suffix on collision) | The entire point of the command: each topic gets its own session-isolated topic directory without the operator having to name one explicitly; explicit `--topic NAME` still overrides |
