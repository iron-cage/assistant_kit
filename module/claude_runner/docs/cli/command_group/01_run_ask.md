...
`dispatch_run()` (`src/cli/mod.rs:331`). `dispatch_ask()` (`src/cli/mod.rs:487`) intercepts only `--help`/`-h`/bare `help` to print ask-specific help text, then calls `dispatch_run(&tokens[1..])` directly — the identical function, not a duplicate. `dispatch_topic()` (`src/cli/topic.rs:93`) intercepts `--help`/`-h`/bare `help` to print topic-specific help text; when `--topic` is absent from the token stream, it computes an auto-generated slug (see [`11_topic.md`](../command/11_topic.md) Algorithm) and injects `--topic <slug>` before calling `dispatch_run(&tokens[1..])` — otherwise forwarding tokens unchanged. Clone-vs-continue behavior requires no `topic`-specific session logic: in dir mode it falls entirely out of the pre-existing `execute_session_transplant()` mechanism (`src/cli/builder.rs:165`) once `--topic` resolves to a session-isolated directory — the first invocation of a new topic clones (no same-named session file exists yet at the destination), a later invocation of the same topic continues (a diverged file already exists there). In fork mode (the default for new topics) the equivalent first-use/repeat-use split lives in `plan_topic_fork()` (`src/cli/builder.rs`), shared identically by all three command names — the topic's deterministic session file either exists (resume) or not (fork from the base's most recent session); see [`param/028_topic.md`](../param/028_topic.md) § Mode selection.
...
| 1 | `topic` | `--topic` | `.` (identity — no topic) | Auto-generated slug from `MESSAGE` (lowercase, hyphenated, truncated, disambiguated via counter suffix on collision) | The entire point of the command: each topic gets its own isolated conversation (fork-mode session, or dir-mode topic directory) without the operator having to name one explicitly; explicit `--topic NAME` still overrides |

### Invariants

1. Every parameter accepted by `run` is accepted by `ask` and `topic` with an identical default — save `topic`'s one stated `--topic` divergence — enforced structurally by `dispatch_ask`/`dispatch_topic` delegating to `dispatch_run`, not by parallel maintenance of separate parameter tables.
2. The only observable difference between `clr ask --help` and `clr run --help` is the help text body. `clr topic --help` differs in help text body AND in `--topic`'s documented default.
3. A future parameter added to `run` (Claude-Native Flags, Runner Control, or System Prompt groups) is automatically available under `ask` and `topic` with no additional wiring — see `command/readme.md`'s maintenance note: "`ask` inherits all `run` params automatically... no separate table update needed for `05_ask.md`." The same inheritance applies to `topic`, minus the one already-documented `--topic` divergence.
