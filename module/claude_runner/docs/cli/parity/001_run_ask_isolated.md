...
| Default model | config-file `model` if set; otherwise claude binary default | config-file `model` if set; otherwise claude binary default | config-file `model` if set; otherwise `opus` alias (`ISOLATED_DEFAULT_MODEL`) |
