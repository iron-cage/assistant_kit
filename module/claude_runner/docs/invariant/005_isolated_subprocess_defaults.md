...
|----------|----------|---------|-----------|
| Model | `"claude-opus-4-8"` (`ISOLATED_DEFAULT_MODEL`) | `"claude-sonnet-5"` (`REFRESH_DEFAULT_MODEL`) | Isolated runs real user tasks requiring maximum capability; refresh executes a trivial `"."` ping to trigger OAuth token exchange only |
...
- Model: prepended via `IsolatedModel::Default` → `ISOLATED_DEFAULT_MODEL = "claude-opus-4-8"` for isolated; `IsolatedModel::Specific(REFRESH_DEFAULT_MODEL)` for refresh where `REFRESH_DEFAULT_MODEL = "claude-sonnet-5"`
...
- Isolated user tasks run on `"sonnet"`/`"claude-sonnet-5"` instead of `"claude-opus-4-8"` (model reverted)
