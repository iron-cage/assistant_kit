...
| A losing reservation attempt finds the existing owner alive, but `CLR_GATE_STALE_SECS` is set and the owner's recorded claim time exceeds it | Owner is treated as reclaim-eligible exactly like a dead owner — falls through into the same ticket-arbitrated atomic handoff, no separate mechanism (see Provenance : BUG-400). Unset (default): this row never applies — behavior is identical to the two rows above |

<!-- BUG-400 — fixed: a third owner category (alive-but-stale) now exists via the opt-in CLR_GATE_STALE_SECS threshold — see the table row above and Enforcement Mechanism : 3, "Staleness-based reclaim", and Provenance : BUG-400 -->
...
```
**Staleness-based reclaim (Fix(BUG-400)):** the reclaim-eligibility test above (`pid_alive( owner ) && !is_stale`) treats a live owner as reclaim-eligible once its recorded claim time also exceeds an opt-in staleness threshold. `gate_stale_secs()` reads `CLR_GATE_STALE_SECS`, returning `None` when unset or unparseable — in that default case `is_stale` is always `false` and behavior is identical to the pre-fix `pid_alive( owner )` test alone. When the threshold is set and `owner_since` exceeds it, execution falls through into the exact same ticket-arbitrated reclaim loop described below (Fix(BUG-392)/(BUG-402)/(BUG-405)) that a dead owner already uses — no separate mechanism, no new `SlotDenialCause` variant: staleness only changes which branch is taken at this one `if`, never how the reclaim itself is arbitrated. This closes the gap where a live-but-stalled (hung/deadlocked/SIGSTOPped) holder blocked every waiter indefinitely even with capacity available elsewhere, since the waiter's candidate index is deterministically re-derived from the same live count each poll, making the collision sticky rather than a one-off; see Provenance : BUG-400.
...
| `../../tests/concurrency_gate_test.rs` | T20: pre-seeds a slot owned by a genuinely alive PID with `since=0` (maximally stale); asserts `CLR_GATE_STALE_SECS` unset denies reclaim (Phase A, backward-compatible default) and set-below-elapsed-duration admits the waiter immediately (Phase B) (BUG-400) |
...
| BUG-400 | Fixed: added an opt-in `CLR_GATE_STALE_SECS` threshold (unset by default, preserving prior behavior exactly) — see Enforcement Mechanism : 3 above, "Staleness-based reclaim". A live owner whose `owner_since` exceeds the threshold now falls through into the same ticket-arbitrated reclaim path a dead owner already uses; no new `SlotDenialCause` variant was needed since staleness only changes which branch is taken, not the arbitration mechanism itself. |
