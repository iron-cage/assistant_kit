...
- **In Scope**: Automatic `-c` injection, `--dangerously-skip-permissions` default-on, `--new-session` override, `--no-skip-permissions` opt-out.
...
The flag injection is implemented in `build_claude_command()` (the function that translates `CliArgs` to a `ClaudeCommand` builder). The flags are added unconditionally unless the corresponding opt-out flag is present in `CliArgs`.
