...
| `topic` | `claude` binary | yes | env vars + assembled `claude` command line (identical to `run`/`ask` — delegates to `run`'s handler; `--topic`'s default never appears in the traced line itself) |
