...
**Command syntax:** `clr` uses bare-word subcommands (`run`, `isolated`, `refresh`, `help`, `ps`, `kill`, `tools`, `ask`, `scope`, `query`) followed by `--flag value` pairs and an optional positional message. This mirrors POSIX CLI convention (like `git`) rather than a unilang-style `.command param::value` syntax. `topic` is a planned additional bare-word subcommand — see [command/11_topic.md](../cli/command/11_topic.md); dispatch wiring is pending implementation (task 521).
