...
The existing TOML config tier ([config_param.md](../cli/config_param.md)) works for `run`/`ask`/`topic` but has three gaps this proposal closes:
...
2. **Scope limited to `run`/`ask`/`topic`.** The current TOML config is not read by `isolated`, `refresh`, or `ps`. Defaults for these subcommands can only be set via `CLR_*` env vars. A global config should be global — covering all subcommands.
...
`timeout` and `journal`/`journal_dir` already appear in the TOML eligible set for `run`/`ask`/`topic`; the YAML global config extends their applicability to `isolated` and `refresh` via the `isolated:` and `refresh:` subcommand sections.
