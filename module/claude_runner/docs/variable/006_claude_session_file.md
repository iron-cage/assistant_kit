...
`CLAUDE_SESSION_FILE` is purely read-only output from `scope_for()`. To inject a specific session, use `--from <DIR>` (directory whose session is computed via `scope_for()`; defaults to cwd when omitted) — `--session-dir` is deprecated and inert (BUG-493) and no longer has any effect.
