...
|-----------|-------|
| Env var | none |
| `--session-dir` param | Deprecated and inert (BUG-493) — no longer overrides CLAUDE_SESSION_DIR derivation; emits a deprecation warning only |
| `--from <DIR>` param | Computes `scope_for(DIR).claude_session_dir` and uses that for session lookup; defaults to cwd when omitted |

**`--from` vs `--session-dir`:**
- `--session-dir /path/to/dir` — deprecated and inert (BUG-493); claude ≥2.x ignores the override, so it no longer affects CLAUDE_SESSION_DIR derivation. Emits a deprecation warning naming the value.
- `--from /home/alice/project` — computes `Df("/home/alice/project")` → uses `~/.claude/projects/-home-alice-project` as the source session dir. The only mechanism that still works for cross-loading; defaults to cwd when omitted.
...
| [`../cli/param/010_session_dir.md`](../cli/param/010_session_dir.md) | `--session-dir` — deprecated, inert raw override (BUG-493) |
