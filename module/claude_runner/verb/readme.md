...
**In Scope:** Canonical verbs (`build`, `test`, `test_only`, `lint`, `run`, `clean`, `verify`, `install`), layer dispatchers (`*.d/`), and meta verbs (`verbs`, `package_info`).
...
| `test_only` | Dispatcher: run a single test or group by nextest filter; delegates via `NEXTEST_FILTER`. |
