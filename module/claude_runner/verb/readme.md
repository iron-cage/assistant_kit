...
**Responsibilities:** Shell scripts implementing the `do` protocol verbs for `claude_runner` (cargo ecosystem).
**In Scope:** Canonical verbs (`build`, `test`, `test1`, `lint`, `run`, `clean`, `verify`, `install`), layer dispatchers (`*.d/`), and meta verbs (`verbs`, `package_info`).
...
| `test1` | Dispatcher: run single test by nextest filter inside container; no level-3 overhead. |
| `test1.d/` | Layer directory: `l1` (container-internal targeted nextest via `VERB_FILTER`). |
