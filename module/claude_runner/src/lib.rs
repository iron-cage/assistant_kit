...
// gate_max_attempts_from/gate_poll_secs_from/gate_stale_secs_from are used by
// tests/gate_unit_test.rs — same rationale as above.
#[ cfg( feature = "enabled" ) ]
#[ doc( hidden ) ]
#[ allow( unused_imports ) ]
pub use cli::{ gate_max_attempts_from, gate_poll_secs_from, gate_stale_secs_from };
...
pub fn run_cli()
{
  use cli::{
    ...
    dispatch_pool, dispatch_delegate, dispatch_broadcast,
  ...
  match tokens.first().map( String::as_str )
  {
    ...
    Some( "pool" )     => dispatch_pool( &tokens ),
    Some( "delegate" ) => dispatch_delegate( &tokens ),
    Some( "broadcast" ) => dispatch_broadcast( &tokens ),
