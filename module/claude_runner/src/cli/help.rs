...
fn runner_option_group() -> cli_fmt::help::OptionGroup
{
  ...
  OptionGroup
  {
    ...
    [
      ...
      OptionEntry { name : "--session-dir <PATH>".into(),            desc : "DEPRECATED, no effect (claude >= 2.x ignores it; use --from)".into() },
...
pub( crate ) fn print_ask_help() -> !
{
  ...
  println!( "  --session-dir <PATH>               DEPRECATED, no effect (claude >= 2.x ignores it; use --from)" );
