...
pub( crate ) fn print_ps_help() -> !
{
  ...
  println!( "  state      State          Kernel process state letter (R/S/D/Z/T…); abnormal values also raise 🧟" );
  ...
  println!( "  mode       Mode           Execution mode: query, print, or interactive; the first two also raise 🔌/🖨" );
  ...
  println!();
  println!( "DEFAULT COLUMNS: idx, pid, elapsed, cpu, ram, path, task" );
  println!( "  (state and mode are carried by the Flags column below — add them back with --columns or --wide)" );
  println!();
  ...
  println!( "  🧟  Odd state       Kernel state is neither R nor S (D uninterruptible, T/t stopped or traced)" );
