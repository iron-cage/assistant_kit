...
Key design decisions made during the claude_manager CLI implementation.
...
canonical syntax for all claude_manager invocations.
...
`src/adapter.rs` layer handles `claude_manager`-specific concerns: `v::` alias
...
formatting across all 15 commands is a further benefit. The custom `adapter.rs` layer
retains full control over `claude_manager`-specific normalisation without forking unilang.
