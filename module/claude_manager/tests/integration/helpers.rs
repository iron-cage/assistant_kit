//! Shared helpers for all `claude_manager` integration tests.
//!
//! Provides subprocess execution helpers so every integration test can invoke
//! the compiled `claude_manager` binary without boilerplate.
//!
//! # Binary Name Coupling
//!
//! The compile-time macro `env!("CARGO_BIN_EXE_claude_manager")` inside
//! `run_clm_with_env` is tightly coupled to `[[bin]] name = "claude_manager"`
...
) -> std::process::Output
{
  ...
  cmd.output().expect( "failed to execute claude_manager binary" )
}
