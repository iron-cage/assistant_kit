...
**Responsibilities:** Shell scripts implementing the `do` protocol verbs for `json_redact` (cargo ecosystem).
...
| File | Responsibility |
|------|---------------|
| `build` | Compile the crate: `cargo build -p json_redact`. |
| `clean` | Remove build artifacts: `cargo clean -p json_redact`. |
