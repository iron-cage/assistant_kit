...
| 4 | [`.search`](../command/04_search.md) | Literal substring search across prompt and captured output |
...
| AC-04 | `clj .search pattern::"error"` searches subprocess output — `stdout`/`stderr` are read unconditionally, with no flag to enable them |
| AC-08 | `clj .search pattern::"<prompt phrase>"` finds the run by what it was asked to do; `message` is written at every journal level, so this is the one search that still works at `meta` |
...
# Deep: search for specific error in output (stdout/stderr are always searched)
clj .search pattern::"connection refused" since::7d
...
| 14 | [`pattern`](../param/14_pattern.md) | Search for error messages — matched literally against `message`, `stdout`, `stderr`, `error_message`, `model`, and `command` |
