...
| 9 | `.chart` | 1 | `cmd_chart()` (`src/cli_main.rs:211`) | N/A — sole member |
...
**Verification method:** every `cmd_*` dispatch function defined in `src/cli_main.rs` — `cmd_list()` (line 89), `cmd_tail()` (line 99), `cmd_stats()` (line 115), `cmd_search()` (line 125), `cmd_prune()` (line 135), `cmd_status()` (line 145), `cmd_export()` (line 151), `cmd_serve()` (line 161), `cmd_chart()` (line 211) — was checked with a full pairwise grep sweep of `src/` for every call site of each function name (excluding its own `fn` definition line):
...
The sweep result: each `cmd_*` function has **exactly one call site**, the `match command { ... }` dispatch block in `main()` (`src/cli_main.rs:274-285`) — no `cmd_*` function calls another, and no `cmd_*` function is called from anywhere outside `main()`. This is the fact backing "no two of the 9 documented commands share a dispatch/handler function," and therefore backing all 9 rows above being Singleton Groups — it is a swept, grepped result, not informal reasoning.

One layer deeper, each `cmd_*` function calls a distinct implementing function — `list_output()` (`src/output.rs:191`), `stats_output()` (`src/output.rs:235`), `search_output()` (`src/output.rs:333`), `prune_output()` (`src/output.rs:400`), `status_output()` (`src/output.rs:372`), `export_output()` (`src/output.rs:514`), `chart_output()` (`src/output.rs:537`) — each swept the same way and confirmed to have no second `cmd_*` caller. Six of them have exactly 2 call sites crate-wide (their own `cmd_*` caller in `src/cli_main.rs`, plus a `*_routine()` caller in `src/routines.rs` under the optional `routines` feature — see the `ast .journal.*` rows below); `chart_output()` has exactly 1, since `src/routines.rs` exposes no `chart_routine()` and `.chart` therefore has no `ast .journal.*` counterpart. `cmd_tail()` and `cmd_serve()` use their own inline logic in `src/cli_main.rs` rather than an `output::` function. No two of the 9 documented commands share a dispatch/handler function at either layer, so none can qualify for a multi-member group regardless of parameter overlap — each therefore opens its own Singleton Group above.

### Evaluated, Not Qualifying

| Candidate Pair | Shared Implementation | Why Not a Command Group |
|-----------------|------------------------|---------------------------|
| `clj .list` / `ast .journal.list` | `output::list_output()` (`src/output.rs:191`) — called by both `cmd_list()` in `src/cli_main.rs:89` and `list_routine()` in `src/routines.rs:50` | `list_routine` is not a documented command under `command/` — it is part of the `routines` feature (`Cargo.toml`: `routines = ["dep:unilang", "unilang/enabled"]`), an optional, non-default super-app integration surface with no doc instance of its own. Out of this entity's scope regardless of the shared handler. |
| `clj .stats` / `ast .journal.stats` | `output::stats_output()` (`src/output.rs:235`) — `cmd_stats()` (`src/cli_main.rs:115`) and `stats_routine()` (`src/routines.rs:66`) | Same reason as above — `stats_routine` is not a documented `command/` entry. |
| `clj .search` / `ast .journal.search` | `output::search_output()` (`src/output.rs:333`) — `cmd_search()` (`src/cli_main.rs:125`) and `search_routine()` (`src/routines.rs:82`) | Same reason as above. |
| `clj .status` / `ast .journal.status` | `output::status_output()` (`src/output.rs:372`) — `cmd_status()` (`src/cli_main.rs:145`) and `status_routine()` (`src/routines.rs:98`) | Same reason as above. |
| `clj .export` / `ast .journal.export` | `output::export_output()` (`src/output.rs:514`) — `cmd_export()` (`src/cli_main.rs:151`) and `export_routine()` (`src/routines.rs:112`) | Same reason as above. |
| `clj .tail` / `ast .journal.tail` | None — `tail_routine()` (`src/routines.rs:146`) is a stub returning a fixed guidance string; it never calls `build_filter()` (`src/output.rs:93`, invoked from `cmd_tail()` at `src/cli_main.rs:101`) or `JournalReader::tail()` (`../claude_journal/src/reader.rs:116`, invoked from `cmd_tail()` at `src/cli_main.rs:108`) | Fails on two independent grounds: not a documented `command/` entry, and (even setting that aside) genuinely different behavior — `cmd_tail()` (`src/cli_main.rs:99`) performs the real tail, `tail_routine()` does not. |
| `clj .serve` / `ast .journal.serve` | None — `serve_routine()` (`src/routines.rs:163`) is a stub returning a fixed guidance string; it never binds an HTTP server | Same two-fold rejection as `.tail` above — undocumented surface plus genuinely different behavior. `cmd_serve()` (`src/cli_main.rs:161`) is the only caller of `tiny_http::Server::http()` (`src/cli_main.rs:169`) in the crate. |
