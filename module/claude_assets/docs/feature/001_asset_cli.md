...
**Registry aggregation:** `register_commands()` is exported from `lib.rs` and called by `assistant::build_registry()` to include all four commands in the `ast` super-app without running a separate process.
