...
| `attachment/` | Harness context-injection payloads — 24 instances, one per `attachment.type` (23 from the 2026-08-27 scan, plus 1 gap found via independent spot-check; the second dispatch level) |
...
**Total**: **486 instances** across 19 collections — 485 as of the 2026-08-27 snapshot (484 files matching `NNN_*.md`, plus `behavior/016h_b16h_tools_system_prompt.md`, whose suffixed number marks it a sub-instance of B16 rather than a numbered peer), plus 1 added afterward via independent local evidence rather than the original scan ([`attachment/024_budget_usd.md`](attachment/024_budget_usd.md) — see [`attachment/readme.md`](attachment/readme.md)). Two files sit outside that count: `endpoint/account_field_index.md` (an unnumbered cross-endpoint field index, not an endpoint instance) and this collection's own master index [001_entity.md](001_entity.md), which carries the authoritative per-collection breakdown.
...
find . -mindepth 2 -name '*.md' ! -name 'readme.md' | wc -l   # → 487 = 486 instances + 1 aux index
```
