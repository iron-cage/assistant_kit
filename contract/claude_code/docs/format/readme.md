...
- **Responsibility**: Master file for the `formats` collection — lists all 7 format instances covering history, credentials, debug logs, shell snapshots, tasks, command definitions, and JSON response output.
- **In Scope**: history.jsonl entry structure, .credentials.json structure, debug log format, shell snapshot format, task JSON format, command definition markdown format, `--output-format json` response schema.
...
| [005](005_task.md) | Task | `~/.claude/tasks/{session-uuid}/{n}.json` — one JSON object per task, plus `.lock`; supersedes the `todos/` array format |
...
| `tasks/{uuid}/{n}.json` | ~200B–2KB | Full parse | Rewrite one task | Task tracking |
