...
- **Purpose**: Records the changelog for Claude Code release 2.1.83.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.83.
- **In Scope**: All changelog entries published for version 2.1.83.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `managed-settings.d/` drop-in directory alongside `managed-settings.json`, letting separate teams deploy independent policy fragments that me...
