...
- **Purpose**: Records the changelog for Claude Code release 2.1.132.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.132.
- **In Scope**: All changelog entries published for version 2.1.132.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `CLAUDE_CODE_SESSION_ID` environment variable to the Bash tool subprocess environment, matching the `session_id...
