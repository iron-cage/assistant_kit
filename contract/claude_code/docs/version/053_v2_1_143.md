...
- **Purpose**: Records the changelog for Claude Code release 2.1.143.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.143.
- **In Scope**: All changelog entries published for version 2.1.143.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added plugin dependency enforcement: `claude plugin disable` now refuses when another enabled plugin depends on the t...
