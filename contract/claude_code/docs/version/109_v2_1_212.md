...
- **Purpose**: Records the changelog for Claude Code release 2.1.212.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.212.
- **In Scope**: All changelog entries published for version 2.1.212.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

`/fork` now copies your conversation into a new background session with its own row in `claude agents` while you keep working, and the in-session subagent it used to launch is now `/subtask`.
