...
- **Purpose**: Records the changelog for Claude Code release 2.1.214.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.214.
- **In Scope**: All changelog entries published for version 2.1.214.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Single-segment `dir/**` allow rules like `Edit(src/**)` no longer auto-approve writes to nested `dir/` directories anywhere in the tree — matching is now scoped to `<cwd>/dir` only.
