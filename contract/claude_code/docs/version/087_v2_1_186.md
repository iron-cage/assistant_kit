...
- **Purpose**: Records the changelog for Claude Code release 2.1.186.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.186.
- **In Scope**: All changelog entries published for version 2.1.186.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `claude mcp login ` and `claude mcp logout ` to authenticate MCP servers from the CLI without opening the inter...
