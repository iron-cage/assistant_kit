...
- **Purpose**: Records the changelog for Claude Code release 2.1.217.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.217.
- **In Scope**: All changelog entries published for version 2.1.217.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

The prompt input now supports emoji shortcode autocomplete — typing `:heart:` inserts an emoji, and partial codes like `:hea` show suggestions — configurable via the `emojiCompletionEnabled` setting.
