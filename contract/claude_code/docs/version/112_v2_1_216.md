...
- **Purpose**: Records the changelog for Claude Code release 2.1.216.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.216.
- **In Scope**: All changelog entries published for version 2.1.216.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

A new `sandbox.filesystem.disabled` setting skips filesystem isolation while keeping network egress control.
