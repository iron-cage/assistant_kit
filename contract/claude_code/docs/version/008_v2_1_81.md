...
- **Purpose**: Records the changelog for Claude Code release 2.1.81.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.81.
- **In Scope**: All changelog entries published for version 2.1.81.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `--bare` flag for scripted `-p` calls — skips hooks, LSP, plugin sync, and skill directory walks; requires `ANTHROPIC_API_KEY` or an `apiKeyH...
