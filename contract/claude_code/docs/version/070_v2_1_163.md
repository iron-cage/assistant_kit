...
- **Purpose**: Records the changelog for Claude Code release 2.1.163.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.163.
- **In Scope**: All changelog entries published for version 2.1.163.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `requiredMinimumVersion` and `requiredMaximumVersion` managed settings — Claude Code refuses to start if its ve...
