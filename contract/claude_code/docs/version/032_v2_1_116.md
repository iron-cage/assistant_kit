...
- **Purpose**: Records the changelog for Claude Code release 2.1.116.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.116.
- **In Scope**: All changelog entries published for version 2.1.116.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

`/resume` on large sessions is significantly faster (up to 67% on 40MB+ sessions) and handles sessions with many dead...
