...
- **Purpose**: Records the changelog for Claude Code release 2.1.161.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.161.
- **In Scope**: All changelog entries published for version 2.1.161.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

`OTEL_RESOURCE_ATTRIBUTES` values are now included as labels on metric datapoints, so you can slice usage metrics by ...
