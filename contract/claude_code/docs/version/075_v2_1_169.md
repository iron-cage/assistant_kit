...
- **Purpose**: Records the changelog for Claude Code release 2.1.169.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.169.
- **In Scope**: All changelog entries published for version 2.1.169.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Self-hosted runner: added a `post-session` lifecycle hook that runs after the session ends and before the workspace i...
