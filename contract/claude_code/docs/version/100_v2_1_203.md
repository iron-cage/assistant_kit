...
- **Purpose**: Records the changelog for Claude Code release 2.1.203.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.203.
- **In Scope**: All changelog entries published for version 2.1.203.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Claude Code now warns when your login is about to expire, so you can re-authenticate before background sessions are interrupted.
