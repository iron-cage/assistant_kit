...
- **Purpose**: Records the changelog for Claude Code release 2.1.110.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.110.
- **In Scope**: All changelog entries published for version 2.1.110.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `/tui` command and `tui` setting — run `/tui fullscreen` to switch to flicker-free rendering in the same conver...
