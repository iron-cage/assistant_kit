...
- **Purpose**: Records the changelog for Claude Code release 2.1.200.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.200.
- **In Scope**: All changelog entries published for version 2.1.200.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Default permission mode changes to "Manual" across the CLI and IDE integrations, `AskUserQuestion` dialogs no longer auto-continue by default, and numerous background-session and daemon reliability fixes land.
