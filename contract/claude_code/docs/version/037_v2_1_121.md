...
- **Purpose**: Records the changelog for Claude Code release 2.1.121.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.121.
- **In Scope**: All changelog entries published for version 2.1.121.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `alwaysLoad` option to MCP server config — when `true`, all tools from that server skip tool-search deferral an...
