...
- **Purpose**: Records the changelog for Claude Code release 2.1.166.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.166.
- **In Scope**: All changelog entries published for version 2.1.166.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `fallbackModel` setting to configure up to three fallback models tried in order when the primary model is overl...
