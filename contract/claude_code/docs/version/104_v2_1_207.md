...
- **Purpose**: Records the changelog for Claude Code release 2.1.207.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.207.
- **In Scope**: All changelog entries published for version 2.1.207.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Auto mode is now available by default on Bedrock, Vertex AI, and Foundry without the `CLAUDE_CODE_ENABLE_AUTO_MODE` opt-in, and can be disabled via `disableAutoMode` in settings.
