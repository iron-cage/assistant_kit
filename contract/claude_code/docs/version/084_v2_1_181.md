...
- **Purpose**: Records the changelog for Claude Code release 2.1.181.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.181.
- **In Scope**: All changelog entries published for version 2.1.181.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `/config key=value` syntax to set any setting from the prompt (e.g. `/config thinking=false`) — works in intera...
