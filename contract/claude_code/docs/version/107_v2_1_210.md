...
- **Purpose**: Records the changelog for Claude Code release 2.1.210.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.210.
- **In Scope**: All changelog entries published for version 2.1.210.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

A live elapsed-time counter on the collapsed tool summary line now shows long-running tool calls ticking instead of appearing stuck.
