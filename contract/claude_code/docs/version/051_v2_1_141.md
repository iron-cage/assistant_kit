...
- **Purpose**: Records the changelog for Claude Code release 2.1.141.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.141.
- **In Scope**: All changelog entries published for version 2.1.141.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `terminalSequence` field to hook JSON output so hooks can emit desktop notifications, window titles, and bells ...
