...
- **Purpose**: Records the changelog for Claude Code release 2.1.144.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.144.
- **In Scope**: All changelog entries published for version 2.1.144.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `/resume` support for background sessions — sessions started via `claude --bg` or agent view now appear alongsi...
