...
- **Purpose**: Records the changelog for Claude Code release 2.1.198.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.198.
- **In Scope**: All changelog entries published for version 2.1.198.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Subagents now run in the background by default, Claude in Chrome reaches general availability, background agent notifications arrive in `claude agents`, and a new `/dataviz` skill ships for chart and dashboard design.
