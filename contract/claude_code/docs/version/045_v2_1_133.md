...
- **Purpose**: Records the changelog for Claude Code release 2.1.133.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.133.
- **In Scope**: All changelog entries published for version 2.1.133.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `worktree.baseRef` setting (`fresh` | `head`) to choose whether `--worktree`, `EnterWorktree`, and agent-isolat...
