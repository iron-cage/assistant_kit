...
- **Purpose**: Records the changelog for Claude Code release 2.1.183.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.183.
- **In Scope**: All changelog entries published for version 2.1.183.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Improved auto mode safety: destructive git commands (`git reset --hard`, `git checkout -- .`, `git clean -fd`, `git s...
