...
- **Purpose**: Records the changelog for Claude Code release 2.1.126.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.126.
- **In Scope**: All changelog entries published for version 2.1.126.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

The `/model` picker now lists models from your gateway's `/v1/models` endpoint when `ANTHROPIC_BASE_URL` points at an...
