...
- **Purpose**: Records the changelog for Claude Code release 2.1.113.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.113.
- **In Scope**: All changelog entries published for version 2.1.113.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Changed the CLI to spawn a native Claude Code binary (via a per-platform optional dependency) instead of bundled Java...
