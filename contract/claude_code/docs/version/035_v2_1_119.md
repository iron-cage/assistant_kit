...
- **Purpose**: Records the changelog for Claude Code release 2.1.119.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.119.
- **In Scope**: All changelog entries published for version 2.1.119.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

`/config` settings (theme, editor mode, verbose, etc.) now persist to `~/.claude/settings.json` and participate in pr...
