...
- **Purpose**: Records the changelog for Claude Code release 2.1.142.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.142.
- **In Scope**: All changelog entries published for version 2.1.142.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added new `claude agents` flags: `--add-dir`, `--settings`, `--mcp-config`, `--plugin-dir`, `--permission-mode`, `--m...
