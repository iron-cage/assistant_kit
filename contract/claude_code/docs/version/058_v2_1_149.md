...
- **Purpose**: Records the changelog for Claude Code release 2.1.149.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.149.
- **In Scope**: All changelog entries published for version 2.1.149.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

`/usage` now shows a per-category breakdown of what's driving your limits usage — skills, subagents, plugins, and per...
