...
- **Purpose**: Records the changelog for Claude Code release 2.1.199.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.199.
- **In Scope**: All changelog entries published for version 2.1.199.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Stacked slash-skill invocations now load up to 5 leading skills, alongside fixes for SSL certificate errors, discarded streaming responses, and background-agent daemon stability on Linux and macOS.
