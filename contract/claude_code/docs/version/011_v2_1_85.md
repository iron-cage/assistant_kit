...
- **Purpose**: Records the changelog for Claude Code release 2.1.85.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.85.
- **In Scope**: All changelog entries published for version 2.1.85.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `CLAUDE_CODE_MCP_SERVER_NAME` and `CLAUDE_CODE_MCP_SERVER_URL` environment variables to MCP `headersHelper` scripts, allowing one helper to s...
