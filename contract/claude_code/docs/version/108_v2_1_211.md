...
- **Purpose**: Records the changelog for Claude Code release 2.1.211.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.211.
- **In Scope**: All changelog entries published for version 2.1.211.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

A new `--forward-subagent-text` flag and `CLAUDE_CODE_FORWARD_SUBAGENT_TEXT` environment variable include subagent text and thinking in stream-json output.
