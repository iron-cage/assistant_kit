...
- **Purpose**: Records the changelog for Claude Code release 2.1.218.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.218.
- **In Scope**: All changelog entries published for version 2.1.218.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

`/code-review` now runs as a background subagent, so review work no longer fills your conversation, and it keeps stacked slash commands as its review target.
