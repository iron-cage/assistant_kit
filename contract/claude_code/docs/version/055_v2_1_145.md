...
- **Purpose**: Records the changelog for Claude Code release 2.1.145.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.145.
- **In Scope**: All changelog entries published for version 2.1.145.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `claude agents --json` to list live Claude sessions as JSON for scripting (tmux-resurrect, status bars, session...
