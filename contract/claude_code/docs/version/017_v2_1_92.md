...
- **Purpose**: Records the changelog for Claude Code release 2.1.92.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.92.
- **In Scope**: All changelog entries published for version 2.1.92.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `forceRemoteSettingsRefresh` policy setting: when set, the CLI blocks startup until remote managed settings are freshly fetched, and exits if...
