...
- **Purpose**: Records the changelog for Claude Code release 2.1.122.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.122.
- **In Scope**: All changelog entries published for version 2.1.122.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `ANTHROPIC_BEDROCK_SERVICE_TIER` environment variable to select a Bedrock service tier (`default`, `flex`, or `...
