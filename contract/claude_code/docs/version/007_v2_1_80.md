...
- **Purpose**: Records the changelog for Claude Code release 2.1.80.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.80.
- **In Scope**: All changelog entries published for version 2.1.80.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `rate_limits` field to statusline scripts for displaying Claude.ai rate limit usage (5-hour and 7-day windows with `used_percentage` and `res...
