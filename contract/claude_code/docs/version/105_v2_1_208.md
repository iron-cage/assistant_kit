...
- **Purpose**: Records the changelog for Claude Code release 2.1.208.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.208.
- **In Scope**: All changelog entries published for version 2.1.208.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

A new opt-in screen reader mode provides plain-text rendering for screen reader users, enabled via `claude --ax-screen-reader`, `CLAUDE_AX_SCREEN_READER=1`, or the `axScreenReader` setting.
