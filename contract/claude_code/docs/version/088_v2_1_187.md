...
- **Purpose**: Records the changelog for Claude Code release 2.1.187.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.187.
- **In Scope**: All changelog entries published for version 2.1.187.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `sandbox.credentials` setting to block sandboxed commands from reading credential files and secret environment ...
