...
- **Purpose**: Records the changelog for Claude Code release 2.1.89.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.89.
- **In Scope**: All changelog entries published for version 2.1.89.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Added `"defer"` permission decision to `PreToolUse` hooks — headless sessions can pause at a tool call and resume with `-p --resume` to have the ho...
