...
- **Purpose**: Records the changelog for Claude Code release 2.1.202.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.202.
- **In Scope**: All changelog entries published for version 2.1.202.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

A new "Dynamic workflow size" setting in `/config` controls how large Claude generally makes dynamic workflows, as an advisory guideline rather than an enforced cap.
