...
- **Purpose**: Records the changelog for Claude Code release 2.1.147.
- **Responsibility**: Documents all features, fixes, and changes introduced in version 2.1.147.
- **In Scope**: All changelog entries published for version 2.1.147.
- **Out of Scope**: Prior and subsequent releases (see sibling version doc instances).

### Abstract

Pinned background sessions (`Ctrl+T` in `claude agents`) now stay alive when idle, are restarted in place to apply Cl...
