...
Shell scripts implementing the `do` protocol verbs for `python` example (runbox/container ecosystem).

| File | Responsibility |
|------|----------------|
| `build` | Build the container image via `./run/runbox .build`. |
...
| `clean` | Remove container image and named `.venv` volume. |
...
| `verify` | Run offline tests via `./run/runbox .test.offline`. |
