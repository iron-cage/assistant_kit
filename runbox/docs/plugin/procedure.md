...
4. Add hook override to `runbox/plugins.sh` — read config via inherited `cfg`, override the stub
...
3. Create `NNN_{snake_case_name}.md` with Status `🔧`, Controls, Mechanism, and Notes
4. Create `runbox/docs/parameter/NNN_{param_name}.md` documenting the new `runbox.yml` key
5. Register in both `plugin/readme.md` and `parameter/readme.md` Overview Tables

## Update Plugin

1. Edit the target `NNN_*.md` file and, if hook logic changed, update `runbox/plugins.sh`
...
3. In `runbox/plugins.sh`: add `GH_PLUGIN="$(cfg gh_plugin)"` and extend `_plugin_test_args` to resolve `which gh` and add the bind-mount
