...
- **Status:** ✅ Configurable — `_plugin_list_cmd()` hook in `runbox/plugins.sh`
...
Default stub: `list_cmd="${cargo_env}cargo nextest list $CMD_SCOPE $CARGO_FEATURES"`. Override `_plugin_list_cmd()` in `runbox/plugins.sh` to swap the list command for a different test runner (e.g., `list_cmd="jest --listTests"` for Node). The hook receives `$cargo_env` from `_plugin_list_args()` via bash dynamic scope — the default stub incorporates any env prefix set by the active bin-plugin.

### Example

`./runbox/runbox .list` triggers `cmd_list()`. With `runbox/plugins.sh` active and `bin_plugin_volume: /tmp/will_test_targets`, `_plugin_list_args` injects `CARGO_TARGET_DIR` and a volume flag:
```bash
CARGO_TARGET_DIR=/tmp/will_test_targets \
  cargo nextest list --workspace --all-features
```
The `CARGO_TARGET_DIR` redirect routes nextest's artifact output into the `workspace_test_plugin_targets` volume, so repeated `.list` invocations reuse prior compilation. Without `runbox/plugins.sh`, cmd_list runs `cargo nextest list --workspace --all-features` with no volume or env override.
...
Override `_plugin_list_cmd()` in the project's `runbox/plugins.sh` for non-Rust ecosystems:
