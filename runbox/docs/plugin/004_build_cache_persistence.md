...
First `./runbox/runbox .test.offline` invocation (default `cache_dir: target`):
...
After `./runbox/runbox .build` (image rebuild):
- `cmd_build()` deletes `workspace_test_target` and any plugin-provided volumes (`workspace_test_plugin_targets` when `runbox/plugins.sh` is active, via `_plugin_build_volumes`)
