...
Library-only projects typically omit `run_script` entirely. Binary projects (Python modules, Node.js services, Rust binaries) set this to `verb/run` — a self-dispatching dispatcher that routes to `verb/run.d/l1` inside the container (via `VERB_LAYER=l1`) or runs `verb/run.d/l1` directly as the default when invoked on the host. `verb/run` has no knowledge of `runbox/runbox`.
