...
Standalone projects (e.g. `runbox/example/python/`) set this to `verb/lint` — a self-dispatching dispatcher that routes to `verb/lint.d/l1` inside the container (via `VERB_LAYER=l1`) or runs `verb/lint.d/l1` directly as the default when invoked on the host. `verb/lint` has no knowledge of `runbox/runbox`.
...
`verb/lint.d/l1` — container execution layer (ecosystem-specific; also the default host-side layer):
