...
Required only for standalone projects where the runbox config lives outside the default workspace root. When unset, `WORKSPACE_ROOT` defaults to the parent of the `runbox-run` script — correct for the Rust workspace where `runbox/runbox-run` sits one level inside the project root (`dev/run/` → `dev/`).
...
Python standalone project with config at `experiment/python_lib/runbox/runbox.yml`:
```yaml
workspace_root: ..
```
