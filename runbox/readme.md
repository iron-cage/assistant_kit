...
One shared image (`claude_journal_viewer_test`) serves the whole workspace: this
...
unique per checkout (not shared across sibling `yrd_core/` checkouts) to avoid
one checkout's rebake swapping the image out from under another.
