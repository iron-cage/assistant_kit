...
itself is not vendored here — it is provisioned separately and must already be
installed globally as `runbox` on `PATH`; the engine derives the workspace root
from this config's location, never from its own). The retired in-tree stack —
...
One shared image (`assistant_kit_claude_runner_test`) serves the whole workspace: this
...
unique to this checkout — sibling `assistant_kit` checkouts under the same
parent directory each declare their own, so a rebake here never swaps the image
out from under an unrelated checkout.
...
(schema, shared-image model, staleness contract, `.clean`) ships with the
engine itself — run `runbox .help` for the installed version's reference.
