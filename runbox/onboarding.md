...
How to integrate runbox test isolation into a new project. One `runbox/` directory, a handful of scripts, any ecosystem.

### Prerequisites

- `runbox-run` must be reachable by walking up from the project's `runbox/` directory (i.e. the project lives somewhere inside a directory tree that contains `runbox/runbox-run`).
- Docker or Podman installed on the host.

---

### Step 1 — `runbox/runbox` (copy verbatim, no edits)
...
### Step 2 — `runbox/runbox.yml` (project-specific config)
...
→ Full parameter reference: `runbox/docs/parameter/`

---

### Step 3 — `runbox/runbox.dockerfile` (ecosystem container image)
...
### Step 4 — `runbox/plugins.sh` (test lister override)
...
→ Full plugin reference: `runbox/docs/plugin/`
...
`verb/` has no knowledge of Docker or `runbox/`. The Docker path is owned entirely by `runbox/runbox`: it sets `VERB_LAYER=l1`, calls `verb/test.d/l1` directly inside the container, and returns results. `verb/test` never references `runbox/`.
...
```bash
chmod +x runbox/runbox runbox/verb-run
...
```bash
./runbox/runbox .build          # build the container image
./runbox/runbox .run            # run the entry point (requires run_script)
./runbox/runbox .test           # online tests (requires mounted credentials if configured)
./runbox/runbox .test.offline   # offline tests using seeded cache volume
./runbox/runbox .lint           # run linter (requires lint_script)
./runbox/runbox .list           # list tests via plugins.sh _plugin_list_cmd
./runbox/runbox .shell          # interactive shell with cache volume mounted
```
...
A verb can define separate implementations for different execution layers. `verb/` has no knowledge of Docker or `runbox/` — it only knows two contexts: host-native (l0, default) and container-internal (l1, set by whoever invokes the container). `runbox/runbox` orchestrates the Docker path independently, calling `verb/test.d/l1` directly inside the container without going through the dispatcher.
...
**Dispatch — `runbox/verb-run`:**

`runbox/verb-run` is simplified under the `.d/` convention. Since `verb/X` is always a file, dispatch is always direct:
...
Via Docker (runbox/runbox .test — run/ is fully independent of verb/):
  ./runbox/runbox .test → docker run ... verb/test.d/l1 directly → w3 .test level::3 ✓
```
