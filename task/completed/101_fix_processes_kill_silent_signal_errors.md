...
- `let _ = send_sigterm(p.pid)` and `let _ = send_sigkill(p.pid)` replaced with failure-accumulating loops that return `Err(InternalError)` when any signal delivery fails.
- Existing SIGTERM → wait → SIGKILL sequence logic unchanged.
- TC-315 already covers the signal error propagation path.
