...
- [x] C1 — Does `parse_json_string_value("\uD83D\uDE80")` return `Ok("🚀")`?
- [x] C2 — Does the implementation check `>= 0xD800 && <= 0xDBFF` to detect lead surrogates?
- [x] C3 — Does a lone lead surrogate return `Err`?
- [x] C4 — Does a lone trail surrogate return `Err`?
- [x] C5 — Is existing BMP `\u` behavior unchanged (TC-T07 passes)?

**Out of Scope confirmation**
- [x] C6 — Is `extract_releases` tag parsing logic unchanged?

### Measurements

- [x] M1 — surrogate decode: unit test `tc460_surrogate_pair_rocket` passes
- [x] M2 — lone lead error: unit test `tc461_lone_lead_surrogate_error` passes

### Invariants

- [x] I1 — test suite: `w3 .test level::3` → 0 failures
- [x] I2 — compiler clean: `RUSTFLAGS="-D warnings" cargo check --features enabled` → 0 warnings

### Anti-faking checks

- [x] AF1 — surrogate detection code: lead and trail range checks present in `src/commands.rs`
- [x] AF2 — pair computation: supplementary codepoint formula `0x10000 + ...` present in `src/commands.rs`

## Outcomes

Implemented UTF-16 surrogate pair decoding in `parse_json_string_value`. Lead surrogates (U+D800–U+DBFF) now trigger consumption of a trailing surrogate (U+DC00–U+DFFF); the supplementary code point is computed as `0x10000 + (lead - 0xD800) * 0x400 + (trail - 0xDC00)` and encoded as UTF-8. Lone surrogates return `Err`. Also fixed `\b` (→ `\x08`) and `\f` (→ `\x0C`) escape sequences that were previously missing.
