...
- `require_claude_paths()` now has three distinct branches: HOME unset (`Err`) → "not set"; HOME empty (`Ok("")`) → "is empty"; HOME set but path resolution fails → "path resolution failed".
- TC-320b already existed and passes (HOME="" → error mentions "HOME").
- All acceptance criteria met; `w3 .test level::3` passes.
