...
- `version_install_routine` calls `OutputOptions::from_cmd()` and passes `opts.format`/`opts.verbosity` to `install_dry_content()` and the live-install output branch.
- `version_guard_routine` calls `OutputOptions::from_cmd()` and passes `opts.verbosity` and `opts.format` to `guard_once()`, `guard_once_latest()`, and `guard_once_pinned()`.
- `.processes.kill` has `v()` and `fmt()` registered in `lib.rs` and `unilang.commands.yaml`; `processes_kill_routine` calls `OutputOptions::from_cmd()` and branches on both `opts.format` and `opts.verbosity`.
- TC-361 (`.version.install dry::1 format::json` → JSON), TC-362 (`format::JSON` → exit 1), TC-313 (`.processes.kill v::0` → exit 0), TC-314 (`format::JSON` → exit 1), TC-316 (`.processes.kill dry::1 format::json` → JSON), TC-417 (`.version.guard dry::1 v::0` shorter than `v::1`), TC-416 (`version::latest` override) all pass.
- All acceptance criteria met; `w3 .test level::3` passes with zero failures.
