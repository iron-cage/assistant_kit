...
One `printf` line per verb. Columns: verb name (13 chars), status (11 chars), command string. Status values: `available`, `unavailable`, `built-in`.
...
`verbs` and `package_info` always show `built-in` status — they are defined by the protocol itself and always present, so they report no specific command.
...
test          available  runbox .live (container, module-scoped)
test_only     available  cargo nextest run --all-features <filter>
