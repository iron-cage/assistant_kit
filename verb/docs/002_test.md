...
|-------|---------|--------|---------------------|---------|
| runbox | host → Docker via `runbox/runbox .test` | yes | yes (inside container) | yes — no `VERB_LAYER` set |
| `l0` | host-native | no | no | no — `VERB_LAYER=l0` only |
| `l1` | container-internal | n/a | yes | no — `VERB_LAYER=l1` (set by runbox-run) |

### Notes

`verb/test` (default, no `VERB_LAYER`) calls `../runbox/runbox .test` — Docker is the default. `runbox/runbox` handles image management and mounts, then executes `verb/test.d/l1` inside the container.
...
`--dry-run` prints `./runbox/runbox .test` and exits 0 — no tests run.

### Example

```bash
# Default — Docker via runbox (any module):
./verb/test               # runs: ./runbox/runbox .test  →  container  →  w3 .test level::3
./verb/test --dry-run     # prints: ./runbox/runbox .test
...
Each module's `runbox/runbox.yml` sets `test_script: module/<name>/verb/test.d/l1` — the container entry point is `l1` directly. `runbox-run` injects `VERB_LAYER=l1` as a safety guard, but the explicit path makes container execution robust without relying on it.
